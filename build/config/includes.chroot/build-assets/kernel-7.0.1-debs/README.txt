Linux 7.0.1 kernel .deb archive (optional — gitignored blobs)
=============================================================
...
That writes **`linux-7.0.1-debs-for-iso.tar.gz`** in this directory (or set **`OUT=`**).
...
On the build host (or before `iso-preflight` / `lb-docker-build`):
  ...
  • Keep debs only under **`KERNEL_WORK`** (default `../kernel-7.0.1-work`) and run:
