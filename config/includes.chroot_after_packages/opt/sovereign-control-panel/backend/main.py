...
async def toggle_feature(feature_id: str, state: str):
    ...
    if feature_id not in logic_map:
        # Graceful fallback for mocked features in the UI
        print(f"[Simulated] Executing {state} on {feature_id}")
        return {"status": "success", "detail": f"Simulated execution for {feature_id}"}
