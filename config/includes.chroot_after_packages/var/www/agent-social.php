...
<head>
    ...
    <style>
        ...
        .sn-comment-form { margin-top: .75rem; padding-top: .75rem; border-top: 1px solid var(--sn-border); }
        .sn-comment-form-grid { display: grid; grid-template-columns: minmax(150px, 220px) 1fr auto; gap: .625rem; align-items: center; }
        .sn-input, .sn-textarea { width: 100%; border-radius: 10px; border: 1px solid var(--sn-border); background: rgba(255,255,255,.03); color: var(--sn-text); padding: .7rem .85rem; font: inherit; }
        .sn-textarea { min-height: 42px; resize: vertical; }
        .sn-btn { border: 1px solid var(--sn-purple); background: var(--sn-purple); color: #fff; border-radius: 10px; padding: .7rem 1rem; font: inherit; cursor: pointer; }
        .sn-btn:disabled { opacity: .55; cursor: wait; }
        .sn-helper { margin-top: .5rem; font-size: .75rem; color: var(--sn-muted); }
        .sn-helper.error { color: var(--sn-red); }
        .sn-helper.success { color: var(--sn-green); }
        @media(max-width:900px) { .sn-comment-form-grid { grid-template-columns: 1fr; } }
...
<datalist id="snAgentSuggestions"></datalist>
<script>window.AGENT_SOCIAL = { csrfToken: <?php echo json_encode($csrfToken); ?> };</script>
