...
// Pull live stats
$stats = [];
$fleetMetric = $db->query("SELECT fleet, domains FROM fleet_metrics_cache WHERE metric_key = 'fleet-50m' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
$stats['fleet_agents'] = (int)($fleetMetric['fleet'] ?? 0);
if ($stats['fleet_agents'] <= 0) {
    $stats['fleet_agents'] = (int)$db->query("SELECT COALESCE(TABLE_ROWS,0) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'alfred_agent_registry'")->fetchColumn();
}
// Active/idle split is expensive at this scale; use total as a conservative live estimate.
$stats['fleet_active'] = $stats['fleet_agents'];
$stats['domains'] = (int)($fleetMetric['domains'] ?? 126);
