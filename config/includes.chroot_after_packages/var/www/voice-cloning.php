...
$page_canonical   = 'https://root.com/voice-cloning.php';
...
.vc-profile-actions button:disabled {
    cursor: default;
    opacity: .7;
}
.vc-profile-actions button:disabled:hover {
    border-color: var(--vc-border);
    color: var(--vc-text);
}
.vc-profiles-empty {
    border: 1px dashed var(--vc-border-hover);
    border-radius: var(--vc-radius);
    padding: 28px;
    text-align: center;
    color: var(--vc-text-muted);
...
<div class="vc-page">
    ...
    <section class="vc-section">
        <div class="vc-container">
            ...
            <p class="vc-section-sub" data-aos="fade-up" data-aos-delay="50">Manage the voice sample profiles you have uploaded from this account.</p>

            <?php if ($is_logged_in): ?>
            <div class="vc-profile-note" data-aos="fade-up" data-aos-delay="75">
                Uploaded samples appear here immediately. Profile training and playback only become available after a voice model is connected.
            </div>
            <div class="vc-profiles-grid" id="vcProfilesGrid" data-aos="fade-up" data-aos-delay="100">
                <div class="vc-profiles-empty" id="vcProfilesEmpty">
                    <i class="fa-solid fa-wave-square"></i>
                    <div>Loading your uploaded voice profiles...</div>
                </div>
            </div>
            <?php else: ?>
            <div class="vc-login-prompt" data-aos="fade-up" data-aos-delay="100">
...
<?php if ($is_logged_in): ?>
<script>
window.VOICE_CLONE_CSRF_TOKEN = <?php echo json_encode($_SESSION['csrf_token'] ?? ''); ?>;
window.GSM_VOICE_CLONE_USER = {
    id: <?php echo (int)$client_id; ?>,
    name: <?php echo json_encode($client_name); ?>,
    email: <?php echo json_encode($client_email); ?>
};
</script>
<script src="/assets/js/voice-cloning-engine.js?v=<?php echo (int) filemtime(__DIR__ . '/assets/js/voice-cloning-engine.js'); ?>"></script>
<?php endif; ?>
