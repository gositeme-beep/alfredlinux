...
$REDIRECT_URI = 'https://root.com/tiktok-callback';
...
if (isset($_GET['action']) && $_GET['action'] === 'callback') {
    ...
    if (isset($_GET['error'])) {
        echo json_encode(['error' => htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8'), 'description' => htmlspecialchars($_GET['error_description'] ?? '', ENT_QUOTES, 'UTF-8')]);
        exit;
    }
