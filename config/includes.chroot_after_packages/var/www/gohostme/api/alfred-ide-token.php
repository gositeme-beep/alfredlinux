...
require_once dirname(__DIR__) . '/includes/alfred-oauth-uri.inc.php';
...
function handleToken(PDO $db): void {
    ...
    // Normalize incoming redirect_uri to the same canonical form that
    // /alfred-login.php stored when the auth code was issued. This lets
    // the IDE send whatever vscode.env.asExternalUri() returned (loopback,
    // <port>.gositeme.com, github.dev tunnel, …) and still match.
    $normalizedRedirect = alfredNormalizeRedirectUri($redirectUri, 35721, [
        'referer' => '',  // no browser context at exchange time
        'host'    => 'gositeme.com',  // exchange always hits gositeme.com
    ]);
    ...
    $stmt = $db->prepare("
        ...
        LIMIT 1
    ");
    $stmt->execute([$code, $normalizedRedirect]);
...
function jsonOut(array $data, int $status = 200): never {
    ...
    echo json_encode($data, gositeme_json_public_encode_flags());
    exit;
}
