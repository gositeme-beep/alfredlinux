...
switch ($action) {
    ...
    case 'performance':
        ...
        break;
    default:
        jsonResponse(['error' => 'Invalid action. Valid: create, list, history, status, deploy, cancel, pause, delete, add_agent, remove_agent, dashboard, performance'], 400);
}
...
/**
 * Cancel fleet (stop active work and retire it from active view)
 */
function cancelFleet($clientId) {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        jsonResponse(['error' => 'POST required'], 405);
    }
    ...
    $fleetId = intval($input['fleet_id'] ?? 0);
    if ($fleetId <= 0) {
        jsonResponse(['error' => 'fleet_id is required'], 400);
    }
    ...
    try {
        ...
        $results['cancelled_at'] = date('c');
        $results['cancelled_by'] = $clientId;
        ...
        $stmt = $db->prepare("UPDATE alfred_fleet_agents SET status = 'cancelled', completed_at = NOW() WHERE fleet_id = ? AND status IN ('queued', 'running')");
        $stmt->execute([$fleetId]);

        jsonResponse([
            'success' => true,
            'message' => "Fleet '{$fleet['fleet_name']}' cancelled",
            'fleet_id' => $fleetId,
            'status' => 'cancelled',
        ]);
    } catch (\Exception $e) {
        error_log('Fleet: cancel failed: ' . $e->getMessage());
        jsonResponse(['error' => 'Failed to cancel fleet'], 500);
    }
}
    ...
    try {
        ...
        $stmt = $db->prepare("
            SELECT
                ...
                COALESCE(AVG(TIMESTAMPDIFF(SECOND, started_at, completed_at)), 0) as avg_time
            FROM alfred_fleet_agents a
            JOIN alfred_fleets f ON a.fleet_id = f.id
            WHERE f.user_id = ?
        ...
        ");
        $stmt->execute([$clientId]);
        $taskStats = $stmt->fetch() ?: ['tasks_completed' => 0, 'avg_time' => 0];
        ...
        jsonResponse([
            ...
            'dashboard' => [
                ...
                'active_fleet_count' => (int) ($statusMap['running'] ?? 0),
                'tasks_completed' => (int) ($taskStats['tasks_completed'] ?? 0),
                'avg_time' => (int) round((float) ($taskStats['avg_time'] ?? 0)),
...
/**
 * Fleet history for the current user
 */
function getFleetHistory($clientId) {
    $db = getDB();
    if (!$db) {
        jsonResponse(['error' => 'Database connection failed'], 500);
    }
    ...
    try {
        $stmt = $db->prepare("
            SELECT id, fleet_name, objective, strategy, status, progress_percent, agent_count, completed_at, created_at
            FROM alfred_fleets
            WHERE user_id = ?
        ...
        foreach ($fleets as &$fleet) {
            $fleet['id'] = (int) $fleet['id'];
            $fleet['agent_count'] = (int) ($fleet['agent_count'] ?? 0);
            $fleet['progress_percent'] = (int) ($fleet['progress_percent'] ?? 0);
        }

        jsonResponse(['success' => true, 'fleets' => $fleets, 'total' => count($fleets)]);
    } catch (\Exception $e) {
        error_log('Fleet: history failed: ' . $e->getMessage());
        jsonResponse(['error' => 'Failed to load fleet history'], 500);
    }
}

/**
 * Performance summary for fleet dashboard widgets
 */
function getPerformance($clientId) {
    $db = getDB();
    if (!$db) {
        jsonResponse(['error' => 'Database connection failed'], 500);
    }
    ...
    try {
        $stmt = $db->prepare("
            SELECT
                ...
                SUM(a.status = 'failed') as failed_agents,
                SUM(a.status = 'completed') as completed_agents,
                COALESCE(AVG(TIMESTAMPDIFF(SECOND, a.started_at, a.completed_at)), 0) as avg_time,
                SUM(a.completed_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR) AND a.status = 'completed') as completed_last_hour
            FROM alfred_fleet_agents a
            JOIN alfred_fleets f ON a.fleet_id = f.id
            WHERE f.user_id = ?
        ");
        ...
        $cpuLoad = min(100, max(0, (int) round(((float) ($load[0] ?? 0)) * 10)));
        $totalAgents = max(1, (int) ($stats['total_agents'] ?? 0));
        $failedAgents = (int) ($stats['failed_agents'] ?? 0);
        $avgTime = (int) round((float) ($stats['avg_time'] ?? 0));

        jsonResponse([
            'success' => true,
            'metrics' => [
                'cpuLoad' => $cpuLoad,
                'memUsage' => min(100, max(0, (int) round((memory_get_usage(true) / max(memory_get_peak_usage(true), 1)) * 100))),
                'taskThroughput' => (int) ($stats['completed_last_hour'] ?? 0),
                'errorRate' => round(($failedAgents / $totalAgents) * 100, 1),
                'avgLatency' => $avgTime > 0 ? $avgTime * 1000 : 0,
                'uptime' => $failedAgents > 0 ? round(max(90, 100 - (($failedAgents / $totalAgents) * 100)), 2) : 100,
            ],
        ]);
    } catch (\Exception $e) {
        error_log('Fleet: performance failed: ' . $e->getMessage());
        jsonResponse(['error' => 'Failed to load performance metrics'], 500);
    }
}
