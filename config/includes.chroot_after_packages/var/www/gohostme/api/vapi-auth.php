...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    echo json_encode(['result' => 'error', 'message' => 'Method not allowed'], gositeme_json_public_encode_flags());
    exit;
}
...
if ($vapiSecret) {
    ...
    if (!hash_equals($vapiSecret, $receivedSecret)) {
        ...
        echo json_encode(["error" => "Unauthorized"], gositeme_json_public_encode_flags());
        exit;
    }
}
    ...
    if ($toolCallId) {
        return [
            'results' => [[
                ...
                'result'     => json_encode($data, gositeme_json_public_encode_flags())
            ]]
        ];
    }
    return $data;
