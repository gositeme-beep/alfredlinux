...
unset($envCandidates, $envFile);

// ── Auth check — determine detail level ──────────────────────
if (session_status() === PHP_SESSION_NONE) { session_start(); }
$isAdmin = (int)($_SESSION['client_id'] ?? 0) === 33;
$internalSecret = getenv('INTERNAL_SECRET') ?: '';
$isInternal = !empty($_SERVER['HTTP_X_INTERNAL_SECRET']) && $internalSecret !== '' && hash_equals(
    $internalSecret, $_SERVER['HTTP_X_INTERNAL_SECRET']
);

// ── Cache layer (file-based, 30 s TTL) ──────────────────────
$cacheDir  = __DIR__ . '/../cache';
$cacheFile = $cacheDir . (($isAdmin || $isInternal) ? '/health-check-full.json' : '/health-check-public.json');
$cacheTTL  = 30; // seconds
...
function checkPQCrypto(): array {
    try {
        ...
        $pdo = getDB();
        if ($pdo === null) {
            return ['status' => 'unknown', 'algorithm' => 'Kyber-1024-Hybrid'];
        }
        ...
        return [
            ...
            'scheme'    => 'ECDH-P256 + Kyber-1024 → HKDF-SHA256 → AES-256-GCM',
        ];
    } catch (\Throwable $e) {
        return ['status' => 'unknown', 'algorithm' => 'Kyber-1024-Hybrid'];
    }
...
/**
 * Check recent Vapi alert volume to catch tool proxy and routing failures quickly.
 */
function checkVapiAlerts(): array {
    try {
        require_once __DIR__ . '/config.php';
        $pdo = getDB();
        if ($pdo === null) {
            return ['status' => 'unknown', 'window_minutes' => 60];
        }

        $row = $pdo->query("SELECT COUNT(*) AS total_events, COALESCE(SUM(CASE WHEN severity = 'warn' THEN 1 ELSE 0 END), 0) AS warn_events, COALESCE(SUM(CASE WHEN severity IN ('critical','emergency') THEN 1 ELSE 0 END), 0) AS critical_events, COALESCE(SUM(CASE WHEN event_type IN ('assistant.request_failed','tool.proxy_failed','tool.proxy_invalid_response','tool.call_missing_name','call.transfer_route_missing') THEN 1 ELSE 0 END), 0) AS action_failures, MAX(created_at) AS last_event_at FROM alfred_event_log WHERE subsystem = 'vapi' AND created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)")->fetch(PDO::FETCH_ASSOC) ?: [];

        $warnEvents = (int)($row['warn_events'] ?? 0);
        $criticalEvents = (int)($row['critical_events'] ?? 0);
        $actionFailures = (int)($row['action_failures'] ?? 0);

        $status = 'up';
        if ($criticalEvents > 0 || $actionFailures >= 3) {
            $status = 'down';
        } elseif ($warnEvents >= 10 || $actionFailures > 0) {
            $status = 'degraded';
        }
...
// Overall status — exclude optional/non-critical services
$optionalServices = ['pq_crypto', 'tts_server', 'icecast', 'livekit'];
$allUp    = true;
$anyDown  = false;
foreach ($services as $name => $svc) {
    if (in_array($name, $optionalServices, true)) continue;
