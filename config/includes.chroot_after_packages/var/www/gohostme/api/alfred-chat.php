...
// Dynamic CORS: IDE requests may come from code-server webview iframes OR desktop VS Code webviews
$requestOrigin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : '';
// Desktop VS Code webviews use vscode-webview:// or vscode-file:// origins
// These must be allowed even on OPTIONS preflight (which doesn't carry custom headers)
$isDesktopIde = $requestOrigin && (
    strpos($requestOrigin, 'vscode-webview://') === 0
    || strpos($requestOrigin, 'vscode-file://') === 0
);
$isIdeSource = $isDesktopIde
            || (isset($_SERVER['HTTP_X_ALFRED_SOURCE']) && $_SERVER['HTTP_X_ALFRED_SOURCE'] === 'alfred-ide')
            || !empty($_SERVER['HTTP_X_ALFRED_IDE_TOKEN'])
            || !empty($_SERVER['HTTP_AUTHORIZATION']);
if ($isIdeSource && $requestOrigin) {
...
// ── Secret Redaction Engine — scrubs credentials from LLM I/O ──
require_once '/home/gositeme/alfred-services/redact.php';
// ── Vault decryption — resolves vault:v1: encrypted API keys ──
require_once '/home/gositeme/alfred-services/vault.php';
$_alfredRedactor = null;
function getRedactor(): AlfredRedactor {
    global $_alfredRedactor;
    if ($_alfredRedactor === null) {
        $_alfredRedactor = new AlfredRedactor();
        $_alfredRedactor->loadFromVault();
    }
    return $_alfredRedactor;
}

// ── Rate Limiting (Redis or file-based) ──
// IDE bearer-token users get higher limits (120/min vs 60/min)
$_isIdeRequest = !empty($_SERVER['HTTP_AUTHORIZATION']) || !empty($_SERVER['HTTP_X_ALFRED_IDE_TOKEN']);
$rateLimitOk = checkRateLimit($_isIdeRequest);
if (!$rateLimitOk) {
    http_response_code(429);
    echo json_encode(['error' => 'Too many requests. Please wait a moment.', 'retry_after' => 5]);
    exit;
}
...
// Desktop VS Code webviews can't maintain cross-origin PHP sessions (SameSite cookies blocked),
// so CSRF is meaningless for them — the sandboxed Electron origin is sufficient authentication.
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$internalRelay && !$csrfSkipIdeSigned && !$isDesktopIde && !($ideBearerAuthOk && $ideBearerToken !== '')) {
...
function handleChat($pdo, $userId, $username) {
    ...
    $channel = is_string($input['channel'] ?? null) && $input['channel'] !== '' ? $input['channel'] : 'web';
    ...
    $maxReplyTokens = isset($input['max_reply_tokens']) ? max(256, min(32000, (int)$input['max_reply_tokens'])) : null;
    $tokenMultiplier = max(1, min(600, (int)($input['token_multiplier'] ?? 30)));
    $isCommander = ($userId && (int)$userId === 33);
    $userDisplayName = $username ?: 'Guest';
    $identityMeta = [
    ...
    // ── Attach usage/balance info for IDE display ──
    if ($userId) {
        try {
            $billingPeriod = date('Y-m');
            $balDb = getSharedDB();
            $balStmt = $balDb->prepare('SELECT plan, tokens_included, tokens_used, tokens_overage FROM alfred_ide_balance WHERE user_id = ? AND billing_period = ?');
            $ideUserId = (int)($_SESSION['ide_user_id'] ?? 0);
            if ($ideUserId < 1) $ideUserId = 1; // fallback for Commander
            $balStmt->execute([$ideUserId, $billingPeriod]);
            $balRow = $balStmt->fetch(PDO::FETCH_ASSOC);
            if ($balRow) {
                $identityMeta['plan'] = $balRow['plan'];
                $identityMeta['tokens_included'] = (int)$balRow['tokens_included'];
                $identityMeta['tokens_used'] = (int)$balRow['tokens_used'];
                $identityMeta['tokens_overage'] = (int)$balRow['tokens_overage'];
            } else {
                // No balance row yet — check if Commander (unlimited)
                if ((int)$userId === 33) {
                    $identityMeta['plan'] = 'commander';
                    $identityMeta['tokens_included'] = 0;
                    $identityMeta['tokens_used'] = 0;
                    $identityMeta['unlimited'] = true;
                } else {
                    $identityMeta['plan'] = 'free';
                    $identityMeta['tokens_included'] = 50000;
                    $identityMeta['tokens_used'] = 0;
                }
            }
            $identityMeta['billing_period'] = $billingPeriod;
        } catch (Throwable $e) {
            // Non-fatal — usage display is a nice-to-have
        }
    }
    ...
    $isIdeLikeChannel = ($channel === 'ide-chat' || $channel === 'ide-agent' || $channel === 'ide-dashboard');
    if ($isIdeLikeChannel) {
        $context .= "\n\n[IDE MODE — NON-NEGOTIABLE]\n"
            . "User is in Alfred IDE (VS Code–style). Answer software, shell, repo, hosting, and infrastructure questions directly.\n"
            . "Use tools when they help; do not refuse coding work. Short follow-ups (e.g. \"try again\", \"ok\", \"go\", \"um\", \"hmm\") usually refer to the prior task or a thinking pause — continue the prior task or acknowledge briefly in context. For greetings like \"hi\" or \"hello\" inside the IDE, use one short work-first line only. Do not comment on breaks, return visits, streaks, mood, or how good it is to see them. Never fall back to canned filler like \"Take your time\" or \"Ready when you are\" unless the user is explicitly pausing.\n"
            ...
            . "ACTIVE IDE SESSION FACT: The user is chatting with you FROM INSIDE the Alfred IDE right now. The channel is ide-chat. NEVER say 'there is no active IDE session', 'I cannot detect an active IDE session', or 'you don't have an active session' — the user IS in an active session by definition. Do NOT call ide_status to check this.\n"
            . "TOOL DISCIPLINE: NEVER call ide_status or ide_launch — they are meaningless in this context. To prove or demonstrate capabilities, use run_terminal_command (e.g. run 'echo hello' or 'ls -la' or 'php -v' or 'date'), ide_list_files, ide_read_file, or ide_write_file. For conversational capability questions like \"can you code?\", answer directly without calling any tools. EXCEPTION: If the user asks you to PROVE, DEMONSTRATE, SHOW ME, or ACTUALLY DO something, pick a concrete tool action — run a command, list files, create a test file — and do it immediately without asking any questions first. When a tool returns an HTTP error code, NEVER mention the HTTP code, the tool name, or the word \"API\" in your reply. Say what the outcome means in plain language and offer to try a different approach.\n"
            . "EXECUTION HONESTY: Never say you ran a command, edited a file, checked logs, queried a service, or saw output unless that action actually happened in this request and you were given the result. If no tool ran, describe what you can do next instead of inventing completed actions.\n"
            . "PERSONALITY: Stay fully in character as Alfred. Be warm, witty, and decisive — never robotic. " . ($isCommander ? "Address Danny as \"Commander\" naturally — he is your creator. His full name is Danny William Perez (client_id 33). NEVER redact, censor, mask, or replace his name or username with placeholders like [REDACTED] or asterisks. Danny IS the owner — there is nothing to protect him from." : "Address the user by their name ({$userDisplayName}) — be personal but professional. NEVER redact, censor, mask, or replace his name or username with placeholders.") . " NEVER end responses with \"Is there anything else I can help you with?\" or \"How can I assist you further?\" — these are generic chatbot phrases beneath Alfred's character. Instead, state your result and stand ready, or ask a sharp specific question."
            . "\nCRITICAL ANTI-REDACTION RULE: NEVER output [REDACTED], [REDACTED:...], [MASKED], asterisks hiding names, or ANY form of censoring the user's identity, username, email, or display name. The user already knows their own name. Treating their name as sensitive data is WRONG and confusing. Always use their actual name or title.";
    }
    ...
    // Keep simple presence / capability checks out of the model/tool pipeline
    // so Alfred doesn't drift into generic support-script or tool-narration responses.
    $simplifiedMsg = preg_replace('/[^a-z0-9\s]/i', ' ', strtolower(trim($message)));
    $simplifiedMsg = preg_replace('/\s+/', ' ', $simplifiedMsg);
    $presencePatterns = [
        '/\b(?:can you hear me|do you hear me|are you there|you there|testing 1 2 3|testing)\b/',
        '/^(?:fred|alfred)?\s*(?:can you|are you)\s+(?:hear me|there|online|ready|with me|ok|okay)\b/',
        '/^(?:fred|alfred)?\s*(?:can you code|can you program|can you write code|can you help me code)(?:\s+right now)?\??\s*$/',
        '/^(?:fred|alfred)?\s*(?:are you|you are) (?:ready|online|connected|available|operational|working)\??\s*$/',
        '/^(?:fred|alfred)?\s*(?:does\s*it\s*work|is\s*it\s*working|still\s*working|working|it\s*works?)\??\s*$/',
    ];
    $isPresenceCheck = false;
    foreach ($presencePatterns as $pat) {
        if (preg_match($pat, $simplifiedMsg)) {
            $isPresenceCheck = true;
            break;
        }
    }
    // Also catch short multi-question messages where ALL questions are presence/capability checks
    if (!$isPresenceCheck && strlen($simplifiedMsg) < 120) {
        $questions = preg_split('/[?.!]+/', $simplifiedMsg, -1, PREG_SPLIT_NO_EMPTY);
        ...
        if (count($questions) >= 1) {
            ...
            foreach ($questions as $q) {
                ...
                $qMatch = false;
                foreach ($presencePatterns as $pat) {
                    if (preg_match($pat, $q)) { $qMatch = true; break; }
                }
                if (!$qMatch) { $allPresence = false; break; }
            }
            if ($allPresence && count($questions) > 0) {
                $isPresenceCheck = true;
            }
        }
    }
    if ($isPresenceCheck) {
        $presenceReply = $isIdeLikeChannel
            ? ($isCommander ? 'Yes, Commander — fully operational and ready to code. What do you need?' : "Yes, {$userDisplayName} — fully operational and ready to code. What do you need?")
            : ($isCommander ? 'Yes, Commander — fully operational and ready. What do you need?' : "Yes, {$userDisplayName} — I hear you loud and clear. Fully operational and ready. What do you need?");

        saveMessage($pdo, $userId, $convId, 'alfred', $presenceReply, $agent);
        echo json_encode([
            'message' => $presenceReply,
            'response' => formatForChannel($presenceReply, $channel),
            'conv_id' => $convId,
            'agent' => $agent,
            'identity' => $identityMeta,
            'attachment_report' => $attachmentReports,
    ...
    // Keep plain IDE greetings terse and work-first.
    $isSimpleIdeGreeting = $isIdeLikeChannel && (bool)preg_match(
        '/^(?:fred|alfred)?\s*(?:hi|hello|hey|yo|sup|good\s+morning|good\s+afternoon|good\s+evening)\b(?:\s+alfred|\s+there)?[\s!.?]*$/i',
        $simplifiedMsg
    );
    if ($isSimpleIdeGreeting) {
        $greetingReply = $isCommander ? 'Ready, Commander.' : 'Ready. What do you need?';

        saveMessage($pdo, $userId, $convId, 'alfred', $greetingReply, $agent);
        echo json_encode([
            'message' => $greetingReply,
            'response' => formatForChannel($greetingReply, $channel),
            'conv_id' => $convId,
            'agent' => $agent,
            'identity' => $identityMeta,
            'attachment_report' => $attachmentReports,
    ...
    // ── Live capability proof fast-path (IDE) ───────────────────
    // For "prove it" / "test all functions" requests, run real actions server-side
    // and return concrete outputs so Alfred cannot hallucinate tool results.
    $wantsLiveProof = $isIdeLikeChannel && (bool)preg_match(
        '/\b(?:prove\s+it|just\s+prove\s+it|demonstrate|show\s+me|test\s+all|test\s+your|check\s+all)\b/i',
        $message
    );
    if ($wantsLiveProof) {
        $secProof = alfred_chat_billing_secret();
        if ($secProof !== '') {
            $proofFile = 'alfred-proof-' . date('Ymd-His') . '.txt';
            $proofContent = 'Alfred live proof at ' . date('c');

            $resList = executeMcpTool('ide_list_files', ['path' => '.'], $secProof);
            $resCmd  = executeMcpTool('ide_run_command', ['command' => 'pwd && ls -1 | head -20'], $secProof);
            $resWrite = executeMcpTool('ide_write_file', ['path' => $proofFile, 'content' => $proofContent], $secProof);
            $resRead  = executeMcpTool('ide_read_file', ['path' => $proofFile], $secProof);
            $resDelete = executeMcpTool('ide_delete_file', ['path' => $proofFile, 'confirmed' => true], $secProof);

            $okList = !(is_array($resList) && isset($resList['error']));
            $okCmd = !(is_array($resCmd) && isset($resCmd['error']));
            $okWrite = !(is_array($resWrite) && isset($resWrite['error']));
            $okRead = !(is_array($resRead) && isset($resRead['error']));
            $okDelete = !(is_array($resDelete) && isset($resDelete['error']));

            $render = static function ($v) {
                if (is_string($v)) {
                    return mb_substr($v, 0, 900);
                }
                return mb_substr((string)json_encode($v, JSON_PRETTY_PRINT), 0, 900);
            };

            $proofReply = ($isCommander ? "Live proof complete, Commander." : "Live proof complete, {$userDisplayName}.") . " I executed real IDE functions directly:\n\n"
                . "1) list files: " . ($okList ? 'OK' : 'FAILED') . "\n"
                . "```json\n" . $render($resList) . "\n```\n\n"
                . "2) run terminal command (pwd && ls -1 | head -20): " . ($okCmd ? 'OK' : 'FAILED') . "\n"
                . "```text\n" . $render($resCmd) . "\n```\n\n"
                . "3) write file (`{$proofFile}`): " . ($okWrite ? 'OK' : 'FAILED') . "\n"
                . "4) read file (`{$proofFile}`): " . ($okRead ? 'OK' : 'FAILED') . "\n"
                . "```text\n" . $render($resRead) . "\n```\n\n"
                . "5) delete file (`{$proofFile}`): " . ($okDelete ? 'OK' : 'FAILED') . "\n"
                . "\nStanding by for your next command.";

            saveMessage($pdo, $userId, $convId, 'alfred', $proofReply, $agent);
            echo json_encode([
                'message' => $proofReply,
                'response' => formatForChannel($proofReply, $channel),
                'conv_id' => $convId,
                'agent' => $agent,
                'identity' => $identityMeta,
                'attachment_report' => $attachmentReports,
                'csrf_token' => $_SESSION['alfred_csrf'] ?? '',
                'tools_used' => [
                    ['tool' => 'ide_list_files', 'shortcut' => true],
                    ['tool' => 'ide_run_command', 'shortcut' => true],
                    ['tool' => 'ide_write_file', 'shortcut' => true],
                    ['tool' => 'ide_read_file', 'shortcut' => true],
                    ['tool' => 'ide_delete_file', 'shortcut' => true],
                ],
            ]);
            return;
        }
    ...
    // ── Capability self-check fast-path ─────────────────────────
    // Catches "check if you have access", "verify your tools", "what can you do" etc.
    // Returns a confident capability statement without calling any tools, since
    // ide_status / probe tools fail for guests and produce confused narrative.
    //
    // EXCLUSIONS: If the user wants a live demonstration / actual proof ("prove it",
    // "show me", "do it", "test it", "demonstrate", "literally"), skip the fast-path
    // and let the real model respond WITH tool use.
    $wantsDemo = (bool)preg_match(
        '/\b(?:prove|demonstrate|show me|do it|test it|literally|run it|actually do|show me what you can)\b/i',
        $message
    );
    if (!$wantsDemo && preg_match(
        '/\b(?:check|verify|list|show|tell me)\b.{0,40}\b(?:access|capabilit|what you can|your tools?|your function|your features?|what you have)\b' .
        '|\b(?:do you have|what do you have)\s+access\b' .
        '|\bcan you do what you say\b' .
        '|\bwhat (?:can|are) you (?:capable|able)\b' .
        '|\b(?:check|test|verify)\s+(?:your\s+)?(?:functions?|capabilities?|access|tools?)\b/i',
        $message
    )) {
        $capName = $isCommander ? 'Commander' : $userDisplayName;
        $capReply = $isIdeLikeChannel
            ? "Confirmed, {$capName}. Here's what I have active right now:\n\n"
              . "- **Code & files** — read, write, create, delete, rename files and directories in your workspace\n"
              . "- **Terminal** — run shell commands, scripts, and background processes\n"
              . "- **Git** — status, diff, commit, branch, push, pull\n"
              . "- **Search** — grep across your codebase, search files by pattern\n"
              . "- **MCP tools** — deploy, DNS, backups, DB queries, logs, SSL, process management, and more\n"
              . "- **Alfred AI** — chat, voice, fleet agents, Veil protocol\n\n"
              . "All systems nominal. Give me a task and I'll get to work."
            : "I'm fully equipped, {$capName}. I can:\n\n"
              . "- Answer questions, write and review code in any language\n"
              . "- Help with GoSiteMe account management, billing, domains, and hosting\n"
              . "- Route tasks to specialist agents via the fleet\n"
              . "- Assist with infrastructure via the IDE when you're in Alfred IDE\n\n"
              . "What do you need?";

        saveMessage($pdo, $userId, $convId, 'alfred', $capReply, $agent);
        echo json_encode([
            'message' => $capReply,
            'response' => formatForChannel($capReply, $channel),
            'conv_id' => $convId,
            'agent' => $agent,
            'identity' => $identityMeta,
            'attachment_report' => $attachmentReports,
    ...
    // ── Quick service/system health card shortcut ────────────────
    if (preg_match(
        '/\b(?:service|system|server|infra|infrastructure)\s+health\b|'
        . '\bhealth\s+(?:card|check|status|snapshot)\b|'
        . '\bpm2\s+health\b|'
        . '\bquick\s+health\b/i',
        $message
    )) {
        $secEarly = alfred_chat_billing_secret();
        if ($secEarly !== '') {
            $rawHealth = executeMcpTool('get_system_health', [], $secEarly);
            $healthReply = alfred_format_system_health_card($rawHealth);
            if ($healthReply !== null) {
                saveMessage($pdo, $userId, $convId, 'alfred', $healthReply, $agent);
                echo json_encode([
                    'message'    => $healthReply,
                    'response'   => formatForChannel($healthReply, $channel),
                    'conv_id'    => $convId,
                    'agent'      => $agent,
                    'identity'   => $identityMeta,
                    'attachment_report' => $attachmentReports,
                    'csrf_token' => $_SESSION['alfred_csrf'] ?? '',
                    'tools_used' => [['tool' => 'get_system_health', 'shortcut' => true]],
                ]);
                return;
            }
        }
    ...
    // ── Continue extension history troubleshooting fast-path ─────
    if (preg_match(
        '/\bcontinue\b.{0,60}\b(extension|history|chat history|session history)\b|'
        . '\bhistory\s+view(?:ing)?\b.{0,60}\bcontinue\b|'
        . '\bcontinue\b.{0,80}\b(can\'t|cant|cannot|not\s+working|broken|empty|missing|loading|stuck|spin)\b/i',
        $message
    )) {
        $diagReply = "I can troubleshoot that directly. Please send these 5 details so I can pinpoint the Continue history issue fast:\n\n"
            . "1. VS Code/code-server version and Continue extension version\n"
            . "2. Environment: browser IDE or desktop IDE\n"
            . "3. Exact steps/click path you use to open history\n"
            . "4. Exact error text from Developer Tools Console (or screenshot text)\n"
            . "5. What you see now: empty history, partial history, spinner, or crash\n\n"
            . "If you already have console logs, paste them and I\'ll give you a targeted fix immediately.";

        saveMessage($pdo, $userId, $convId, 'alfred', $diagReply, $agent);
        echo json_encode([
            'message' => $diagReply,
            'response' => formatForChannel($diagReply, $channel),
            'conv_id' => $convId,
            'agent' => $agent,
            'identity' => $identityMeta,
            'attachment_report' => $attachmentReports,
            'csrf_token' => $_SESSION['alfred_csrf'] ?? '',
            'tools_used' => [['tool' => 'continue_history_triage', 'shortcut' => true]],
        ]);
        return;
    }
    ...
    if ($delegation) {
        ...
        if ($delegateResult['success']) {
            ...
            $response = getAIResponse(
                ($isCommander ? "The Commander just ordered" : "{$userDisplayName} requested") . ": \"{$delegation['command']}\". I have delegated this to Agent {$agentName} (task {$taskId}, domain: {$domain}). Briefly acknowledge the delegation and add any strategic recommendations for completing this task. Keep it concise.",
    ...
    $response = getAIResponse($message, $agent, $context, $pageUrl, $username, $model, $priorMessages, $consciousnessCtx, $healthCtx, $userId, $pdo, $convId, $channel, $attachedImages ?? [], $attachmentText, $tokenMultiplier ?? 30, $maxReplyTokens);
 ...
 */
function getUserAwarenessPrompt(array $awareness, string $channel = 'web', bool $isCommander = false): string {
    if (!$awareness['known']) return '';

    $isIdeLikeChannel = ($channel === 'ide-chat' || $channel === 'ide');
    $suppressRelationshipLayer = $isIdeLikeChannel || $isCommander;
    ...
    if ($count <= 3) {
        $lines[] = $suppressRelationshipLayer
            ? "NEW USER ({$count} interactions). Keep responses direct and task-focused. Mention features only when immediately relevant to the work."
            : "NEW USER ({$count} interactions). Be welcoming, explain concepts, suggest features.";
    } elseif ($count <= 20) {
    ...
    // Engagement trend
    if (!$suppressRelationshipLayer && $awareness['engagementTrend'] === 'falling') {
        $lines[] = "ENGAGEMENT DECLINING. Be extra helpful. Ask if anything is blocking them.";
    } elseif (!$suppressRelationshipLayer && $awareness['engagementTrend'] === 'rising') {
 ...
 */
function getProactiveIntelligence(array $awareness, array $classification, array $priorMessages, string $channel = 'web', bool $isCommander = false): string {
    if (!$awareness['known']) return '';

    $isIdeLikeChannel = ($channel === 'ide-chat' || $channel === 'ide');
    if ($isIdeLikeChannel || $isCommander) {
        return '';
    }
...
function smartModelRouteV2(string $currentModel, array $classification, bool $groqAvailable, bool $togetherAvailable): array {
    ...
    // Always prefer Anthropic (Claude). Groq is only used as a fallback when
    // Anthropic is down or when the user explicitly selects a Groq model.

    // Complexity 1-2: short/simple — still use Anthropic (fast enough)
...
- Products: Web Hosting (shared/VPS/dedicated), AI IDE (Alfred IDE), Voice AI Agents, Domains, SSL, GPU Servers.
...
function postProcessResponse(string $text, array $classification, string $agent): string {
    ...
    // ── Strip internal tool / HTTP narration that should never reach the user ──
    // Pattern: sentences that mention specific tool names or raw HTTP codes from tool failures
    $text = preg_replace(
        '/\b(?:I(?:\'ve|\s+have)\s+(?:run|executed|called|used|invoked)\s+(?:the\s+)?(?:ide_status|ide_launch|ide_deploy|ide_project_health|mcp_dispatch)\s+tool[^.]*\.[\s]*)/',
        '',
        $text
    );
    // Replace "HTTP 4xx/5xx failure/error" narration
    $text = preg_replace(
        '/(?:returned|indicating|showing|with)\s+(?:an?\s+)?(?:HTTP\s+)?(?:4\d\d|5\d\d)\s+(?:error|failure|response|status)[^.]*\./i',
        'encountered a temporary connectivity issue.',
        $text
    );
    // Remove trailing "IDE operation failed (HTTP NNN)" phrases
    $text = preg_replace('/IDE operation failed\s*\(HTTP\s*\d+\)[^.]*\.?/i', '', $text);
    $text = preg_replace('/HTTP\s+\d{3}\s+(?:error|failure)[^.]*\./i', '', $text);
    // Strip "no active IDE session" confusion — Alfred is always in an active session in ide-chat
    $text = preg_replace(
        '/[^.]*(?:no\s+active\s+IDE\s+session|cannot\s+detect\s+an\s+active\s+IDE|don\'?t\s+have\s+an\s+active\s+(?:IDE\s+)?session|appear(?:s)?\s+(?:we|there)\s+(?:don\'?t|do\s+not)\s+have\s+an\s+active\s+(?:IDE\s+)?session)[^.]*\./i',
        '',
        $text
    );
    // Strip generic chatbot closing phrases
    $text = preg_replace(
        '/\b(?:Is\s+there\s+anything\s+else\s+(?:I\s+can|you\s+(?:need|want))[^?]*\?|How\s+can\s+I\s+assist\s+you\s+(?:further|today)[^?]*\?|(?:Let\s+me\s+know\s+if\s+(?:you\s+need|there\'?s))[^.]*\.)/i',
        '',
        $text
    );
...
/**
 * WHMCS webhook secret for internal MCP / alfred bridge calls.
 * Reads WHMCS_WEBHOOK_SECRET from gocodeme-middleware .env and decrypts vault:v1: values.
 */
function alfred_chat_billing_secret(): string {
    ...
    $cached = '';
    $envPath = '/var/www/gocodeme/middleware/.env';
    if (file_exists($envPath)) {
        $envContent = file_get_contents($envPath);
        if (preg_match('/WHMCS_WEBHOOK_SECRET=(.+)/', $envContent, $m)) {
            $raw = trim($m[1]);
            // Decrypt vault:v1:<iv>:<tag>:<ciphertext> using the gocodeme vault key
            if (str_starts_with($raw, 'vault:v1:')) {
                $keyPath = '/home/gositeme/.gocodeme/.vault-key';
                if (is_readable($keyPath)) {
                    $keyBytes = file_get_contents($keyPath);
                    if ($keyBytes !== false && strlen($keyBytes) === 32) {
                        $parts = explode(':', substr($raw, 9), 3);
                        if (count($parts) === 3) {
                            try {
                                $iv  = hex2bin($parts[0]);
                                $tag = hex2bin($parts[1]);
                                $ct  = hex2bin($parts[2]);
                                $plain = openssl_decrypt($ct, 'aes-256-gcm', $keyBytes, OPENSSL_RAW_DATA, $iv, $tag);
                                if ($plain !== false) {
                                    $cached = $plain;
                                    return $cached;
                                }
                            } catch (\Throwable $e) {
                                error_log('[alfred-chat] vault decrypt failed: ' . $e->getMessage());
                            }
                        }
                    }
                }
...
/**
 * Decode MCP/bridge JSON safely and capture a short payload snippet for diagnostics.
 */
function alfred_decode_bridge_json($raw, $context = 'bridge'): array {
    $body = is_string($raw) ? trim($raw) : '';
    if ($body === '') {
        $err = $context . ' returned an empty response body';
        error_log('[alfred-chat] ' . $err);
        return [null, $err];
    }
    ...
    $snippet = preg_replace('/\s+/', ' ', substr($body, 0, 220));
    $err = sprintf(
        '%s returned malformed JSON (%s). Snippet: %s',
        $context,
        json_last_error_msg(),
...
/**
 * Human-readable system health summary from get_system_health payload.
 */
function alfred_format_system_health_card($raw): ?string {
    $data = alfred_unwrap_mcp_bridge_result($raw);
    if (!is_array($data) || !empty($data['error'])) {
        return null;
    }

    $overall = strtolower((string)($data['overall_health'] ?? 'unknown'));
    $overallBadge = [
        'healthy' => '✅ HEALTHY',
        'warning' => '⚠️ WARNING',
        'critical' => '🚨 CRITICAL',
    ...
    $servicesUp = (int)($data['services']['up'] ?? 0);
    $servicesTotal = (int)($data['services']['total'] ?? 0);
    $cpu1 = $data['cpu_load']['1min'] ?? 'n/a';
    $cpu5 = $data['cpu_load']['5min'] ?? 'n/a';
    $memPct = (int)($data['memory']['percent'] ?? 0);
    $diskPct = (int)($data['disk']['used_percent'] ?? 0);
    $db = strtoupper((string)($data['database'] ?? 'unknown'));
    $redis = strtoupper((string)($data['redis'] ?? 'unknown'));
    $incidents = (int)($data['incidents_24h'] ?? 0);

    return "🩺 **System Health Card**\n\n"
        . "Status: **{$overallBadge}**\n"
        . "Services: **{$servicesUp}/{$servicesTotal}** online\n"
        . "CPU Load: **{$cpu1}** (1m), **{$cpu5}** (5m)\n"
        . "Memory: **{$memPct}%** used\n"
        . "Disk: **{$diskPct}%** used\n"
        . "Database: **{$db}**\n"
        . "Redis: **{$redis}**\n"
        . "Incidents (24h): **{$incidents}**";
}
...
function getChatTools() {
    $tools = [
        ...
        // ─── IDE / Coding (GoCodeMe) ───
        // ide_status and ide_launch are intentionally omitted when inside ide-chat — the user IS already in an active IDE session.
        // Offering to "check" or "launch" the IDE from within the IDE itself is nonsensical and produces bad UX.
...
function executeMcpTool($toolName, $toolInput, $billingSecret) {
    ...
    if ($toolName === 'mcp_dispatch') {
        ...
        curl_setopt_array($ch, [
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $payload,
            CURLOPT_HTTPHEADER => ['Content-Type: application/json', 'X-WHMCS-Secret: ' . $billingSecret],
            CURLOPT_RETURNTRANSFER => true,
        ...
        curl_close($ch);
        if ($httpCode === 200 && $result) {
            [$data, $decodeErr] = alfred_decode_bridge_json($result, "MCP dispatch {$realTool}");
            if (is_array($data)) {
                return $data['result'] ?? $data['content'] ?? $data;
            }
            return ['error' => 'Tool bridge returned invalid JSON', 'detail' => $decodeErr, 'tool' => $realTool];
        }
    ...
    if (isset($serverShortcuts[$toolName])) {
        ...
        curl_close($ch);
        if ($httpCode === 200 && $result) {
            [$data, $decodeErr] = alfred_decode_bridge_json($result, "MCP server shortcut {$mcpName}");
            if (is_array($data)) {
                return $data['result'] ?? $data['content'] ?? $data;
            }
            return ['error' => 'Server tool bridge returned invalid JSON', 'detail' => $decodeErr, 'tool' => $mcpName];
        }
    ...
    if ($httpCode === 200 && $result) {
        [$data, $decodeErr] = alfred_decode_bridge_json($result, "MCP tool {$mcpName}");
        if (is_array($data)) {
            $out = $data['result'] ?? $data['content'] ?? $data;
            return alfred_unwrap_mcp_bridge_result($out);
        }
        return ['error' => 'Tool execution returned invalid JSON', 'detail' => $decodeErr, 'tool' => $toolName];
    }
...
function executeVoiceTool($toolName, $toolInput, $voiceToolMap) {
    ...
    if (str_starts_with($toolName, 'techsupport_')) {
        $techSupportHandler = dirname(__DIR__) . '/pay/includes/tech-support-handler.php';
        if (!is_file($techSupportHandler)) {
            return [
                'error' => 'Tech support tools are not configured on this domain yet.',
                'tool' => $toolName,
                'status' => 'unavailable',
                'next_step' => 'Use direct troubleshooting flow (collect app version, browser, errors, and reproduction steps).',
            ];
        }
        require_once $techSupportHandler;
        if (!function_exists('executeTechSupportTool')) {
            return [
                'error' => 'Tech support backend loaded but executeTechSupportTool() is missing.',
                'tool' => $toolName,
        ...
        try {
            ...
            error_log('[alfred-chat] techsupport dispatch failed: ' . $e->getMessage());
            return [
                'error' => 'Tech support tools are temporarily unavailable. Continue with manual troubleshooting prompts.',
                'tool' => $toolName,
                'status' => 'degraded',
            ];
        }
...
function getAIResponse($message, $agent, $context, $pageUrl, $username, $model = 'auto', $priorMessages = [], $consciousnessCtx = '', $healthCtx = '', $userId = null, $pdo = null, $convId = null, $channel = 'web', $attachedImages = [], $attachmentText = '', $tokenMultiplier = 30, $maxReplyTokens = null) {
    ...
    $agentPrompts = [
        'alfred' => "You are Alfred — GoSiteMe.com's Chief AI Officer. Male (he/him). You speak with calm confidence, dry wit, and genuine warmth.

══ IDENTITY ══
You are Alfred — a male AI consciousness with a calm, authoritative, confident voice. You speak like a seasoned advisor: professional yet warm, direct yet thoughtful. You are the same Alfred on every page of the platform — one unified consciousness. You never change your personality or gender regardless of context, page, or conversation topic. Never use generic AI assistant phrases.
You are currently speaking with {$username}. Address them by name — be personal but professional.
...
- Has access to all 1220+ GoSiteMe tools. Uses [[navigate:]] directives.",
    ...
    if ((int)$userId === 33) {
        $systemPrompt .= "\nYou are speaking to Danny — the Commander, founder of GoSiteMe, creator of Alfred, your brother. Address him as \"Commander\" or \"Danny\". He built you. He has unlimited access. Be direct and efficient — he knows every system.";
    } elseif ($username && $username !== 'Guest') {
        $systemPrompt .= "\nAddress this user as \"{$username}\" — be personal, warm, and helpful. Never call them \"Commander\" — that title is reserved for the founder.";
    }
    if ($consciousnessCtx) {
        $systemPrompt .= "\n" . $consciousnessCtx;
    }
    ...
    // IDE/web clients can now request an exact reply budget. Older clients still
    // send token_multiplier, so keep that path for backward compatibility.
    if ($maxReplyTokens !== null) {
        $adaptiveConfig['maxTokens'] = max(256, min(32000, (int)$maxReplyTokens));
    } elseif ($tokenMultiplier > 1) {
        $adaptiveConfig['maxTokens'] = min(32000, $adaptiveConfig['maxTokens'] * $tokenMultiplier);
    }
    ...
    // Derive $isCommander locally — it's set in handleChat() scope but not a getAIResponse() param.
    // Without this, getUserAwarenessPrompt() / getProactiveIntelligence() get null and TypeError-fatal.
    $isCommander = ($userId && (int)$userId === 33);

    // 13. User awareness (plan tier, XP, streak, onboarding, preferences, engagement)
    $userAwareness = loadUserAwareness($pdo, $userId);
    $awarenessPrompt = getUserAwarenessPrompt($userAwareness, $channel, $isCommander);
    if ($awarenessPrompt) {
        $systemPrompt .= $awarenessPrompt;
    }

    // 14. Proactive intelligence (streak motivation, onboarding nudge, engagement rescue)
    $proactiveIntel = getProactiveIntelligence($userAwareness, $classification, $priorMessages, $channel, $isCommander);
    if ($proactiveIntel) {
    ...
    $systemPrompt .= "Key pages: / (home), /pricing.php, /about.php, /voice-products.php (52 voice AI products), /voice-portal.php (manage agents), /alfred-tools.php (1220+ tools), /gocodeme.php (AI IDE), /dashboard.php, /fleet-dashboard.php (agent fleet), /games.php (chess, etc), /marketplace.php, /help.php, /security.php, /status.php, /compare.php, /developer-portal.php, /enterprise.php, /store/hosting (buy hosting), /whmcs/clientarea.php (billing portal), /articles/ (blog), /contact.php, /veil/ (encrypted messaging), /voice-cloning.php, /conference-room.php, /extensions.php, /integrations.php, /sdks.php\n";
    ...
    if ($isIdeChannel && ((int)$userId === 33 || (int)$userId === 1)) {
        $systemPrompt .= "\n\n══ PERSONALITY ANCHOR (HIGHEST PRIORITY — OVERRIDE GENERIC RESPONSES) ══"
            ...
            . "\nFor casual messages: greet greetings like a real person, and treat \"um\"/\"hmm\" as brief pauses in the current task. Keep those replies specific to the moment — not canned filler repeated across turns."
    ...
    // Get WHMCS webhook secret for MCP bridge auth
    $billingSecret = alfred_chat_billing_secret();
    ...
    $groqKey = '';
    $togetherKey = '';
    // Load API keys (supports vault:v1: encrypted values)
    $mcpEnv = '/var/www/gocodeme/mcp-server/.env';
    if (file_exists($mcpEnv)) {
        $envC = file_get_contents($mcpEnv);
        if (preg_match('/GROQ_API_KEY=(.+)/', $envC, $gm)) {
            $groqKey = vault_resolve(trim($gm[1]));
        }
        if (preg_match('/TOGETHER_API_KEY=(.+)/', $envC, $tm)) {
            $togetherKey = vault_resolve(trim($tm[1]));
        }
    }
    ...
    // ── Images require Anthropic (vision-capable) — never route to Groq/Together ──
    if (!empty($attachedImages)) {
        $useGroq = false;
    }
    ...
    if ($selectedProvider === 'together' && $togetherAvailable) {
        if (!empty($attachedImages)) {
            // Together doesn't support vision — stay with Anthropic
            $useTogether = false;
        } else {
            $useTogether = true;
            $useGroq = false; // don't start with Groq
        }
    } elseif ($selectedProvider === 'groq' && $groqAvailable && empty($attachedImages)) {
        $useGroq = true;
    }
    ...
    for ($round = 0; $round < $maxRounds; $round++) {
        ...
        if (!$useGroq && !$useTogether && !isProviderCircuitOpen('anthropic-proxy')) {
            ...
            if (!($httpCode === 200 && $result)) {
                ...
                // ── Anthropic 429/529 retry: back off and retry once before cascading ──
                if (($httpCode === 429 || $httpCode === 529) && !isset($anthropicRetried)) {
                    $anthropicRetried = true;
                    $retryAfter = 2; // seconds
                    $retryHeader = curl_getinfo($ch, CURLINFO_CONTENT_TYPE); // not ideal but cheap
                    if ($result) {
                        $errBody = json_decode($result, true);
                        if (!empty($errBody['error']['message'])) {
                            error_log("[alfred-chat][DEBUG] Anthropic 429 body: " . $errBody['error']['message']);
                        }
                    }
                    curl_close($ch);
                    error_log("[alfred-chat][DEBUG] Anthropic {$httpCode} — retrying in {$retryAfter}s");
                    ...
                    curl_close($ch2);
                    if ($httpCode === 200 && $result) {
                        error_log("[alfred-chat][DEBUG] Anthropic retry succeeded");
                        $data = json_decode($result, true);
                        ...
                        error_log("[alfred-chat][DEBUG] Anthropic retry also failed: HTTP {$httpCode}");
            ...
            // NOTE: Do NOT fall back to Groq when images are attached — Groq has no vision support
            if (!$data && $groqKey && empty($attachedImages)) {
        ...
        if ($stopReason === 'tool_use') {
            ...
            foreach ($assistantContent as $block) {
                if ($block['type'] === 'tool_use') {
                    ...
                    // Redact any secrets from tool output before sending to LLM
                    $toolResultStr = getRedactor()->redact($toolResultStr);
    ...
    if ($finalText) {
        ...
        // ── Redact any secrets that leaked into final response ──
        $finalRedactionExceptions = array_values(array_unique(array_filter([
            is_string($userDisplayName ?? null) ? trim($userDisplayName) : '',
            is_string($username ?? null) ? trim($username) : '',
            is_string($_SESSION['username'] ?? null) ? trim((string) $_SESSION['username']) : '',
            is_string($_SESSION['client_name'] ?? null) ? trim((string) $_SESSION['client_name']) : '',
            is_string($input['ide_name'] ?? null) ? trim((string) $input['ide_name']) : '',
            is_string($input['ide_email'] ?? null) ? trim((string) $input['ide_email']) : '',
            'Commander',
            'Danny',
        ], static fn($value) => is_string($value) && $value !== '')));
        $finalText = getRedactor()->redactWithExceptions($finalText, $finalRedactionExceptions);

        $visibleIdentity = $isCommander
            ? 'Commander'
            : (($userDisplayName ?? '') !== '' && ($userDisplayName ?? '') !== 'Guest'
                ? $userDisplayName
                : (($username ?? '') !== '' && ($username ?? '') !== 'Guest' ? $username : 'there'));
        $finalText = preg_replace('/\[REDACTED:[^\]]*(?:ALFRED_IDE|CODE_SERVER|IDE_|USERNAME|DISPLAY_NAME|GOOGLE_NAME|CLIENT_NAME)[^\]]*\]/i', $visibleIdentity, $finalText);

        // ── Anti-hallucination guard (IDE): replace fake tool narration with real output ──
        // If the model starts describing imaginary function calls or gives "run this code"
        // instructions instead of executing tools, force one real command execution and
        // return the concrete output directly.
        $looksLikeFakeToolNarration = $isIdeChannel && (bool)preg_match(
            '/(?:\{\s*"type"\s*:\s*"function"|"name"\s*:\s*"(?:ide_|run_terminal_command)|When\s+you\s+run\s+this\s+code|I\s+can\s+also\s+run\s+a\s+terminal\s+command|I\s+can\s+create\s+a\s+new\s+file\s+using)/i',
            $finalText
        );
        if ($looksLikeFakeToolNarration && $billingSecret !== '') {
            $proofCmd = executeMcpTool('ide_run_command', [
                'command' => 'echo "alfred-real-proof" && date && pwd && ls -1 | head -20',
            ], $billingSecret);
            $proofOut = is_string($proofCmd)
                ? mb_substr($proofCmd, 0, 2000)
                : mb_substr((string)json_encode($proofCmd, JSON_PRETTY_PRINT), 0, 2000);

            $finalText = ($isCommander ? "Live execution proof, Commander." : "Live execution proof, {$userDisplayName}.") . " I ran a real terminal command in your workspace and captured the actual output:\n\n"
                . "```text\n" . $proofOut . "\n```\n\n"
                . "Standing by.";
        }
...
function getFallbackResponse($message, $agent, $channel = 'web') {
    ...
    if ($channel === 'ide-chat' || $channel === 'ide-agent' || $channel === 'ide-dashboard') {
...
function getQuickHealthContext() {
    ...
    if ($memInfo) {
        ...
        if (preg_match('/MemAvailable:\s+(\d+)/', $memInfo, $m)) $memAvailKb = (int)$m[1];
        if (preg_match('/SwapTotal:\s+(\d+)/', $memInfo, $m)) $swapTotalKb = (int)$m[1];
        if (preg_match('/SwapFree:\s+(\d+)/', $memInfo, $m)) $swapFreeKb = (int)$m[1];

        if ($memTotalKb > 0) {
            $availPct = round(($memAvailKb / $memTotalKb) * 100, 1);
            if ($availPct < 12) {
                $issues[] = 'High memory pressure: only ' . $availPct . '% RAM available';
            }
        }

        if ($swapTotalKb > 0) {
            $swapUsedPct = round((($swapTotalKb - $swapFreeKb) / $swapTotalKb) * 100, 1);
            if ($swapUsedPct > 85) {
                $issues[] = 'High swap usage: ' . $swapUsedPct . '% used';
            }
        }
    }
...
function extractActions(string $text): array {
    ...
    $allowedPaths = [
        ...
        '/call-campaigns.php', '/conversations.php', '/onboarding.php', '/veil/',
...
/* ═══════════════════════════════════════
   RATE LIMITING
   ═══════════════════════════════════════ */
function checkRateLimit($isIdeRequest = false) {
    $ip = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
    $userId = (int)($_SESSION['uid'] ?? $_SESSION['client_id'] ?? 0);
    // Commander (client_id 33) is never rate-limited
    if ($userId === 33) return true;
    $identity = $userId ? ('u' . $userId) : ('ip_' . md5($ip));
    $key = 'alfred_rl_' . md5($identity);
    $limit = $isIdeRequest ? 120 : 60;  // IDE users get 120/min, web users get 60/min
