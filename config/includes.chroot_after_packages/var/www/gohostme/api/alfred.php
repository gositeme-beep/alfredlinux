...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    echo json_encode(['error' => 'POST only'], gositeme_json_public_encode_flags());
    exit;
}
...
if (!is_array($body) || empty($body['q'])) {
    ...
    echo json_encode(['error' => 'missing q'], gositeme_json_public_encode_flags());
    exit;
}
...
if (strlen($q) > 4000) {
    http_response_code(400);
    echo json_encode(['error' => 'question too long (max 4000 chars)'], gositeme_json_public_encode_flags());
    exit;
}
...
if ($context !== 'mercy' && $urgency !== 'high') {
    ...
    if (count($hits) >= 10) {
        ...
        echo json_encode(['error' => 'rate limit — 10 questions per 5 min'], gositeme_json_public_encode_flags());
        exit;
    }
    $hits[] = $now;
    @file_put_contents($rate_key, json_encode(array_values($hits), gositeme_json_public_encode_flags()));
}
...
@file_put_contents('/var/log/alfred-api.jsonl', json_encode($log, gositeme_json_public_encode_flags()) . "\n", FILE_APPEND);
