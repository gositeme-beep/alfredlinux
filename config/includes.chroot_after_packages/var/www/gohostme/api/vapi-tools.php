...
function mcpActionVoiceDispatch(string $realTool, $realArgs, string $vapiCallId): array {
    ...
    if (in_array($realTool, $fileTools, true)) {
        return ['error' => 'That tool is only available in the Alfred IDE, not over the phone. I can create a support ticket instead.'];
    }
...
foreach ($toolCalls as $toolCall) {
    ...
    try {
        switch ($toolName) {
            ...
            // ── Alfred IDE File & Dev Tools (voice parity with chat) ──
...
function toolAuthenticate($args, $vapiCallId = '', $vapiCallerNum = '') {
    ...
    $pin          = trim($args['pin'] ?? '');
    $otpPin       = trim($args['otp_pin'] ?? $args['phone_pin'] ?? '');
    ...
    // Auto-detect: 6-digit PIN in the pin field → treat as OTP
    if (empty($otpPin) && strlen($pin) === 6 && preg_match('/^\d{6}$/', $pin)) {
        $otpPin = $pin;
        $pin = '';
    }

    // ═══ OTP Phone PIN path — no email needed, maximum OPSEC ═══
    if (!empty($otpPin)) {
        if (!preg_match('/^\d{6}$/', $otpPin)) {
            return ['authenticated' => false, 'step' => 'need_otp',
                'message' => 'That does not look like a valid phone PIN. It should be 6 digits. You can generate one from your GoSiteMe dashboard.'];
        }
        $db = getDB();
        if (!$db) return ['authenticated' => false,
            'message' => 'I am having a technical issue. I will have our team call you back within 24 hours.'];

        // Find active (unexpired, unused, not revoked) PINs and verify
        $stmt = $db->prepare("
            SELECT p.id, p.client_id, p.pin_hash,
                   c.id as cid, c.firstname, c.lastname, c.email, c.phone, c.city, c.date_created, c.support_pin,
            ...
            FROM client_phone_pins p
            JOIN clients c ON c.id = p.client_id
            WHERE p.used_at IS NULL AND p.revoked = 0 AND p.expires_at > NOW() AND c.status = 'Active'
        ...
        foreach ($candidates as $row) {
            if (password_verify($otpPin, $row['pin_hash'])) {
                $matched = $row;
                break;
            }
        }
        ...
        if (!$matched) {
            return ['authenticated' => false, 'step' => 'need_otp',
                'message' => 'That PIN is invalid or has expired. Please generate a fresh one from your GoSiteMe dashboard and try again.'];
        }

        // Consume the PIN — mark used with call_id
        $stmtUse = $db->prepare("UPDATE client_phone_pins SET used_at = NOW(), used_call_id = :cid WHERE id = :id");
        $stmtUse->execute([':cid' => $vapiCallId, ':id' => $matched['id']]);

        // Map to standard client structure for authSuccess
        $matched['id'] = $matched['cid'];
        authLog($db, $matched['email'], $callerPhone, true, 'otp_pin');
        $r = authSuccess($matched, 'otp_pin', $vapiCallId, $vapiCallerNum);
        $r['message'] = 'Identity verified with your one-time PIN! Welcome back, ' . $matched['firstname'] . '! Your PIN has been consumed and cannot be reused. How can I help you today?';
        return $r;
    }

    if (empty($email)) {
        return ['authenticated' => false, 'step' => 'need_identifier',
            'message' => 'I can verify you two ways. If you have a one-time phone PIN from your GoSiteMe dashboard, just tell me the 6-digit code — no email needed. Otherwise, what is your email address?'];
    }
    ...
    $r['message'] .= ' Tip: for even stronger security, you can generate a one-time phone PIN from your GoSiteMe dashboard — that way no personal info is ever spoken on the phone!';
    return $r;
}
