...
// === LOCKDOWN_API_INJECT ===
if (file_exists(__DIR__."/../includes/lockdown.inc.php")) {
    require_once __DIR__."/../includes/lockdown.inc.php";
    if (defined("LOCKDOWN_ENABLED") && LOCKDOWN_ENABLED) {
        $__act = strtolower((string)($_REQUEST["action"] ?? ""));
        if (in_array($__act, ["login","oauth_login","google_login","register","signup","create_account","reset_password","forgot_password"], true)) {
            // Allow login flow ONLY for the allow-list email(s).
            $__email = strtolower(trim((string)($_REQUEST["email"] ?? "")));
            $__allowEmails = ["gositeme@gmail.com","danny@gositeme.com","officialjordanmaxwell@gmail.com","brnavecchio@proton.me"];
            $__allow = in_array($__email, $__allowEmails, true);
            // For OAuth callbacks the email may not be in REQUEST yet — let through then re-check post-auth in auth-gate.
            if ($__act === "login" && !$__allow) {
                lockdown_block_api("Sign-in temporarily restricted.");
            }
            if (in_array($__act, ["register","signup","create_account","reset_password","forgot_password"], true)) {
                lockdown_block_api("Account changes temporarily disabled.");
            }
        }
    }
}
...
require_once __DIR__ . '/../includes/auth-debug-trace.inc.php';
require_once __DIR__ . '/../includes/path-guard.inc.php';

// === OAUTH_STATE_FIX ===
if (!function_exists('oauth_state_dir')) {
    function oauth_state_dir(): string
    {
        $custom = gositeme_safe_absolute_dir((string) (getenv('GOSITEME_OAUTH_STATE_DIR') ?: ''));
        $d = $custom !== null ? $custom : '/home/gositeme/law/var/oauth-state';
        if (!is_dir($d)) {
            @mkdir($d, 0700, true);
        }
        @chmod($d, 0700);
    ...
    function oauth_state_save(string $state, array $payload): void
    {
        $clean = substr(preg_replace('/[^a-f0-9]/', '', $state), 0, 128);
        if (strlen($clean) < 16) {
            return;
        }
        // Garbage collect old (>15 min)
        foreach (glob(oauth_state_dir() . '/*.json') as $g) {
            if (is_file($g) && filemtime($g) < time() - 900) {
                @unlink($g);
            }
        }
        $fp = oauth_state_dir() . '/' . $clean . '.json';
        file_put_contents($fp, json_encode([
            'payload' => $payload,
            'ts'      => time(),
            'ip'      => $_SERVER['REMOTE_ADDR'] ?? null,
        ], gositeme_json_public_encode_flags()), LOCK_EX);
        @chmod($fp, 0600);
    }
    function oauth_state_consume(string $state): ?array
    {
        $clean = preg_replace('/[^a-f0-9]/', '', $state);
        ...
        if (filemtime($fp) < time() - 900) { @unlink($fp); return null; }
        $j = json_decode(@file_get_contents($fp), true);
        @unlink($fp);
        return is_array($j) ? ($j['payload'] ?? []) : null;
    }
}
// === /OAUTH_STATE_FIX ===
 ...
 */
function logout() {
    // Browser GET → full redirect through /logout.php so cookies clear properly
    $isXhr  = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest';
    $accept = $_SERVER['HTTP_ACCEPT'] ?? '';
    $wantsJson = $isXhr || stripos($accept, 'application/json') !== false || ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST';
    if (!$wantsJson) {
...
function verify2FA() {
    ...
    $oauthRedirect = $_SESSION['oauth_redirect'] ?? '/dashboard.php';
    unset(
    ...
    if ($isOAuth) {
        ...
        $redirPath = parse_url($oauthRedirect, PHP_URL_PATH) ?: '/';
        gositeme_auth_debug_trace_log(
            date('H:i:s') . ' [oauth-callback-end] sid_prefix=' . substr(session_id(), 0, 8)
            . ' cid=' . (int) ($_SESSION['client_id'] ?? 0) . ' redirect_path=' . $redirPath
        );
...
function googleLogin() {
    ...
    $oauthRedir = $_GET['redirect'] ?? '/dashboard.php';
    if (!preg_match('#^/[^/]#', $oauthRedir)) { $oauthRedir = '/dashboard.php'; }
    $_SESSION['oauth_redirect'] = $oauthRedir;
    // BILLING_ADMIN_OAUTH_FIX: capture flow (e.g. billing_admin) from start URL
    $oauthFlow = $_GET['flow'] ?? 'oauth';
    if (!in_array($oauthFlow, ['oauth','billing_admin','client'], true)) { $oauthFlow = 'oauth'; }
    $_SESSION['oauth_flow'] = $oauthFlow;
    $state = bin2hex(random_bytes(16));
    oauth_state_save($state, ['flow' => $oauthFlow, 'redirect' => $_SESSION['oauth_redirect'] ?? '/dashboard.php']);
...
function googleCallback() {
    ...
    $state = $_GET['state'] ?? '';
    $__sp = oauth_state_consume($state);
    if ($__sp === null) {
        header('Location: /login?error=' . urlencode('OAuth session expired. Please try signing in again.'));
        exit;
    }
    if (!empty($__sp['redirect'])) { $_SESSION['oauth_redirect'] = $__sp['redirect']; }
    if (!empty($__sp['flow'])) { $_SESSION['oauth_flow'] = $__sp['flow']; }
...
function githubLogin() {
    ...
    $ghRedir = $_GET['redirect'] ?? '/dashboard.php';
    if (!preg_match('#^/[^/]#', $ghRedir)) { $ghRedir = '/dashboard.php'; }
    $_SESSION['oauth_redirect'] = $ghRedir;
    // BILLING_ADMIN_OAUTH_FIX: capture flow
    $ghFlow = $_GET['flow'] ?? 'oauth';
    if (!in_array($ghFlow, ['oauth','billing_admin','client'], true)) { $ghFlow = 'oauth'; }
    $_SESSION['oauth_flow'] = $ghFlow;
    $state = bin2hex(random_bytes(16));
    oauth_state_save($state, ['flow' => $ghFlow, 'redirect' => $_SESSION['oauth_redirect'] ?? '/dashboard.php']);
...
function completeOAuthLogin(string $email, string $firstName, string $lastName, string $provider, string $providerId) {
    ...
    // BILLING_ADMIN_OAUTH_FIX: if this OAuth originated from /pay/admin/login.php, also create the admin session
    if (($_SESSION['oauth_flow'] ?? '') === 'billing_admin') {
        try {
            ...
            require_once dirname(__DIR__).'/pay/includes/admin-auth-session.php';
            $bdb = function_exists('billingDB') ? billingDB() : null;
            if ($bdb) {
                $astmt = $bdb->prepare('SELECT * FROM admin_users WHERE LOWER(email)=LOWER(?) AND is_active=1 LIMIT 1');
                $astmt->execute([$email]);
                $adminRow = $astmt->fetch(PDO::FETCH_ASSOC);
                if ($adminRow) {
                    if (!empty($adminRow['two_factor_enabled']) && !empty($adminRow['two_factor_secret'])) {
                        $_SESSION['admin_2fa_pending']  = (int)$adminRow['id'];
                        $_SESSION['admin_2fa_attempts'] = 0;
                        unset($_SESSION['oauth_flow']);
                        header('Location: /pay/admin/login.php?step=2fa');
                        exit;
                    }
                    admin_apply_login_session($bdb, $adminRow, 'google');
                    unset($_SESSION['oauth_flow']);
                    $adminTarget = (strpos($oauthRedirect, '/pay/admin') === 0) ? $oauthRedirect : '/pay/admin/';
                    header('Location: ' . $adminTarget);
                    exit;
                }
            }
        } catch (\Throwable $e) { error_log('billing_admin oauth elevate: '.$e->getMessage()); }
        unset($_SESSION['oauth_flow']);
        header('Location: /pay/admin/login.php?error=' . urlencode('No admin account is linked to this Google email.'));
        exit;
    }
    unset($_SESSION['oauth_flow']);
