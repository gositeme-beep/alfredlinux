...
function findAgentById(array $agents, string $agentId): ?array {
...
function ensureSessionCsrfToken(): string {
...
function getWagerMinMoves(string $gameType, string $result): int {
    if ($result === 'lost') {
        return 0;
    }
    ...
    switch ($gameType) {
        case 'chess':
            return $result === 'draw' ? 4 : 6;
        case 'checkers':
            return $result === 'draw' ? 3 : 4;
        case 'pool':
            return $result === 'draw' ? 2 : 3;
        case 'backgammon':
            return 2;
        default:
            return $result === 'draw' ? 2 : 3;
...
function getWagerMinDurationSeconds(string $gameType, string $result): int {
    if ($result === 'lost') {
        return 0;
    }
...
$csrfToken = ensureSessionCsrfToken();
$sessionId = session_id();

$db = getDB();

// Forward wager actions to consolidated universal-betting API
$wagerActions = ['place-wager','settle-wager','active-wager','wager-history','cancel-wager','create-wager','get-active','get-wagers'];
if (in_array($action, $wagerActions)) {
...
switch ($action) {
    ...
    case 'place-wager':
        ...
        $opponentAgent = preg_replace('/[^a-z0-9\-]/', '', strtolower($input['opponent_agent'] ?? ''));
        ...
        if (!findAgentById($AGENTS, $opponentAgent)) {
    ...
    case 'settle-wager':
        ...
        $reportedGameType = preg_replace('/[^a-z0-9\-]/', '', strtolower($input['game_type'] ?? ''));
        $reportedOpponentAgent = preg_replace('/[^a-z0-9\-]/', '', strtolower($input['opponent_agent'] ?? ''));
        $moves = max(0, (int)($input['moves'] ?? 0));
        ...
        try {
            ...
            $gameType = (string)$wager['game_type'];
            $opponentAgent = (string)$wager['opponent_agent'];

            if (($reportedGameType && $reportedGameType !== $gameType) || ($reportedOpponentAgent && $reportedOpponentAgent !== $opponentAgent)) {
                error_log("Anti-cheat: wager settlement mismatch for wager $wagerId (reported {$reportedGameType}/{$reportedOpponentAgent}, stored {$gameType}/{$opponentAgent})");
                echo json_encode(['success' => false, 'error' => 'Wager details do not match']);
                break;
            }

            $minDuration = getWagerMinDurationSeconds($gameType, $result);
            $createdAt = strtotime((string)($wager['created_at'] ?? ''));
            if ($minDuration > 0 && $createdAt && (time() - $createdAt) < $minDuration) {
                error_log("Anti-cheat: wager $wagerId settled too quickly for {$gameType}");
                echo json_encode(['success' => false, 'error' => 'Game ended too quickly to settle']);
                break;
            }

            $minMoves = getWagerMinMoves($gameType, $result);
            if ($moves < $minMoves) {
                error_log("Anti-cheat: suspicious {$result} settlement for {$gameType} with {$moves} moves, wager {$wagerId}");
                echo json_encode(['success' => false, 'error' => 'Invalid game state']);
                break;
            }

            if ($gameType === 'pool') {
                if ($result === 'won' && !$gameEvidence) {
                    error_log("Anti-cheat: pool win claimed without evidence, wager {$wagerId}");
                    echo json_encode(['success' => false, 'error' => 'Pool wins require game evidence']);
                    break;
                }

                if ($gameEvidence) {
                    $pocketed = array_map('intval', is_array($gameEvidence['pocketed'] ?? null) ? $gameEvidence['pocketed'] : []);
                    $shots = max($moves, (int)($gameEvidence['shots'] ?? 0));
                    $legitimate = !empty($gameEvidence['legitimate']);

                    if ($result === 'won' && !in_array(8, $pocketed, true)) {
                        error_log("Anti-cheat: pool win claimed without pocketing 8-ball, wager {$wagerId}");
                        echo json_encode(['success' => false, 'error' => 'Invalid pool finish']);
                        break;
                    }

                    if ($result === 'won' && !$legitimate) {
                        error_log("Anti-cheat: illegitimate pool win claimed, wager {$wagerId}");
                        echo json_encode(['success' => false, 'error' => 'Invalid pool finish']);
                        break;
                    }

                    if ($shots < $minMoves) {
                        error_log("Anti-cheat: pool settlement below shot threshold, wager {$wagerId}");
                        echo json_encode(['success' => false, 'error' => 'Invalid game state']);
                        break;
                    }
                }
    ...
    default:
        echo json_encode(['success' => false, 'error' => 'Unknown action. Available: csrf, agent-profiles, agent-profile, place-wager, settle-wager, game-stats, leaderboard, ecosystem-status, heartbeat, live-stats, agent-presence, agent-deploy, agent-activity, agent-world-stats, spawn-agent, dispute-game, game-history']);
        break;
}
