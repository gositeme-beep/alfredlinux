...
switch ($action) {
    ...
    case 'proxy':
        ...
        // Build header for internal secret (never pass via query string)
        $internalHeader = '';
        $secretKey = $secretKeyMap[$target] ?? '';
        if ($secretKey && $secretKey !== 'quick' && $secretKey !== 'action') {
            $internalHeader = "X-Internal-Secret: " . INTERNAL_SECRET . "\r\n";
        }
        ...
        if ($result === false) {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_TIMEOUT => 10,
                CURLOPT_HTTPHEADER => ['Cookie: ' . ($_SERVER['HTTP_COOKIE'] ?? ''), 'X-Internal-Secret: ' . INTERNAL_SECRET],
