...
function getOVHCredentials() {
    try {
        ...
        // Look for OVH API credentials by credential_id (plaintext column)
        $stmt = $pdo->query("SELECT username, password, notes, service_url FROM commander_credentials WHERE credential_id = 'ovh-api-keys' LIMIT 1");
...
switch ($action) {
    ...
    // ═══════════════════════════════════════════════════════
    //  Backup FTP Storage — /dedicated/server/{name}/features/backupFTP
    // ═══════════════════════════════════════════════════════

    // ─── Get backup FTP status ───
    case 'backup_ftp_status':
        $name = $_GET['name'] ?? '';
        if (!$name) { echo json_encode(['error' => 'Server name required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $result = ovhRequest('GET', "/dedicated/server/$name/features/backupFTP");
        echo json_encode($result);
        break;

    // ─── Activate backup FTP ───
    case 'backup_ftp_activate':
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { echo json_encode(['error' => 'POST required']); break; }
        $input = json_decode(file_get_contents('php://input'), true);
        $name = $input['name'] ?? '';
        if (!$name) { echo json_encode(['error' => 'Server name required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $result = ovhRequest('POST', "/dedicated/server/$name/features/backupFTP");
        echo json_encode(['success' => true, 'task' => $result, 'message' => 'Backup FTP activation initiated']);
        break;

    // ─── List authorized IPs for backup FTP ───
    case 'backup_ftp_access':
        $name = $_GET['name'] ?? '';
        if (!$name) { echo json_encode(['error' => 'Server name required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $result = ovhRequest('GET', "/dedicated/server/$name/features/backupFTP/access");
        echo json_encode(['access' => $result]);
        break;

    // ─── Grant IP access to backup FTP ───
    case 'backup_ftp_grant':
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { echo json_encode(['error' => 'POST required']); break; }
        $input = json_decode(file_get_contents('php://input'), true);
        $name = $input['name'] ?? '';
        $ip = $input['ipBlock'] ?? '';
        $ftp = $input['ftp'] ?? true;
        $nfs = $input['nfs'] ?? false;
        $cifs = $input['cifs'] ?? false;
        if (!$name || !$ip) { echo json_encode(['error' => 'Server name and ipBlock required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $body = ['ipBlock' => $ip, 'ftp' => (bool)$ftp, 'nfs' => (bool)$nfs, 'cifs' => (bool)$cifs];
        $result = ovhRequest('POST', "/dedicated/server/$name/features/backupFTP/access", $body);
        echo json_encode(['success' => true, 'task' => $result, 'message' => 'Backup FTP access granted']);
        break;

    // ─── Revoke IP access from backup FTP ───
    case 'backup_ftp_revoke':
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { echo json_encode(['error' => 'POST required']); break; }
        $input = json_decode(file_get_contents('php://input'), true);
        $name = $input['name'] ?? '';
        $ip = $input['ipBlock'] ?? '';
        if (!$name || !$ip) { echo json_encode(['error' => 'Server name and ipBlock required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $result = ovhRequest('DELETE', "/dedicated/server/$name/features/backupFTP/access/" . urlencode($ip));
        echo json_encode(['success' => true, 'message' => 'Backup FTP access revoked']);
        break;

    // ─── Change backup FTP password ───
    case 'backup_ftp_password':
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { echo json_encode(['error' => 'POST required']); break; }
        $input = json_decode(file_get_contents('php://input'), true);
        $name = $input['name'] ?? '';
        if (!$name) { echo json_encode(['error' => 'Server name required']); break; }
        $name = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        $result = ovhRequest('POST', "/dedicated/server/$name/features/backupFTP/password");
        echo json_encode(['success' => true, 'task' => $result, 'message' => 'Backup FTP password change initiated — new password will be emailed']);
        break;
    ...
    case 'info':
        echo json_encode([
            ...
            'endpoints' => [
                ...
                'GET backup_ftp_status' => 'Backup FTP storage status',
                'POST backup_ftp_activate' => 'Activate backup FTP storage',
                'GET backup_ftp_access' => 'List authorized IPs for backup FTP',
                'POST backup_ftp_grant' => 'Grant IP access to backup FTP',
                'POST backup_ftp_revoke' => 'Revoke IP access from backup FTP',
                'POST backup_ftp_password' => 'Change backup FTP password'
            ]
        ]);
        break;
