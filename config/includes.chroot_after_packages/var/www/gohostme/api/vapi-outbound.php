...
function callScheduledCallbacks() {
    ...
    $s = $db->query("
        SELECT id, caller_number, verified_number, client_id, callback_reason, greeting_summary, created_at
        FROM alfred_callbacks
        WHERE callback_status = 'pending'
        AND callback_reason IN ('follow-up', 'reconnect', 'security_callback')
    ...
    foreach ($callbacks as $cb) {
        ...
        $newStatus = ($result['success'] ?? false) ? 'calling' : 'failed';
          $db->prepare("UPDATE alfred_callbacks SET callback_status = :s, called_at = NOW(), outbound_call_id = :ocid WHERE id = :id")
              ->execute([':s' => $newStatus, ':ocid' => $result['call_id'] ?? '', ':id' => $cb['id']]);
