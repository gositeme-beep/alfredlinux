...
function fetchPage($url) {
    ...
    // SSRF protection — validate target host is not internal
    // DNS-pinning TOCTOU risk is mitigated: curl goes to r.jina.ai, not the resolved IP
    $ip = gethostbyname($parsed['host']);
    if ($ip === $parsed['host']) return null; // DNS resolution failed
    if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE) === false) {
        return null;
    }

    // Fetch via Jina reader (proxied — SSRF risk is on Jina side, but we still validate above)
