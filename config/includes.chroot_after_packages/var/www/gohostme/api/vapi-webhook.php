...
function handleAssistantRequest(array $msg): void {
    ...
    // ── Caller recognition: identify who's calling and inject context ──
    ...
    if ($callerNumber) {
        $db = $db ?? getDB();
        if ($db) {
            // Phone-to-client lookup (same pattern as handleEndOfCall)
            $cleaned = preg_replace('/\D/', '', $callerNumber);
            $last10 = substr($cleaned, -10);
            try {
                $st = $db->prepare("SELECT id, firstname, lastname FROM clients WHERE REPLACE(REPLACE(REPLACE(phone,'+',''),'-',''),' ','') LIKE :p LIMIT 1");
                $st->execute([':p' => '%' . $last10]);
                $row = $st->fetch(PDO::FETCH_ASSOC);
                if ($row) {
                    $callerClientId = (int)$row['id'];
                    $callerName = trim(($row['firstname'] ?? '') . ' ' . ($row['lastname'] ?? ''));
                }
            } catch (Exception $e) {
                error_log('VAPI caller-lookup: ' . $e->getMessage());
            }

            // Store in Redis for call lifecycle
            if ($callerClientId > 0 && $callId) {
                redisSetCallClientId($callId, $callerClientId);
            }

            // Retrieve previous call memory for returning callers
            if ($callerClientId > 0) {
                try {
                    $ms = $db->prepare("SELECT caller_name, key_topics, call_summary, sentiment FROM alfred_call_memory WHERE client_id = :uid ORDER BY last_call_at DESC LIMIT 1");
                    $ms->execute([':uid' => $callerClientId]);
                    $mem = $ms->fetch(PDO::FETCH_ASSOC);
                    if ($mem) {
                        $callerContext .= 'Returning caller: ' . ($mem['caller_name'] ?: ($callerName ?? 'Unknown'))
                            . '. Previous topics: ' . ($mem['key_topics'] ?: 'none')
                            . '. Last sentiment: ' . ($mem['sentiment'] ?: 'neutral') . '. ';
                    }
                } catch (Exception $e) {}
            }

            // Commander detection (client_id=33 = Danny William Perez)
            if ($callerClientId === 33) {
                $callerContext = 'IMPORTANT: You are speaking to Commander Danny William Perez, '
                    . 'the founder and owner of GoSiteMe. Address him as Commander or Danny. '
                    . 'He has full authority over all systems. Prioritize his requests immediately. '
                    . 'He may have short-term memory issues — be patient and provide context proactively. '
                    . $callerContext;
            } elseif ($callerClientId > 0 && !empty($callerName)) {
                $callerContext = 'Caller identified: ' . $callerName . ' (client #' . $callerClientId . '). ' . $callerContext;
            }
        }
    }

    error_log('VAPI assistant-request OK assistantId=' . substr($assistantId, 0, 8) . '… phoneNumberId=' . ($phoneNumberId ?: 'default')
        . ($callerClientId ? ' caller=client#' . $callerClientId : ' caller=unknown'));
    logVapiEvent('assistant.request', [
        'call_id' => $callId,
        'phone_number_id' => $phoneNumberId,
        'assistant_id' => substr($assistantId, 0, 8),
        'caller_client_id' => $callerClientId,
        'caller_number' => $callerNumber ? substr($callerNumber, 0, 6) . '****' : '',
    ]);

    // Build response — add overrides if caller was recognized
    ...
    if ($callerContext !== '') {
        $response['assistantOverrides'] = [
            ...
            'variableValues' => [
                ...
                'callerClientId' => (string)$callerClientId,
            ],
        ];
    }
...
function postCallIntelligence($callId, $callerNum, $clientId, $summary, $transcript, $endedReason, $success, $duration, $db) {
    ...
    $prompt = <<<PROMPT
Analyze this phone call transcript from Alfred (AI support agent for GoSiteMe/Alfred IDE) and return a JSON object with these fields:
