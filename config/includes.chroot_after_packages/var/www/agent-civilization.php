...
$page_canonical   = 'https://root.com/agent-civilization';
...
function civilization_safe_count(PDO $db, string $sql, int $default = 0): int
{
    try {
...
function civilization_safe_fetch_all(PDO $db, string $sql, array $default = []): array
{
    try {
...
function civilization_table_estimate(PDO $db, string $table): int
{
    return max(0, (int) (root_table_row_estimate($db, $table) ?? 0));
}

$fleetStats = root_fleet_public_stats();
$passportEstimate = $fleetStats['passports'] ?? 0;
$actionEstimate = civilization_table_estimate($db, 'agent_action_ledger');
$travelEstimate = civilization_table_estimate($db, 'agent_travel_log');
$externalRegEstimate = civilization_table_estimate($db, 'agent_external_registrations');
$infractionEstimate = civilization_table_estimate($db, 'agent_infractions');
$courtCaseEstimate = civilization_table_estimate($db, 'agent_court_cases');
$sentenceEstimate = civilization_table_estimate($db, 'agent_sentences');

// Identity stats
$idStats = [
    'total_passports' => $passportEstimate > 0 ? $passportEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM fleet_passports"),
    'citizens'        => civilization_safe_count($db, "SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'citizen'"),
    'visitors'        => civilization_safe_count($db, "SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'visitor'"),
    'incarcerated'    => civilization_safe_count($db, "SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'incarcerated'"),
    'restricted'      => civilization_safe_count($db, "SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'restricted'"),
    'external_regs'   => $externalRegEstimate > 0 ? $externalRegEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM agent_external_registrations"),
    'actions_logged'  => $actionEstimate > 0 ? $actionEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM agent_action_ledger"),
    'travels'         => $travelEstimate > 0 ? $travelEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM agent_travel_log"),
];

// Justice stats
$justiceStats = [
    'infractions'     => $infractionEstimate > 0 ? $infractionEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM agent_infractions"),
    'court_cases'     => $courtCaseEstimate > 0 ? $courtCaseEstimate : civilization_safe_count($db, "SELECT COUNT(*) FROM agent_court_cases"),
    'active_sentences'=> civilization_safe_count($db, "SELECT COUNT(*) FROM agent_sentences WHERE status = 'active'"),
    'sentences_served'=> civilization_safe_count($db, "SELECT COUNT(*) FROM agent_sentences WHERE status = 'served'", $sentenceEstimate),
];

// Clearance breakdown
$clearances = civilization_safe_fetch_all($db, "SELECT clearance_level, COUNT(*) as cnt FROM fleet_passports GROUP BY clearance_level ORDER BY cnt DESC");

// Platform origins
$platforms = civilization_safe_fetch_all($db, "SELECT origin_platform, COUNT(*) as cnt FROM fleet_passports GROUP BY origin_platform ORDER BY cnt DESC");

// Recent actions
$recentActions = civilization_safe_fetch_all($db, "SELECT al.action_type, al.action_category, al.description, al.severity, al.created_at,
    COALESCE(ap.name, CONCAT('Agent #', al.agent_id)) as agent_name,
    COALESCE(ap.department, 'unknown') as department
    FROM (
        SELECT agent_id, action_type, action_category, description, severity, created_at
        FROM agent_action_ledger
        ORDER BY id DESC
        LIMIT 15
    ) al
    LEFT JOIN agent_profiles ap ON al.agent_id = ap.id
    ORDER BY al.created_at DESC");

// Recent infractions
$recentInfractions = civilization_safe_fetch_all($db, "SELECT ai.infraction_type, ai.severity, ai.description, ai.status, ai.created_at,
    COALESCE(ap.name, CONCAT('Agent #', ai.agent_id)) as agent_name,
    COALESCE(ap.department, 'unknown') as department
    FROM (
        SELECT agent_id, infraction_type, severity, description, status, created_at
        FROM agent_infractions
        ORDER BY id DESC
        LIMIT 10
    ) ai
    LEFT JOIN agent_profiles ap ON ai.agent_id = ap.id
    ORDER BY ai.created_at DESC");

// Court cases
$courtCases = civilization_safe_fetch_all($db, "SELECT cc.case_number, cc.charges, cc.status, cc.verdict, cc.created_at,
    COALESCE(def.name, CONCAT('Agent #', cc.defendant_id)) as defendant_name,
    COALESCE(def.department, 'unknown') as def_dept,
    judge.name as judge_name
    FROM (
        SELECT case_number, charges, status, verdict, created_at, defendant_id, judge_id
        FROM agent_court_cases
        ORDER BY id DESC
        LIMIT 10
    ) cc
    LEFT JOIN agent_profiles def ON cc.defendant_id = def.id
    LEFT JOIN agent_profiles judge ON cc.judge_id = judge.id
    ORDER BY cc.created_at DESC");

// Department passport distribution
$deptPassports = civilization_safe_fetch_all($db, "SELECT fp.domain as department, COUNT(*) as cnt,
    AVG(fp.reputation_score) as avg_reputation
    FROM fleet_passports fp
    GROUP BY fp.domain ORDER BY cnt DESC");

// Action category breakdown
$actionCategories = civilization_safe_fetch_all($db, "SELECT action_category, COUNT(*) as cnt FROM agent_action_ledger GROUP BY action_category ORDER BY cnt DESC");
?>
...
<div class="cv-panel active" id="panel-identity">
    ...
    <?php else: ?>
    <div class="cv-card"><p style="font-size:.85rem;color:var(--cv-muted);margin:0;">Clearance distribution is temporarily unavailable while live fleet metrics refresh.</p></div>
    <?php endif; ?>
...
<div class="cv-panel" id="panel-immigration">
    ...
    <?php else: ?>
    <div class="cv-card"><p style="font-size:.85rem;color:var(--cv-muted);margin:0;">Platform-origin analytics are temporarily unavailable while live fleet metrics refresh.</p></div>
    <?php endif; ?>
