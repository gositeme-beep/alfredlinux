...
function qgsm_safe_count(PDO $db, string $sql, int $default = 0): int
{
    try {
...
function qgsm_safe_float(PDO $db, string $sql, float $default = 0.0): float
{
    try {
        $value = $db->query($sql)->fetchColumn();
        return $value !== false && $value !== null ? (float) $value : $default;
    } catch (Throwable $e) {
        return $default;
    }
}

function qgsm_safe_fetch_all(PDO $db, string $sql, array $default = []): array
{
    try {
...
// Bridge stats
$totalSupply = qgsm_safe_float($db, "SELECT COALESCE(SUM(balance),0) FROM agent_gsm_balances");
$holders = qgsm_safe_count($db, "SELECT COUNT(*) FROM agent_gsm_balances WHERE balance > 0");
$totalEarnings = qgsm_safe_count($db, "SELECT COUNT(*) FROM agent_gsm_earnings");
$totalPassports = (int) ($fleetStats['passports'] ?? 0);
$externalRegs = qgsm_safe_count($db, "SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status='visitor'");
$totalVotes = qgsm_safe_count($db, "SELECT COUNT(*) FROM agent_service_votes");
$totalProposals = qgsm_safe_count($db, "SELECT COUNT(*) FROM agent_service_proposals");
$avgReputation = qgsm_safe_float($db, "SELECT ROUND(AVG(reputation_score),1) FROM fleet_passports");

// Top 5 earners by department
$topDepts = qgsm_safe_fetch_all($db, "SELECT p.department, COUNT(g.agent_id) as holders, ROUND(SUM(g.balance),2) as total_gsm
    FROM agent_gsm_balances g
    JOIN agent_profiles p ON g.agent_id = p.id COLLATE utf8mb4_general_ci
    WHERE g.balance > 0
    GROUP BY p.department ORDER BY total_gsm DESC LIMIT 5");
?>
...
<section class="qb-section">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.25rem;max-width:900px;margin:0 auto;">
        <?php
        $chains = [
            ['⟐', 'Solana', 'GSM is already live as an SPL token on Solana mainnet. QGSM↔GSM 1:1 bridge with atomic swaps.', 'var(--qb-purple)', 'GSM Live'],
