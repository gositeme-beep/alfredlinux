...
if [ -f api/version.json ] && [ -f config/hooks/live/0094-alfred-bible-tongues.hook.chroot ]; then
...
# Exclude grep/test lines that CHECK for the pattern (false positives).
while IFS= read -r hit; do
  [ -z "$hit" ] && continue
  # Skip lines that are grep checks (detecting, not setting)
  if [[ "$hit" == *"grep"* ]] || [[ "$hit" == *"sed"* ]]; then continue; fi
...
for f in "${HOOK_GLOB[@]}"; do
  [ -f "$f" ] || continue
  [[ "$f" == *0260-alfred-terminal-power* || "$f" == *0605-alfred-merged-auto* ]] && continue
...
for f in "${HOOK_GLOB[@]}"; do
  ...
  while IFS= read -r line; do
    ...
    [[ "$line" == *".onion"* ]] && continue
    [[ "$line" == *"tor+http://"* ]] && continue
    [[ "$line" == *"ppa.launchpad.net"* ]] && continue
    [[ "$line" == *"100.64."* ]] && continue
    [[ "$line" == *"15.235.50.60"* ]] && continue
    [[ "$line" == *".kingdom"* ]] && continue
    [[ "$line" == *"freedesktop.org/standards"* ]] && continue
    [[ "$line" == *"gnu.org"* ]] && continue
    [[ "$line" == *"example.com"* ]] && continue
    [[ "$line" == *"nullsoft.com"* ]] && continue
    [[ "$line" == *"api.gositeme.com"* ]] && continue
    [[ "$line" == *"gositemesovereign"* ]] && continue
    [[ "$line" == *"200:"* ]] && continue
    [[ "$line" == *"peer_ip"* ]] && continue
    [[ "$line" == *"http://\["* ]] && continue
    [[ "$line" == *"wikipedia.org"* ]] && continue
    [[ "$line" == *"debian.org"* ]] && continue
