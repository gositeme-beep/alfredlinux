...
# Run before lb build. Exit 1 if staged kernel debs are missing (linux-image-7.0.12*).
set -euo pipefail
...
shopt -s nullglob
debs=( "$PC"/linux-image-7.0.12*.deb )
if ((${#debs[@]})); then
  for f in "${debs[@]}"; do echo "  OK $f"; done
else
  echo "  FAIL: no linux-image-7.0.12*.deb -- lb/apt will fail without the Alfred kernel debs."
  echo "  See: $ROOT/config/packages.chroot/README-KERNEL7.txt"
  echo "  Fetch sources: bash scripts/kernel-download-7.0.12.sh"
  echo "  Pack debs from a builder: bash scripts/pack-kernel-debs-archive.sh"
  echo "  Then place linux-7.0.12-debs-for-iso.tar.gz under build-assets/kernel-7.0.12-debs/ or set ALFRED_KERNEL_DEBS_ARCHIVE"
  exit 1
fi

hdr=( "$PC"/linux-headers-7.0.12*.deb )
if ((${#hdr[@]})); then
  for f in "${hdr[@]}"; do echo "  OK $f"; done
else
  echo "  WARN: no linux-headers-7.0.12*.deb (often required alongside image)."
fi
