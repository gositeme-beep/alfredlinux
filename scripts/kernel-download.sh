...
# Download linux-7.0.12 tarball + patch into KERNEL_WORK (default: sibling dir kernel-7.0.12-work). Does NOT compile.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
source "$REPO/config/kernel.env"
BASE="${KERNEL_WORK:-$REPO/../kernel-${ALFRED_KERNEL_VERSION}-work}"
mkdir -p "$BASE"
cd "$BASE"

TAR="linux-${ALFRED_KERNEL_VERSION}.tar.xz"
PATCH="patch-${ALFRED_KERNEL_VERSION}.xz"
...
#   ALFRED_SKIP_KERNEL_SHA256_VERIFY=1 bash scripts/kernel-download-7.0.12.sh
verify_kernel_sha256() {
  ...
  grep -E "^[0-9a-f]{64}  linux-${ALFRED_KERNEL_VERSION}\.tar\.xz$" "$asc" >"$sums" || true
  grep -E "^[0-9a-f]{64}  patch-${ALFRED_KERNEL_VERSION}\.xz$" "$asc" >>"$sums" || true
  if ! [[ -s "$sums" ]]; then
...
echo "=== Next: tar xf $TAR ; cd linux-${ALFRED_KERNEL_VERSION} ; xzcat ../$PATCH | patch -p1"
