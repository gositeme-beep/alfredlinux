...
# fakeroot make bindeb-pkg in background for linux-7.0.12.
# KERNEL_WORK default: <repo>/../kernel-7.0.12-work  (put linux-7.0.12.tar.xz there; use kernel-download-7.0.12.sh)
...
TAR="${WORK}/linux-${ALFRED_KERNEL_VERSION}.tar.xz"
LOG="${WORK}/bindeb-pkg.log"
NJOBS="${NJOBS:-$(nproc)}"

[[ -f "$TAR" ]] || { echo "Missing $TAR — run: bash scripts/kernel-download.sh" >&2; exit 1; }
...
cd "$WORK"
if [[ ! -d "linux-${ALFRED_KERNEL_VERSION}" ]]; then
  echo "Extracting $TAR ..."
  tar xf "$TAR"
fi

cd "linux-${ALFRED_KERNEL_VERSION}"
...
nohup fakeroot make -j"$NJOBS" ARCH=x86_64 bindeb-pkg LOCALVERSION= KDEB_PKGVERSION="${ALFRED_KERNEL_VERSION}${ALFRED_KERNEL_DEB_SUFFIX}" >>"$LOG" 2>&1 &
