...
source /work/scripts/kernel.env || source /work/alfred-source/config/kernel.env || source /work/config/kernel.env || { echo "Missing kernel.env"; exit 1; }
...
cd "/work/linux-${ALFRED_KERNEL_VERSION}"
if [[ ! -f .config ]]; then
...
fakeroot make -j"$NJOBS" ARCH=x86_64 bindeb-pkg LOCALVERSION= KDEB_PKGVERSION="${ALFRED_KERNEL_VERSION}${ALFRED_KERNEL_DEB_SUFFIX}"
