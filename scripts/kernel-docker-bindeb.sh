...
[[ -f "$WORK/linux-7.0.1.tar.xz" ]] || { echo "Run first: bash scripts/kernel-download-7.0.1.sh" >&2; exit 1; }
[[ -f "$WORK/linux-7.0.1/Makefile" ]] || tar xf "$WORK/linux-7.0.1.tar.xz" -C "$WORK"
...
echo "=== .deb under $WORK ==="
ls -lh "$WORK"/linux-image-7.0.1*.deb 2>/dev/null || ls -lh "$WORK"/*.deb 2>/dev/null | head -30
