...
sed -i 's/cp -a /cp -al /g' /usr/lib/live/build/lb_binary_chroot /usr/lib/live/build/lb_chroot_includes /usr/lib/live/build/lb_binary_includes 2>/dev/null || true
echo "[inner] Executing lb bootstrap..."; if lb bootstrap </dev/null; then echo "[inner] Bootstrap complete."; else echo "Bootstrap failed"; exit 1; fi; if lb chroot </dev/null; then
  echo "[inner] Chroot phase complete. Locking OverlayFS snapshot..."
  bash /work/scripts/overlayfs-snapshot.sh lock || true
