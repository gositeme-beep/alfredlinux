...
# Download linux-7.0.4 tarball + patch into KERNEL_WORK (default: sibling dir kernel-7.0.4-work). Does NOT compile.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
BASE="${KERNEL_WORK:-$REPO/../kernel-7.0.4-work}"
mkdir -p "$BASE"
cd "$BASE"

TAR="linux-7.0.4.tar.xz"
PATCH="patch-7.0.4.xz"
...
#   ALFRED_SKIP_KERNEL_SHA256_VERIFY=1 bash scripts/kernel-download-7.0.4.sh
...
echo "=== Next: tar xf $TAR ; cd linux-7.0.4 ; xzcat ../$PATCH | patch -p1"
