...
security-audit.sh
  ...
  **repo_health_json**=`1` (optional JSON gate; see scripts/SECURITY-WAVES.txt).
  Ecosystem roadmap (cross-repo): **docs/MEGA-PLAN-L1-L9.txt** — content release manifest spec: **docs/MANIFEST-v1.txt** — advisory bookmarks: **docs/UPSTREAM-SECURITY-SIGNALS.txt** — GoSiteMe mega-plan PHP copies: **contrib/gositeme-web/** — unsigned JSON skeleton: **`bash scripts/emit-manifest-stub.sh`** (see **docs/MANIFEST-v1.txt**).
...
iso-preflight.sh
  Run before `lb build`. Fails if linux-image-7.0.12*.deb missing from
  config/packages.chroot/ (kernel debs + lists). Runs **`stage-kernel-debs-for-iso.sh`** first (archive under
  **`build-assets/kernel-7.0.12-debs/`**, **`ALFRED_KERNEL_DEBS_ARCHIVE`**, or **`KERNEL_WORK`**).
...
kernel-download-7.0.12.sh
  Downloads linux-7.0.12.tar.xz + patch-7.0.12.xz into ../kernel-7.0.12-work/ (or KERNEL_WORK=…).
  ...
  Kernel security scope + maintainer “best effort”: docs/KERNEL-7.0.12-SUPPLY-CHAIN-AUDIT.txt
  Supply-chain / trojan-path audit: docs/KERNEL-7.0.12-SUPPLY-CHAIN-AUDIT.txt
...
kernel-docker-bindeb.sh + kernel-docker-inner-bindeb.sh
  **No host sudo:** build inside `debian:bookworm` Docker. Usage:
  ...
  Then `docker logs -f <name>` (name in ../kernel-7.0.12-work/docker-bindeb.containername).
...
stage-kernel-debs-for-iso.sh
  Idempotent: unpack **`linux-7.0.12-debs-for-iso.tar.gz`** (or `.tar.zst`) from
  **`build-assets/kernel-7.0.12-debs/`** or **`ALFRED_KERNEL_DEBS_ARCHIVE`**, else copy from
  **`KERNEL_WORK`**. Used by **`iso-preflight.sh`** automatically.

pack-kernel-debs-archive.sh
  On the machine that already built the kernel: create **`linux-7.0.12-debs-for-iso.tar.gz`** under
  **`build-assets/kernel-7.0.12-debs/`** (gitignored) for another host / thin checkout. See
  **`build-assets/kernel-7.0.12-debs/README.txt`**.
