...
# DISABLED-BY-COMMANDER: "$REPO/.lock" \
#           "$REPO/build/.lock" \
#           "$REPO/build/.alfred-lb-docker-build.lock"
    find "$REPO/build" -maxdepth 1 -name "*.lock" -delete 2>/dev/null || true
    find "$REPO/build/.build" -maxdepth 1 -name "chroot_*" ! -name "chroot_cache" -delete 2>/dev/null || true
    find "$REPO/build/.build" -maxdepth 1 -name "binary_*" -delete 2>/dev/null || true
...
check_prereqs() {
    ...
    if [ -d "$REPO/cache/bootstrap/opt" ]; then
        echo "  ✅ Master chroot present at cache/bootstrap/"
    else
        if [ "$MODE" = "fast" ]; then
    ...
    if [ "$MODE" = "fast" ] && [ ! -f "$REPO/scripts/lb-docker-inner-build.sh" ]; then
        echo "  FATAL: Missing scripts/lb-docker-inner-build.sh" >&2
        exit 1
    fi
...
sync_config() {
    echo "[alfred-build] Syncing hooks, packages, and assets to build/config/ via Docker (ROOT)..."
    docker run --rm -v "$REPO:/work" debian:trixie bash -c '
        # # rm -rf /work/build/config 2>/dev/null
        mkdir -p /work/build/config/hooks/live /work/build/config/hooks/normal                  /work/build/config/packages.chroot /work/build/config/package-lists
        cp /work/config/hooks/live/* /work/build/config/hooks/live/ 2>/dev/null
        cp /work/config/hooks/normal/* /work/build/config/hooks/normal/ 2>/dev/null
        cp /work/config/packages.chroot/* /work/build/config/packages.chroot/ 2>/dev/null
        cp /work/config/package-lists/* /work/build/config/package-lists/ 2>/dev/null
        
        chmod 755 /work/build/config/hooks/live/*.hook.chroot 2>/dev/null
        chmod 755 /work/build/config/hooks/normal/*.hook.chroot 2>/dev/null
        chown -R root:root /work/build/config
    '
    echo "[alfred-build] Sync complete (ROOT)."
}
...
run_full() {
    ...
    DOCKER_ARGS+=( -v "/home/gositeme/law/models-vault:/models:ro" )
...
# DISABLED-BY-COMMANDER: docker rm -f alfred-lb-v-fast34 alfred-lb-v-fast-runner 2>/dev/null || true

    build_docker_args
    DOCKER_ARGS+=( -e "ALFRED_FAST_REPACK=1" )

    echo "[alfred-build] Hardlinking golden cache -> build/chroot to bypass debootstrap..."
    mkdir -p build/chroot
    sudo cp -al cache/bootstrap/* build/chroot/ 2>/dev/null || true
...
# # # DISABLED-BY-COMMANDER: /work/build/.lock /work/build/.alfred-lb-docker-build.lock
                find /work/build/ -maxdepth 1 -name '*.lock' -delete 2>/dev/null || true
                # Clear dpkg/apt locks from crashed builds
# DISABLED-BY-COMMANDER: /work/build/chroot/var/lib/dpkg/lock* 2>/dev/null || true
# DISABLED-BY-COMMANDER: /work/build/chroot/var/cache/apt/archives/lock 2>/dev/null || true
                ...
                bash $inner 2>&1 | tee -a /work/build/lb-docker-build.log
            "
        echo "$CONTAINER_NAME" > "$REPO/lb-docker.containername"
        echo "[alfred-build] ✅ Detached."
        echo "[alfred-build] Follow:  docker logs -f $CONTAINER_NAME"
