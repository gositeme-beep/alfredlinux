...
if [[ -d build-assets ]]; then
  ...
  # Sync alfred-models to includes.binary/live/alfred-models/ instead of includes.chroot
  if false; then # DISABLED FOR RAPID DESKTOP PROTOTYPE
    echo "[sync-canonical] alfred-models → includes.binary/live/alfred-models/ (outside squashfs)"
    mkdir -p "${ROOT}/build/config/includes.binary/live/alfred-models"
  fi

  if [[ "${ALFRED_FULL_BUILD_ASSETS:-}" == 1 ]]; then
    rsync -a --exclude="alfred-models" "${ROOT}/build-assets/" "$dst/"
    echo "[sync-canonical] build-assets/ → includes/build-assets/ (FULL rsync, excluding alfred-models)"
  else
...
# Persistent user experience overlay: keep personal settings in canonical source,
# not in disposable build/chroot runtime state.
if [[ -d state/experience-overlay ]]; then
  mkdir -p build/config/includes.chroot
  rsync -a "${ROOT}/state/experience-overlay/" "${ROOT}/build/config/includes.chroot/"
  echo "[sync-canonical] state/experience-overlay/ -> includes.chroot/ (persistent overlay)"
fi
...
# ─── Sync bootloaders (ISOLINUX + GRUB) ─────────────────────────────────
# live-build uses build/config/bootloaders/ for custom boot menus.
# Without this, it falls back to generic Debian syslinux configs.
if [[ -d config/bootloaders ]]; then
  mkdir -p build/config/bootloaders
  rsync -a "${ROOT}/config/bootloaders/" "${ROOT}/build/config/bootloaders/"
  echo "[sync-canonical] bootloaders → build/config/bootloaders/"
fi

# ─── Sync includes.binary (custom GRUB cfg, EFI assets) ─────────────────
if [[ -d config/includes.binary ]]; then
  mkdir -p build/config/includes.binary
  rsync -a "${ROOT}/config/includes.binary/" "${ROOT}/build/config/includes.binary/"
  echo "[sync-canonical] includes.binary → build/config/includes.binary/"
fi
if test -f config/kernel.env; then
  mkdir -p build/config
  cp -f config/kernel.env build/config/kernel.env
  echo sync-canonical kernel.env copied
fi
cp -r config/bootloaders build/config/ 2>/dev/null || true
