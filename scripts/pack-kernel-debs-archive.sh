...
# Pack linux-image-7.0.4 / headers / linux-libc-dev .deb files into a portable archive for other
...
#
# Sources (first with any matching .deb wins): KERNEL_WORK (default ../kernel-7.0.4-work), else
...
OUT_DIR="$ROOT/build-assets/kernel-7.0.4-debs"
OUT="${OUT:-$OUT_DIR/linux-7.0.4-debs-for-iso.tar.gz}"
...
collect() {
  ...
  local f found=0
  shopt -s nullglob
  for f in "$dir"/linux-image-7.0.4*.deb "$dir"/linux-headers-7.0.4*.deb "$dir"/linux-libc-dev_*.deb; do
    [[ -f "$f" ]] || continue
    cp -a "$f" "$TMP/"
    found=1
  done
...
if collect "$WORK"; then
  ...
  echo "No linux-image-7.0.4*.deb (or headers/libc) in $WORK or $PC" >&2
...
ls -lh "$OUT"
echo "OK. On another checkout: place under build-assets/kernel-7.0.4-debs/ or set ALFRED_KERNEL_DEBS_ARCHIVE, then: bash scripts/iso-preflight.sh"
