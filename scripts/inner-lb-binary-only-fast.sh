#!/bin/bash
exec > >(tee -a /work/lb-docker-build.log) 2>&1
echo "[inner-fast] Build started at $(date -u +%Y-%m-%dT%H:%M:%S+00:00)"
set -e
...
# DISABLED-BY-COMMANDER: rm -f /work/build/chroot/etc/apt/apt.conf.d/80-timeshift
...
# DISABLED-BY-COMMANDER: rm -f /work/build/binary/live/vmlinuz-7.0.12
...
# DISABLED-BY-COMMANDER: rm -f chroot/tmp/linux-*7.0.12*.deb
    else
...
# DISABLED-BY-COMMANDER: rm -f chroot/root/custom-packages/*99alfred*.deb || true
# DISABLED-BY-COMMANDER: rm -f chroot/root/masterstroke_kernel/*99alfred*.deb || true
chroot chroot dpkg --configure -a || true

# DISABLED-BY-COMMANDER: rm -f $B/binary_memtest $B/binary_grub-legacy \
...
# DISABLED-BY-COMMANDER: rm -f /work/build/chroot/var/lib/dpkg/info/linux-image*.postinst
chroot /work/build/chroot dpkg --configure -a || true


echo "=== manually running mksquashfs ==="
mkdir -p binary/live
# DISABLED-BY-COMMANDER: rm -f binary/live/filesystem.squashfs
