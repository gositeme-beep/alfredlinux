...
sudo chown gositeme:gositeme "$CANON"
ls -lah "$CANON"
...
sudo chown gositeme:gositeme "$CANON.torrent"
ls -lh "$CANON.torrent"
...
sudo rm -f "$CANON.asc"
sudo -u gositeme gpg --batch --yes \
  --local-user "$GPG_KEY" \
  --armor --detach-sign --output "$CANON.asc" "$CANON"
sudo -u gositeme gpg --verify "$CANON.asc" "$CANON"
ls -la "$CANON.asc"
...
for sf in "$SUMS" "$SUMS512" "$SUMSB3" "$SUMS256"; do
    if sudo test -f "$sf"; then
        ...
        sudo -u gositeme gpg --verify "$sf.asc" "$sf" >/dev/null 2>&1 \
          && echo "  signed: $sf.asc" \
...
HOOKS_RAN="${HOOKS_RAN:-0}"
echo "hooks-ran proxy count: $HOOKS_RAN (target: 42)"
...
echo "  1) Boot test (UEFI + BIOS) in QEMU/VirtualBox: uname -r should be 7.0.1"
