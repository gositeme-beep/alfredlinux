...
docker run \
  --name alfred-lb-runner-1783473912 \
  --privileged \
  --network=host \
  --shm-size=16G \
  --init \
...
if [ -f /work/build/live-image-amd64.hybrid.iso ]; then
  ...
  cp /work/build/live-image-amd64.hybrid.iso /work/iso-output/AlfredLinux-Alpha-Matrix-7.77-x86_64.iso && cp /work/build/live-image-amd64.hybrid.iso /work/iso-output/live-image-amd64.hybrid.iso
