 ...
 */
function gositeme_table_row_estimate(\PDO $db, string $table): ?int {
    try {
        $stmt = $db->prepare(
...
function gositeme_fleet_public_stats(): array {
    static $cached = null;
    if ($cached !== null) {
        return $cached;
    }
    ...
    try {
        ...
        $pEst = gositeme_table_row_estimate($db, 'fleet_passports');
        if ($pEst !== null && $pEst > 0) {
            $passports = $pEst;
        } else {
            try {
        ...
        try {
            $rEst = gositeme_table_row_estimate($db, 'alfred_agent_registry');
            if ($rEst !== null && $rEst > 0) {
                $registry = $rEst;
            } else {
