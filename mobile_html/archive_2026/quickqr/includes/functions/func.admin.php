...
function check_update_available(){
    global $config;
    // SOVEREIGNTY: External version check disabled — no phone-home
    $getVersions = ''; // was: file_get_contents('https://bylancer.com/api/quickad-release-versions.php')
    $versionList = explode("\n", $getVersions);
...
function check_purchse_valid(){
    ...
    if((time()-$cron_validation_exec_time) > $cron_validation_time) {
        ...
        if(isset($config['purchase_key'])){
            // SOVEREIGNTY: License verification disabled — no external phone-home
            $output = ['success' => true, 'purchase_type' => 'extended']; // Local override
            if ($output['success']) {
