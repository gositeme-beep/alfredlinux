        ...
        if($status != "error") {
            ...
            if($_POST['current_plan'] != 'free'){
                ...
                if($subsc_check == 1){
                    ...
                    $stmt = $pdo->prepare("UPDATE `".$config['db']['pre']."upgrades` SET `sub_id` = ?, `upgrade_expires` = ? WHERE `user_id` = ? LIMIT 1");
                    $stmt->execute([validate_input($_POST['current_plan']), validate_input($expires), validate_input($_POST['id'])]);
                }else{
...
function editMembershipPackage()
{
    ...
    if (isset($_POST['id'])) {
        ...
        $pdo = ORM::get_db();
        $stmt = $pdo->prepare("UPDATE `".$config['db']['pre']."usergroups` SET `group_name` = ?, `group_removable` = ?, `category_limit` = ?, `menu_limit` = ?, `scan_limit` = ? WHERE `group_id` = ? LIMIT 1");
        $query_result = $stmt->execute([validate_input($_POST['group_name']), validate_input($removable), validate_input($category_limit), validate_input($menu_limit), validate_input($scan_limit), validate_input($_POST['id'])]);
...
function addFAQentry()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!count($errors) > 0) {
            ...
            $pdo = ORM::get_db();
            $stmt = $pdo->prepare("UPDATE `".$config['db']['pre']."faq_entries` SET `translation_of` = ?, `parent_id` = ? WHERE `faq_id` = ? LIMIT 1");
            $query_result = $stmt->execute([validate_input($id), validate_input($id), validate_input($id)]);
    ...
    if (isset($_POST['id'])) {
        ...
        if (!count($errors) > 0) {
            ...
            if(isset($info['faq_title'])) {
                $pdo = ORM::get_db();
                $stmt = $pdo->prepare("UPDATE `" . $config['db']['pre'] . "faq_entries` SET `faq_title` = ?, `faq_content` = ?, `active` = ? WHERE `faq_id` = ? LIMIT 1");
                $stmt->execute([validate_input($_POST['title']), $_POST['content'], validate_input($active), $_POST['id']]);
            } else {
                if(isset($_POST['parent'], $_POST['lang']))
                $insert_faq = ORM::for_table($config['db']['pre'].'faq_entries')->create();
...
function paymentEdit()
{
    ...
    if (isset($_POST['id'])) {
        ...
        $pdo = ORM::get_db();
        $stmt = $pdo->prepare("UPDATE `".$config['db']['pre']."payments` SET `payment_title` = ?, `payment_install` = ? WHERE `payment_id` = ? LIMIT 1");
        $query_result = $stmt->execute([validate_input($_POST['title']), validate_input($_POST['install']), $_POST['id']]);
