...
module.exports = {
  apps: [
    ...
    // ── GoCodeMe MCP Server (500+ hosting/domain tools for IDE) ───
    {
      name: 'gocodeme-mcp',
      script: 'src/mcpHttpServer.js',
      cwd: `${APP_DIR}/gocodeme/mcp-server`,
      instances: 1,
      autorestart: true,
      watch: false,
    ...
    // ── Handshake SPV Node (domain sovereignty) ──────────
    {
      name: 'hsd-node',
      script: `${HOME}/handshake/start-hsd.js`,
      cwd: `${HOME}/handshake`,
      interpreter: 'node',
      autorestart: true,
      watch: false,
    ...
    // System Optimizer Watchdog — samples load and writes optimization findings continuously
    {
      name: 'system-optimizer-watchdog',
      script: `${APP_DIR}/scripts/system-optimizer-watchdog.php`,
      args: '--interval=30 --kill-enabled=1 --kill-threshold=20',
      interpreter: 'php',
      autorestart: true,
      watch: false,
      max_memory_restart: '128M',
    ...
    // ── Crypto Trading Agent — analyzes and trades every 15 min ──
    {
      name: 'crypto-trading-agent',
      script: `${APP_DIR}/scripts/crypto-trading-agent.js`,
      cron_restart: '*/15 * * * *',
      autorestart: false,
      watch: false,
      max_memory_restart: '128M',
    ...
    // ── Self-Evolution Engine — daily self-audit and upgrade proposals ──
    {
      name: 'self-evolution-engine',
      script: `${APP_DIR}/scripts/self-evolution-engine.js`,
      cron_restart: '0 6 * * *',
      autorestart: false,
      watch: false,
      max_memory_restart: '256M',
    ...
    // ── Social Outreach Engine — cross-posts to Discord/Twitter every 3h ──
    {
      name: 'social-outreach-engine',
      script: `${APP_DIR}/scripts/social-outreach-engine.js`,
      cron_restart: '0 */3 * * *',
      autorestart: false,
      watch: false,
      max_memory_restart: '128M',
    ...
    // ── Vault Guardian — watches vault key integrity every 30s ──
    {
      name: 'vault-guardian',
      script: `${HOME}/.vault/vault-guardian.sh`,
      interpreter: 'bash',
      autorestart: true,
      watch: false,
      max_restarts: 10,
    ...
    // ── Department Agent Loops ────────────────────────────────
    // Each department has a persistent agent that reads dept_intel,
    // generates analysis responses, and writes back to commander_intel_briefs.
    ...['education','families','justice','legal','nonprofit','police',
        'productivity','realestate','seniors','teachers','transport','university'
       ].map(dept => ({
      name: `dept-agent-${dept}`,
      script: `${APP_DIR}/scripts/dept-agent-loop.php`,
      args: `--dept=${dept} --poll=30`,
      interpreter: 'php',
      autorestart: true,
      watch: false,
      max_memory_restart: '128M',
