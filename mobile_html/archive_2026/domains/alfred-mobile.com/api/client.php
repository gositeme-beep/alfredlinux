...
function getServices($clientId) {
    ...
    if (!$db) {
        jsonResponse(['success' => false, 'services' => [], 'error' => 'Database connection failed'], 500);
        return;
    }
    ...
    try {
        ...
        error_log("getServices for client $clientId found " . count($services) . " services");
    } catch (Exception $e) {
        error_log("getServices error for client $clientId: " . $e->getMessage());
        jsonResponse(['success' => false, 'services' => [], 'error' => 'Query failed: ' . $e->getMessage()], 500);
        return;
    }
    ...
    foreach ($services as $service) {
        $formatted[] = [
            'id' => $service['id'],
            'product' => $service['product_name'] ?? 'Unknown Product',
            'group' => $service['group_name'] ?? 'Other',
            'domain' => $service['domain'] ?? '',
            'status' => $service['status'] ?? 'Unknown',
            'status_class' => getStatusClass($service['status'] ?? 'Unknown'),
            'registered' => $service['regdate'] ?? '0000-00-00',
            'next_due' => $service['next_due_date'] ?? '0000-00-00',
            'amount' => '$' . number_format(floatval($service['amount'] ?? 0), 2),
            'amount_raw' => floatval($service['amount'] ?? 0),
            'cycle' => ucfirst($service['billing_cycle'] ?? 'N/A'),
