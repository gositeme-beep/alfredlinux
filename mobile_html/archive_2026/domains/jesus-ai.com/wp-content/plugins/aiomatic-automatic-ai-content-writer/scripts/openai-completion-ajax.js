...
function openaifunct() {
    ...
    jQuery('#openai-response').html('<div><i class="fas fa-spinner fa-spin"></i></div>');
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            {
                jQuery('#openai-response').html('<div class="text-primary highlight-text-fail" role="status">' + response + '</div>');
            }
            ...
            else
            {
                ...
                {
                    jQuery('#openai-response').html('<i class="fas fa-pen ai-writing-icon"></i>');
                    var i = 0;
                    response = response.replace(/\n/g, '<br>');
                    function typeWriter() {
                        if (i < response.length) {
                            // Append the response to the input field
                            jQuery('#aiomatic_input').html(input_text + '<span class="highlight-green">' + response.substring(0, i + 1) + '</span>');
                            i++;
...
jQuery(document).ready(function() {
    jQuery('#copy-button').click(function() {
        ...
        var jsf = jQuery("#aiomatic_input").clone() //clone the element
        .find("br").replaceWith("\n")    //add new lines
    ...
    if(!jQuery('#aiomatic_completion_templates').length)
    {
        ...
        if ('webkitSpeechRecognition' in window) {
            ...
            jQuery('#openai-speech-button').click(function() {
                if (recognizing) {
                    recognition.stop();
                    recognizing = false;
                } else {
    ...
    {
        jQuery('#aiomatic_completion_templates').change(function()
        {
