...
function stableimagefunct() {
    ...
    jQuery('#openai-stable-image-response').html('<div><i class="fas fa-spinner fa-spin"></i></div>');
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            {
                jQuery('#openai-stable-image-response').html('<div class="text-primary highlight-text-fail" role="status">' + response + '</div>');
            ...
            else
            {
                ...
                {
                    jQuery("#aiomatic_stable_image_response").attr("src", "data:image/png;base64," + response).fadeIn();
...
jQuery(document).ready(function() {
    ...
    if(!jQuery('#aiomatic_stable_image_templates').length)
    {
        ...
        if ('webkitSpeechRecognition' in window) {
            ...
            jQuery('#openai-stable-image-speech-button').click(function() {
                if (recognizing) {
                    recognition.stop();
                    recognizing = false;
                } else {
    ...
    {
        jQuery('#aiomatic_stable_image_templates').change(function()
        {
