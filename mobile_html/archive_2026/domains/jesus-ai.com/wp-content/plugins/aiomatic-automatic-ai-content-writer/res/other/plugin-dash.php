		...
		function show_dashboard_widget() {
		    ...
		    if ( is_wp_error($rss) ) {
				if ( is_admin() || current_user_can('manage_options') ) {
				   echo '<p>';
				   printf('<strong>' . esc_html__( 'Error getting content: ', 'aiomatic-automatic-ai-content-writer' ) . '</strong>: %s', $rss->get_error_message());
				   echo '</p>';
				}
		    	return;
			}
			...
			if ( !$rss->get_item_quantity() ) {
			     echo '<p>' . esc_html__( 'Apparently, there are no updates to show!', 'aiomatic-automatic-ai-content-writer' ) . '</p>';
			     $rss->__destruct();
			     unset($rss);
			     return;
			}
			...
			if ( !isset($items) )
			     foreach ( $rss->get_items(0, $this->num_items) as $item ) {
			          ...
			          $content .= "<p class='cr_al_right'><a href='" . esc_attr($link) . "'  target='_blank'>" . esc_html__( 'Learn more', 'aiomatic-automatic-ai-content-writer' ) . "</a></p>";
			...
			$str .= '</ul><div class="cr_clear"></div><a href="https://coderevolution.ro/knowledge-base" class="button button-secondary" target="_blank">' . esc_html__( 'FAQ', 'aiomatic-automatic-ai-content-writer' ) . '</a>&nbsp;<a href="https://coderevolution.ro/support" class="button button-secondary" target="_blank">' . esc_html__( 'Support', 'aiomatic-automatic-ai-content-writer' ) . '</a>&nbsp;<a href="https://1.envato.market/coderevolutionplugins" class="button button-primary" target="_blank">+ ' . esc_html__( 'View More', 'aiomatic-automatic-ai-content-writer' ) . '</a><a class="cr_float_right" href="#" id="wp_coderevodashboard_hide">' . esc_html__( 'Don\'t show this widget', 'aiomatic-automatic-ai-content-writer' ) . '</a><div class="cr_clear"></div>';
		...
		public function requestPluginInfo( $info ) {
			...
			$ret = array(
				...
				'sections' => array(
					...
					'installation' => '<p>' . esc_html__( 'Just download the plugin from CodeCanyon and install it to your site in a few seconds.', 'aiomatic-automatic-ai-content-writer' ) . '</p>
