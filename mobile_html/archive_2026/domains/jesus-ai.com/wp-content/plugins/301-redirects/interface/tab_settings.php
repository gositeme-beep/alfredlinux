...
class WF301_tab_settings extends WF301
{
    static function display()
    {
        ...
        if(array_key_exists('scheduled_scans', $license['meta']) && $license['meta']['scheduled_scans'] == true){
            $tabs[] = array('id' => 'tab_settings_link_scanner', 'class' => 'tab-content', 'label' => __('Link Scanner', '301-redirects'), 'callback' => array(__CLASS__, 'tab_link_scanner'));
        }
    ...
    static function tab_link_scanner()
    {
        $options = WF301_setup::get_options();
        
        $schedule_scan_settings =   array();
        $schedule_scan_settings[] = array('val' => 'off', 'label' => 'Off');
        $schedule_scan_settings[] = array('val' => '2days', 'label' => 'Every 2 days ');
        $schedule_scan_settings[] = array('val' => '3days', 'label' => 'Every 3 days ');
        $schedule_scan_settings[] = array('val' => '1week', 'label' => 'Every week ');
        $schedule_scan_settings[] = array('val' => '2week', 'label' => 'Every 2 weeks ');

        echo '<table class="form-table">';
        
        echo '<tr valign="top">
        <th scope="row"><label for="scheduled_scans">Schedule Scans</label></th>
        <td><select id="scheduled_scans" name="' . WF301_OPTIONS_KEY . '[scheduled_scans]">';
        WF301_utility::create_select_options($schedule_scan_settings, $options['scheduled_scans']);
        echo '</select>';
        echo '</td></tr>';
