...
class WF301_admin extends WF301
{
    ...
    static function admin_enqueue_scripts($hook)
    {
        ...
        if ('settings_page_301redirects' == $hook) {
            ...
            wp_dequeue_style('wpf_bootstrap_script');
        } else {
            $js_localize = array(
                'run_tool_nonce' => wp_create_nonce('wf301_run_tool'),
                'permalink_change_notices' => WF301_functions::get_changed_permalinks_notices(isset($post->ID) ? $post->ID : false),
    ...
    static function widget_content()
    {
        ...
        if (!$events) {
            ...
            foreach ($events as $l) {
                ...
                echo '<td><a title="Open target URL in a new tab" target="_blank" href="' . esc_url($l->url) . '">' . esc_html($l->url) . '</a> <span class="dashicons dashicons-external"></span></td>';
    ...
    static function main_page()
    {
        ...
        if ($wf_301_licensing->is_active()) {
            ...
            if (!array_key_exists('disable_link_scanner', $license['meta']) || $license['meta']['disable_link_scanner'] == false) {
                $tabs[] = array('id' => 'wf301_scanner', 'icon' => 'wf301-icon wf301-search', 'class' => '', 'label' => __('Link Scanner', '301-redirects'), 'callback' => array('WF301_tab_scanner', 'display'));
            }
