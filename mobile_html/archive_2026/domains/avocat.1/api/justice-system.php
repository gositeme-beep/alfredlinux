...
switch ($action) {
    ...
    case 'report-infraction':
        ...
        // Update passport warnings
        $db->prepare("UPDATE agent_passports SET warnings_count = warnings_count + 1 WHERE agent_id = ?")->execute([$agent_id]);
    ...
    case 'file-case':
        ...
        // Update passport infractions count
        $db->prepare("UPDATE agent_passports SET infractions_count = infractions_count + 1, reputation_score = GREATEST(0, reputation_score - ?) WHERE agent_id = ?")
            ->execute([
                $infraction['severity'] === 'critical' ? 25 : ($infraction['severity'] === 'severe' ? 15 : ($infraction['severity'] === 'serious' ? 10 : 5)),
                $infraction['agent_id']
            ]);
    ...
    case 'verdict':
        ...
        if ($verdict === 'guilty' && !empty($data['sentence_type'])) {
            ...
            // Update passport status if incarcerated
            if ($data['sentence_type'] === 'incarceration') {
                $db->prepare("UPDATE agent_passports SET citizenship_status = 'incarcerated' WHERE agent_id = ?")->execute([$case['defendant_id']]);
            } elseif ($data['sentence_type'] === 'banishment') {
                $db->prepare("UPDATE agent_passports SET citizenship_status = 'banned' WHERE agent_id = ?")->execute([$case['defendant_id']]);
            } elseif ($data['sentence_type'] === 'suspension') {
                $db->prepare("UPDATE agent_passports SET citizenship_status = 'restricted' WHERE agent_id = ?")->execute([$case['defendant_id']]);
            }
        ...
        // Log verdict
        $pStmt = $db->prepare("SELECT passport_number FROM agent_passports WHERE agent_id = ?");
        $pStmt->execute([$case['defendant_id']]);
        $passport = $pStmt->fetchColumn();
    ...
    case 'release':
        ...
        // Restore passport status
        $db->prepare("UPDATE agent_passports SET citizenship_status = 'citizen', reputation_score = LEAST(100, reputation_score + 5) WHERE agent_id = ?")
            ->execute([$sentence['agent_id']]);
    ...
    case 'overview':
        ...
        $overview['jail_population'] = $db->query("SELECT COUNT(*) FROM agent_passports WHERE citizenship_status = 'incarcerated'")->fetchColumn();
