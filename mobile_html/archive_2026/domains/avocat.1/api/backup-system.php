...
function createBackup($db, $backupDir) {
    ...
    try {
        ...
        $dbFile = "{$backupDir}/{$backupId}-db.sql";
        
        // Get all base tables (skip views which may reference invalid objects)
        $tables = $db->query("SHOW FULL TABLES WHERE Table_type = 'BASE TABLE'")->fetchAll(PDO::FETCH_NUM);
        $tables = array_column($tables, 0);
        $tableCount = count($tables);
        
        $dump = "-- GoSiteMe Database Backup\n";
        $dump .= "-- Backup ID: {$backupId}\n";
        $dump .= "-- Date: " . date('Y-m-d H:i:s') . "\n";
        $dump .= "-- Tables: {$tableCount}\n";
        ...
        foreach ($tables as $table) {
          try {
            ...
            while ($row = $rows->fetch(PDO::FETCH_ASSOC)) {
                ...
                $dump .= "INSERT INTO `{$table}` ({$cols}) VALUES (" . implode(',', $values) . ");\n";
                
                // Batch insert statements for large tables
                if ($rowCount % 1000 === 0) {
                    file_put_contents($dbFile, $dump, FILE_APPEND);
                    $dump = '';
                }
            }
            $dump .= "\n";
          } catch (Exception $tableErr) {
            $dump .= "-- SKIPPED TABLE `{$table}`: " . str_replace("\n", " ", $tableErr->getMessage()) . "\n\n";
          }
        }
        ...
        $dbGzFile = $dbFile . '.gz';
        $fp = fopen($dbFile, 'rb');
        $gz = gzopen($dbGzFile, 'wb9');
        while (!feof($fp)) {
            gzwrite($gz, fread($fp, 524288)); // 512KB chunks
        }
        fclose($fp);
        gzclose($gz);
        unlink($dbFile); // Remove uncompressed 
        
        $dbSizeMB = round(filesize($dbGzFile) / (1024 * 1024), 2);
