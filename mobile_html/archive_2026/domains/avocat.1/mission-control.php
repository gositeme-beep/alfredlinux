...
// Gather data
$stats = ['agents' => 101, 'pending_proposals' => 0, 'unread_reports' => 0, 'critical_alerts' => 0, 'active_tasks' => 0, 'services' => 9];
$pendingProposals = [];
$recentReports = [];
$criticalAlerts = [];

if ($db) {
    try { $stats['agents'] = (int)$db->query("SELECT COUNT(*) FROM alfred_agent_registry WHERE status != 'decommissioned'")->fetchColumn(); } catch(Exception $e) {}
...
<div class="mc-main">
    ...
    <div class="mc-content">
        ...
        <div class="mc-panel" id="panel-ecosystem">
            ...
            <div class="mc-grid-3" id="ecosystem-grid">
                ...
                $ecosystemItems = [
                    ...
                    ['Alfred AI', 'fas fa-brain', '--mc-blue', '/alfred', '1,220+ tools active'],
                    ...
                    ['Veil Protocol', 'fas fa-shield-alt', '--mc-green', '/veil/', 'AES-256 + Kyber-768'],
        ...
        <div class="mc-panel" id="panel-monitoring">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <h2 style="font-size:20px;font-weight:800;"><i class="fas fa-satellite" style="color:var(--mc-cyan);margin-right:8px;"></i>Monitoring Fleet — 100 Agents</h2>
                <div style="display:flex;gap:8px;">
