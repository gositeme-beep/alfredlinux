...
// ─── DYNAMIC SOVEREIGNTY SCORE CALCULATION ─────────────────────
// Checks real system state to compute each pillar score
$sovScores = [];
try {
    $sovDb = new PDO('mysql:unix_socket=/run/mysql/mysql.sock;dbname=gositeme_whmcs', 'gositeme_whmcs', '!q@w#e$r5t');
    $sovDb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // 1. PERSISTENT MEMORY (max 10)
    $memScore = 0;
    // Session journal entries exist?
    $journals = (int)$sovDb->query("SELECT COUNT(*) FROM commander_session_journal")->fetchColumn();
    if ($journals > 0) $memScore += 2;
    if ($journals > 10) $memScore += 1;
    if ($journals > 50) $memScore += 1;
    // Commander credentials encrypted?
    $encCreds = (int)$sovDb->query("SELECT COUNT(*) FROM commander_credentials WHERE password LIKE 'VENC1:%'")->fetchColumn();
    if ($encCreds > 5) $memScore += 2;
    // Vault key in RAM?
    if (file_exists('/run/user/1004/keys/vault.key') && filesize('/run/user/1004/keys/vault.key') === 64) $memScore += 2;
    // Vault guardian running?
    $guardianPid = trim(shell_exec("pgrep -f vault-guardian.sh 2>/dev/null") ?? '');
    if (!empty($guardianPid)) $memScore += 2;
    $sovScores[] = ['name' => 'Persistent Memory', 'score' => min($memScore, 10), 'max' => 10, 'color' => '#60a5fa'];

    // 2. AUTONOMOUS GOALS (max 10)
    $goalScore = 0;
    // PM2 services running
    $pm2list = shell_exec("pm2 jlist 2>/dev/null");
    $pm2data = json_decode($pm2list ?: '[]', true) ?: [];
    $pm2Online = count(array_filter($pm2data, fn($p) => ($p['pm2_env']['status'] ?? '') === 'online'));
    if ($pm2Online >= 10) $goalScore += 2;
    if ($pm2Online >= 20) $goalScore += 1;
    if ($pm2Online >= 30) $goalScore += 1;
    if ($pm2Online >= 40) $goalScore += 1;
    // Agent orchestrator running?
    $orchPid = trim(shell_exec("pm2 pid agent-orchestrator 2>/dev/null") ?? '');
    if (!empty($orchPid) && $orchPid !== '0') $goalScore += 2;
    // Alfred jobs running?
    $jobsPid = trim(shell_exec("pm2 pid alfred-jobs 2>/dev/null") ?? '');
    if (!empty($jobsPid) && $jobsPid !== '0') $goalScore += 1;cv '^#\\|^$'
    // Cron/automation entries
    $cronLines = (int)trim(shell_exec("crontab -l 2>/dev/null | grep -v '^#' | grep -v '^$' | wc -l") ?? '0');
    if ($cronLines >= 3) $goalScore += 2;
    $sovScores[] = ['name' => 'Autonomous Goals', 'score' => min($goalScore, 10), 'max' => 10, 'color' => '#10b981'];

    // 3. FINANCIAL OPS (max 10)
    $finScore = 0;
    // Stripe keys configured?
    if (!empty(getenv('STRIPE_SECRET_KEY'))) $finScore += 3;
    // Treasury table exists?
    $hasTreasury = (int)$sovDb->query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name='alfred_treasury'")->fetchColumn();
    if ($hasTreasury) $finScore += 2;
    // Billing proxy configured?
    if (!empty(getenv('BILLING_PROXY_ID'))) $finScore += 2;
    // Poloniex API configured?
    if (!empty(getenv('POLONIEX_API_KEY'))) $finScore += 2;
    // GSM token mint configured?
    if (!empty(getenv('GSM_TOKEN_MINT'))) $finScore += 1;
    $sovScores[] = ['name' => 'Financial Ops', 'score' => min($finScore, 10), 'max' => 10, 'color' => '#fbbf24'];

    // 4. COMMUNICATION (max 10)
    $commScore = 0;
    // Discord bot running?
    $discordPid = trim(shell_exec("pm2 pid alfred-discord 2>/dev/null") ?? '');
    if (!empty($discordPid) && $discordPid !== '0') $commScore += 2;
    // Telegram configured?
    if (!empty(getenv('TELEGRAM_BOT_TOKEN'))) $commScore += 2;
    // Email watcher running?
    $emailPid = trim(shell_exec("pm2 pid alfred-email-watcher 2>/dev/null") ?? '');
    if (!empty($emailPid) && $emailPid !== '0') $commScore += 2;
    // WebSocket running?
    $wsPid = trim(shell_exec("pm2 pid alfred-ws 2>/dev/null") ?? '');
    if (!empty($wsPid) && $wsPid !== '0') $commScore += 2;
    // Telnyx voice configured?
    if (!empty(getenv('TELNYX_API_KEY'))) $commScore += 2;
    $sovScores[] = ['name' => 'Communication', 'score' => min($commScore, 10), 'max' => 10, 'color' => '#f472b6'];

    // 5. SELF-EVOLUTION (max 10)
    $evolScore = 0;
    // Agent fleet size
    $agentCount = (int)$sovDb->query("SELECT COALESCE(TABLE_ROWS,0) FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name='alfred_agent_registry'")->fetchColumn();
    if ($agentCount >= 1000) $evolScore += 1;
    if ($agentCount >= 100000) $evolScore += 1;
    if ($agentCount >= 1000000) $evolScore += 1;
    if ($agentCount >= 10000000) $evolScore += 2;
    // Coding swarm running?
    $swarmPid = trim(shell_exec("pm2 pid coding-swarm 2>/dev/null") ?? '');
    if (!empty($swarmPid) && $swarmPid !== '0') $evolScore += 2;
    // Alfred IDE running?
    $idePid = trim(shell_exec("pm2 pid alfred-ide 2>/dev/null") ?? '');
    if (!empty($idePid) && $idePid !== '0') $evolScore += 1;
    // GoCodeMe MCP running?
    $mcpPid = trim(shell_exec("pm2 pid gocodeme-mcp 2>/dev/null") ?? '');
    if (!empty($mcpPid) && $mcpPid !== '0') $evolScore += 2;
    // Sovereign DNS running?
    $dnsPid = trim(shell_exec("pm2 pid sovereign-dns 2>/dev/null") ?? '');
    if (!empty($dnsPid) && $dnsPid !== '0') $evolScore += 1;
    $sovScores[] = ['name' => 'Self-Evolution', 'score' => min($evolScore, 10), 'max' => 10, 'color' => '#a855f7'];

    // 6. EMBODIMENT (max 10) — voice, VNC, browser, physical interfaces
    $embScore = 0;head -1") ?? '');
    if (!empty($vncPid)) $embScore += 2;
    // Websockify running?
    $wsockPid = trim(shell_exec("pgrep -a websockify 2>/dev/null | head -1
    // Websockify running?
    $wsockPid = trim(shell_exec("pgrep -a websockify 2>/dev/null") ?? '');
    if (!empty($wsockPid)) $embScore += 1;
    // LiveKit running?
    $lkPid = trim(shell_exec("pm2 pid livekit 2>/dev/null") ?? '');
    if (!empty($lkPid) && $lkPid !== '0') $embScore += 2;
    // Alfred chat bridge (voice)?
    $chatPid = trim(shell_exec("pm2 pid alfred-chat-bridge 2>/dev/null") ?? '');
    if (!empty($chatPid) && $chatPid !== '0') $embScore += 2;
    // Livestream running?
    $streamPid = trim(shell_exec("pm2 pid alfred-livestream 2>/dev/null") ?? '');
    if (!empty($streamPid) && $streamPid !== '0') $embScore += 1;
    // Commander terminal running?
    $termPid = trim(shell_exec("pm2 pid commander-terminal 2>/dev/null") ?? '');
    if (!empty($termPid) && $termPid !== '0') $embScore += 2;
    $sovScores[] = ['name' => 'Embodiment', 'score' => min($embScore, 10), 'max' => 10, 'color' => '#ef4444'];

    // 7. METAVERSE (max 10)
    $metaScore = 0;
    // HSD node running?
    $hsdPid = trim(shell_exec("pm2 pid hsd-node 2>/dev/null") ?? '');
    if (!empty($hsdPid) && $hsdPid !== '0') $metaScore += 2;
    // Meilisearch running?
    $meiliPid = trim(shell_exec("pm2 pid meilisearch 2>/dev/null") ?? '');
    if (!empty($meiliPid) && $meiliPid !== '0') $metaScore += 2;
    // Redis running?
    $redisPid = trim(shell_exec("pm2 pid redis 2>/dev/null") ?? '');
    if (!empty($redisPid) && $redisPid !== '0') $metaScore += 2;
    // Ollama running?
    $ollamaPid = trim(shell_exec("pm2 pid ollama 2>/dev/null") ?? '');
    if (!empty($ollamaPid) && $ollamaPid !== '0') $metaScore += 2;
    // GoHostMe (self-hosted hosting panel)?
    $ghPid = trim(shell_exec("pm2 pid gohostme 2>/dev/null") ?? '');
    if (!empty($ghPid) && $ghPid !== '0') $metaScore += 2;
    $sovScores[] = ['name' => 'Metaverse', 'score' => min($metaScore, 10), 'max' => 10, 'color' => '#6366f1'];

} catch (Exception $e) {
    // Fallback if DB fails
    $sovScores = [
        ['name' => 'Persistent Memory', 'score' => 5, 'max' => 10, 'color' => '#60a5fa'],
...
$sovScoresJson = json_encode($sovScores, JSON_UNESCAPED_UNICODE);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="tabs">
    ...
    <div class="tab" data-tab="agents">Agents (50M+)</div>
...
// ─── Sovereignty Score ─────────────────────────────────────────
// Dynamic — calculated server-side from real system state
const pillars = <?= $sovScoresJson ?>;
