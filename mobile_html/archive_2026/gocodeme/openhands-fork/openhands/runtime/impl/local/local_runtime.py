...
def check_dependencies(code_repo_path: str, check_browser: bool) -> None:
    ...
    if sys.platform != 'win32':
        ...
        try:
            # Kill any leftover test-session from a previous run
            try:
                old = server.sessions.get(session_name='test-session')
                if old:
                    old.kill()
            except Exception:
                pass
        ...
        pane = session.active_pane  # attached_pane removed in libtmux 0.31+
        pane.send_keys('echo "test"')
        pane_output = '\n'.join(pane.cmd('capture-pane', '-p').stdout)
        session.kill()  # kill_session() removed in libtmux 0.30+
