...
const CFG = {
  wsUrl:      window.AW_WS_URL      || ((typeof location !== 'undefined' ? (location.protocol === 'https:' ? 'wss://' : 'ws://') + location.host : 'wss://gositeme.com') + '/voice-ws'),
...
function connectWS() {
  ...
  try {
    ...
    state.ws.onclose = () => {
      ...
      // If WS dies while waiting for response, immediately fall back to REST
      if (state._wsTimeout && DOM.messages.querySelector('.aw-typing-indicator')) {
        clearTimeout(state._wsTimeout);
        state._wsTimeout = null;
        const lastUser = [...state.messages].reverse().find(m => m.role === 'user');
        if (lastUser) { showTyping(false); sendViaRest(lastUser.text); }
      }
      wsRetries++;
...
function handleWSMessage(evt) {
  ...
  try {
    ...
    switch(msg.type) {
      case 'transcript':
        if (state._voiceTimeout) { clearTimeout(state._voiceTimeout); state._voiceTimeout = null; }
        showTyping(false);
...
async function sendViaRest(text, _retry) {
  showTyping(true);
    ...
    const _to = _ac ? setTimeout(() => _ac.abort(), 90000) : null;
  try {
    const effectiveModel = getEffectiveModel();
    let ctx = state.pageContext || '';
    if (typeof window.getChessContext === 'function') {
    ...
    if (e.name === 'AbortError') {
      addSystemMsg('Request timed out. Please try again.');
    } else {
...
function handleVoiceCommand(text) {
  ...
  if (/^(read\s*(?:that|it|last)?\s*(?:back|aloud|out\s*loud)?|speak\s*(?:that|it|last)?|say\s*(?:that|it)\s*(?:again|aloud)?)$/i.test(t)) {
    ...
    if (lastAlfred) {
      const ttsText = lastAlfred.text.replace(/<[^>]*>/g, '').substring(0, 4000);
      addSystemMsg('🔊 Reading with Alfred\'s voice...');
      fetch('/middleware/api/voice-relay/tts', {
        method: 'POST',
        ...
        .then(buf => {
          ...
          return state.audioCtx.decodeAudioData(buf.slice(0));
        }).then(decoded => {
          const src = state.audioCtx.createBufferSource();
          src.buffer = decoded; src.connect(state.audioCtx.destination); src.start();
        }).catch(() => {
  ...
  try {
    ...
    const safetyTimer = setTimeout(() => {
      if (state._speechRec === recognition) {
        ...
        // Force cleanup if onend doesn't fire within 2s after stop()
        setTimeout(() => {
          if (state._speechRec === recognition) {
            ...
            if (t) {
              ...
              addSystemMsg('Speech recognition timed out. Try typing instead.');
            }
          }
        }, 2000);
      }
...
async function sendAudioBlob(blob) {
  ...
  if (state.wsReady) {
    ...
    // Safety timeout — if WS doesn't respond within 10s, clear typing and show error
    if (state._voiceTimeout) clearTimeout(state._voiceTimeout);
    state._voiceTimeout = setTimeout(() => {
      if (DOM.messages.querySelector('.aw-typing-indicator')) {
        showTyping(false);
        addSystemMsg('Voice processing timed out. Try typing your message instead.');
      }
      state._voiceTimeout = null;
    }, 10000);
  } else {
