        ...
        async function loadServices() {
            try {
                ...
                if (svcs.length === 0) {
                    console.warn('No services returned. Data:', data);
        ...
        async function loadDomains() {
            try {
                ...
                let doms = [];
                if (Array.isArray(data)) doms = data;
                else if (data.success && data.domains) doms = data.domains;
        ...
        async function loadInvoices() {
            try {
                ...
                let invs = [];
                if (Array.isArray(data)) invs = data;
                else if (data.success && data.invoices) invs = data.invoices;
        ...
        async function loadTickets() {
            try {
                ...
                let tix = [];
                if (Array.isArray(data)) tix = data;
                else if (data.success && data.tickets) tix = data.tickets;
        ...
        // Load all data — staggered to avoid mod_evasive rate limiting
        // (DOSPageCount 2 per second to same PHP file)
        async function loadAllDashboardData() {
            ...
            // PIN status uses a different endpoint, safe to call
            loadPinStatus();
        }
