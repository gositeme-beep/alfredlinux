...
function handleBrowse(PDO $db): void {
    $type     = sanitize($_REQUEST['type'] ?? '', 20);
    $category = sanitize($_REQUEST['category'] ?? '', 60);
    $sort     = in_array($_REQUEST['sort'] ?? '', ['popular','newest','rating','name','price_low','price_high']) 
                ? $_REQUEST['sort'] : 'popular';
    $rating   = sanitize($_REQUEST['content_rating'] ?? '', 10);
...
function handleDetail(PDO $db): void {
    $slug = sanitize($_REQUEST['slug'] ?? '', 120);
...
function handleInstall(PDO $db, int $client_id): void {
    ...
    // Verify item exists
    $item = $db->prepare("SELECT id, version, price FROM store_items WHERE id = ? AND status = 'active'");
    ...
    echo json_encode(['success' => true, 'installed_version' => $itemData['version']]);
}
...
function handleReview(PDO $db, int $client_id): void {
    $item_id = (int)($_REQUEST['item_id'] ?? 0);
    $rating  = max(1, min(5, (int)($_REQUEST['rating'] ?? 0)));
    $title   = sanitize($_REQUEST['review_title'] ?? '', 200);
    $body    = sanitize($_REQUEST['body'] ?? '', 2000);
...
function handleCollections(PDO $db): void {
    $slug = sanitize($_REQUEST['slug'] ?? '', 80);
...
function seedStoreIfEmpty(PDO $db): void {
    ...
    $items = [
        ...
        ['app','gocodeme-ide','GoCodeMe IDE — Web','Cloud IDE with Monaco editor, AI chat, project management. No download needed.','GoSiteMe','💻','development','0.00','free','everyone','2.0.0','/editor/',1,1,80],
