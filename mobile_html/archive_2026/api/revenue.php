...
function getAuthUser(): ?array {
    ...
    if (!empty($_SESSION['logged_in']) && !empty($_SESSION['client_id'])) {
        return ['id' => (int)$_SESSION['client_id'], 'name' => $_SESSION['client_name'] ?? ''];
    }
...
switch ($action) {
    ...
    case 'allocate':
        ...
        $stmt = $db->prepare("UPDATE platform_treasury SET total_allocated = total_allocated + ?, available_balance = total_platform_share - total_allocated - ? WHERE id = 1");
