...
function parseDOCX($filePath) {
    ...
    if ($zip->open($filePath) === true) {
        ...
        if ($content) {
            ...
            $xml = simplexml_load_string($content, 'SimpleXMLElement', LIBXML_NONET | LIBXML_NOENT | LIBXML_DTDLOAD);
            if ($xml) {
...
function parseXML($filePath) {
    $content = file_get_contents($filePath);
    // Reject files with DOCTYPE declarations (XXE/XML bomb vector)
    if (preg_match('/<!DOCTYPE/i', $content)) {
        return ['text' => '[XML with DOCTYPE rejected for security]', 'pages' => 0, 'method' => 'blocked'];
    }
    libxml_use_internal_errors(true);
