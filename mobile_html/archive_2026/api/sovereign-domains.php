...
function searchSovereignDomain() {
    ...
    // Get all active TLDs
    $tlds = $db->query("SELECT id, tld, display_name, icon, price_usd, price_gsm, status FROM sovereign_tlds WHERE status IN ('active', 'coming_soon') ORDER BY FIELD(status, 'active', 'coming_soon'), price_usd ASC")->fetchAll(PDO::FETCH_ASSOC);
...
function checkSovereignDomain() {
    ...
    // Verify TLD exists
    $tldRow = $db->prepare("SELECT id, tld, display_name, price_usd, price_gsm, status FROM sovereign_tlds WHERE tld = ?");
    $tldRow->execute([$tld]);
...
function getSovereignTLDs() {
    $db = getSovereignDB();
    $rows = $db->query("SELECT tld, display_name, description, icon, category, price_usd, price_gsm, status, registrations_count FROM sovereign_tlds WHERE status IN ('active', 'coming_soon') ORDER BY FIELD(category, 'ecosystem', 'community', 'identity', 'commerce', 'creative'), tld")->fetchAll(PDO::FETCH_ASSOC);
...
function registerSovereignDomain() {
    ...
    $tldStmt = $db->prepare("SELECT id, tld, price_usd, price_gsm, status FROM sovereign_tlds WHERE tld = ? AND status = 'active'");
    $tldStmt->execute([$tld]);
    ...
    $insertStmt = $db->prepare("INSERT INTO sovereign_domains (domain_name, subdomain, tld_id, client_id, dns_a, status, expires_at, registration_years, registered_at) VALUES (?, ?, ?, ?, ?, 'active', ?, ?, NOW())");
    $insertStmt->execute([
        $domain,
        $subdomain,
        $tldData['id'],
    ...
    $logStmt->execute([$domainId, $clientId, "Registered {$domain} for {$years} year(s)"]);
    ...
    jsonResponse([
        ...
        'years' => $years,
        'price_usd' => (float)$tldData['price_usd'] * $years,
        'price_gsm' => (float)$tldData['price_gsm'] * $years,
    ], 201);
}
