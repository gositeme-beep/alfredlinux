...
<div class="page" id="server-hardening-page">
 ...
 <div class="panel"><div class="panel-header"><h3>What this is</h3></div><div class="panel-body">
  ...
  <ul style="color:var(--text2);font-size:13px;line-height:1.7;margin-top:10px">
   ...
   <li><strong>Fail2Ban:</strong> <code>gositeme-access-probes</code>, <code>recidive</code>, <code>sshd</code>, <code>sshd-ddos</code> — HTTP scanner paths are <em>not</em> normal Vapi tool URLs; Alfred/Vapi issues are usually load/DB, not this jail.</li>
   <li><strong>Docs:</strong> <a href="https://gositeme.com/docs/OPS-COMPLETE-PACK.md" target="_blank" rel="noopener">Ops master index</a> · <a href="https://gositeme.com/docs/GOHOSTME_VS_DIRECTADMIN.md" target="_blank" rel="noopener">GoHostMe vs DirectAdmin</a> · <a href="https://gositeme.com/docs/DIRECTADMIN_MIGRATION_CHECKLIST.md" target="_blank" rel="noopener">DA migration checklist</a> · <a href="https://gositeme.com/docs/ALFRED_VAPI_PHONE_PROMPT.md" target="_blank" rel="noopener">Alfred Vapi troubleshooting</a>.</li>
  </ul>
 </div></div>
...
const B=(p,o={})=>{o.headers={...o.headers,'Content-Type':'application/json'};if(token)o.headers.Authorization='Bearer '+token;return fetch(API+p,o).then(r=>r.json())};
...
function showModal(id){E(id).classList.add('active')}
function hideModal(id){E(id).classList.remove('active')}
function switchTab(btn,group){btn.closest('.tab-bar').querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));btn.classList.add('active');const p=btn.closest('[id$="-page"]')||btn.closest('.page');p.querySelectorAll('.tab-content[data-group="'+group+'"]').forEach(c=>c.style.display='none');E(btn.dataset.tab).style.display='block'}
function statusBadge(s){const c={active:'green',running:'green',online:'online',stopped:'red',offline:'red',errored:'red',error:'red'};return '<span class="badge '+(c[s]||'yellow')+'">'+s+'</span>'}

// AUTH
E('login-btn').onclick=async()=>{
 const u=E('login-user').value,p=E('login-pass').value;
 if(!u||!p)return T('Enter credentials');
 try{const r=await B('/api/auth/login',{method:'POST',body:JSON.stringify({username:u,password:p})});
 if(r.token){token=r.token;localStorage.setItem('ghm_token',token);E('login-screen').style.display='none';E('app').style.display='flex';loadPage('dashboard')}
 else T(r.error||'Login failed')}catch(e){T('Connection failed')}
};
E('logout-btn').onclick=()=>{token=null;localStorage.removeItem('ghm_token');E('app').style.display='none';E('login-screen').style.display='flex'};
if(token){E('login-screen').style.display='none';E('app').style.display='flex';loadPage('dashboard')}
...
document.querySelectorAll('.nav-item').forEach(n=>n.addEventListener('click',function(){
 ...
 document.querySelectorAll('.nav-item').forEach(x=>x.classList.remove('active'));this.classList.add('active');loadPage(p);
}));
...
async function loadServices(){
 ...
 E('services-list').innerHTML=d.services?.map(s=>`<tr><td>${s.name}</td><td>${statusBadge(s.status)}</td><td>${s.description||''}</td></tr>`).join('')||'<tr><td colspan="3">No services</td></tr>';
 }catch(e){E('services-list').innerHTML='<tr><td colspan="3">Error loading</td></tr>'}
}

// PROCESSES
async function loadProcesses(){
 try{const d=await B('/api/system/processes');
 E('processes-list').innerHTML=d.processes?.slice(0,50).map(p=>`<tr><td>${p.pid}</td><td>${p.user}</td><td>${p.cpu}%</td><td>${p.mem}%</td><td>${p.command?.substring(0,80)}</td></tr>`).join('')||'None'}catch(e){E('processes-list').innerHTML='<tr><td colspan="5">Error</td></tr>'}
}

// DOMAINS
async function loadDomains(){
 try{const d=await B('/api/domains');
 E('domains-list').innerHTML=d.domains?.map(dm=>`<tr><td>${dm.name||dm}</td><td>${dm.docRoot||'--'}</td><td>${dm.ssl?'<span class="badge green">Yes</span>':'<span class="badge red">No</span>'}</td><td><button class="btn btn-xs btn-danger" onclick="deleteDomain('${dm.name||dm}')">Delete</button></td></tr>`).join('')||'<tr><td colspan="4">No domains</td></tr>';
 }catch(e){E('domains-list').innerHTML='<tr><td colspan="4">Error</td></tr>'}
}
...
async function loadDNSSelect(){
 ...
 s.innerHTML=d.domains?.map(dm=>`<option>${dm.name||dm}</option>`).join('')||'';
 if(s.value)loadDNS(s.value);s.onchange=()=>loadDNS(s.value)}catch(e){}
}
async function loadDNS(domain){
 try{const d=await B('/api/dns/'+domain);
 E('dns-records').innerHTML=d.records?.map(r=>`<tr><td>${r.type}</td><td>${r.name}</td><td style="max-width:300px;overflow:hidden;text-overflow:ellipsis">${r.value}</td><td>${r.ttl||3600}</td><td><button class="btn btn-xs btn-danger" onclick="deleteDNSRecord('${domain}','${r.id||r.name}','${r.type}')">Del</button></td></tr>`).join('')||'<tr><td colspan="5">No records</td></tr>';
 }catch(e){E('dns-records').innerHTML='<tr><td colspan="5">Error</td></tr>'}
}
...
async function loadDatabases(){
 ...
 E('db-list').innerHTML=d.databases?.map(db=>`<tr><td>${db.name||db}</td><td>${db.tables||'--'}</td><td>${db.size||'--'}</td><td><button class="btn btn-xs" onclick="viewTables('${db.name||db}')">Tables</button> <button class="btn btn-xs btn-danger" onclick="deleteDatabase('${db.name||db}')">Drop</button></td></tr>`).join('')||'<tr><td colspan="4">No databases</td></tr>';
 }catch(e){E('db-list').innerHTML='<tr><td colspan="4">Error</td></tr>'}
}
...
async function viewTables(n){const r=await B('/api/databases/'+n+'/tables');E('sql-output').textContent=JSON.stringify(r.tables||r,null,2)}
async function runSQL(){const db=E('sql-db').value,q=E('sql-query').value;if(!q)return;const r=await B('/api/databases/query',{method:'POST',body:JSON.stringify({database:db,query:q})});E('sql-output').textContent=JSON.stringify(r.results||r,null,2)}

// FILES
async function loadFiles(path){
 if(path)currentPath=path;
 try{const d=await B('/api/files?path='+encodeURIComponent(currentPath));
 E('current-path').textContent=currentPath;
 E('file-list').innerHTML=d.files?.map(f=>{
  const fp=currentPath+'/'+f.name, isDir=f.type==='directory';
  return `<tr><td>${isDir?'📁':'📄'} ${isDir?`<a href="#" onclick="loadFiles('${fp}');return false">${f.name}</a>`:f.name}</td><td>${f.size||'--'}</td><td>${f.permissions||'--'}</td><td>${f.modified||'--'}</td><td>
   ${!isDir?`<button class="btn btn-xs" onclick="editFile('${fp}')">Edit</button> `:''}
   <button class="btn btn-xs" onclick="showRename('${fp}')">Ren</button>
   <button class="btn btn-xs" onclick="showChmod('${fp}')">Chmod</button>
   <button class="btn btn-xs btn-danger" onclick="deleteFile('${fp}')">Del</button></td></tr>`}).join('')||'<tr><td colspan="5">Empty</td></tr>';
 }catch(e){E('file-list').innerHTML='<tr><td colspan="5">Error</td></tr>'}
}
...
async function editFile(p){
 ...
 editingFile=p;E('editor-container').style.display='block';E('editor-path').textContent=p;E('editor-content').value=d.content||''}catch(e){T('Cannot read file')}
}
async function saveFile(){if(!editingFile)return;const r=await B('/api/files/write',{method:'POST',body:JSON.stringify({path:editingFile,content:E('editor-content').value})});T(r.message||r.error)}
function closeEditor(){E('editor-container').style.display='none';editingFile=null}
async function createFileAction(){const n=E('new-filename').value;if(!n)return T('Enter filename');const r=await B('/api/files/write',{method:'POST',body:JSON.stringify({path:currentPath+'/'+n,content:E('new-file-content').value||''})});T(r.message||r.error);hideModal('file-modal');loadFiles()}
...
async function loadSSL(){
 ...
 E('ssl-list').innerHTML='';
 for(const dm of (doms.domains||[])){
  const d=await B('/api/ssl/'+(dm.name||dm));
  E('ssl-list').innerHTML+=`<tr><td>${dm.name||dm}</td><td>${d.issuer||'None'}</td><td>${d.expires||'N/A'}</td><td>${d.valid?'<span class="badge green">Valid</span>':'<span class="badge red">Invalid/None</span>'}</td><td><button class="btn btn-xs btn-primary" onclick="requestSSL('${dm.name||dm}')">Issue/Renew</button></td></tr>`}}catch(e){E('ssl-list').innerHTML='<tr><td colspan="5">Error</td></tr>'}
}
...
async function loadApache(){
 try{const d=await B('/api/apache/vhosts');
 E('apache-list').innerHTML=d.vhosts?.map(v=>`<tr><td>${v.domain}</td><td>${v.port||80}</td><td>${v.docRoot||'--'}</td><td>${v.ssl?'<span class="badge green">SSL</span>':'HTTP'}</td></tr>`).join('')||'<tr><td colspan="4">No vhosts</td></tr>';
 }catch(e){E('apache-list').innerHTML='<tr><td colspan="4">Error</td></tr>'}
}
...
async function loadPM2(){
 ...
 E('pm2-list').innerHTML=d.processes?.map(p=>`<tr><td>${p.pm_id}</td><td>${p.name}</td><td>${statusBadge(p.pm2_env?.status||p.status)}</td><td>${p.pm2_env?.pm_uptime?new Date(p.pm2_env.pm_uptime).toLocaleString():'--'}</td><td>${p.monit?.cpu||0}%</td><td>${p.monit?.memory?Math.round(p.monit.memory/1048576)+'MB':'--'}</td><td>
  <button class="btn btn-xs" onclick="pm2Action('restart','${p.name}')">Restart</button>
  <button class="btn btn-xs" onclick="pm2Action('stop','${p.name}')">Stop</button>
  <button class="btn btn-xs" onclick="pm2Action('start','${p.name}')">Start</button>
  <button class="btn btn-xs" onclick="pm2Logs('${p.name}')">Logs</button>
 </td></tr>`).join('')||'<tr><td colspan="7">No processes</td></tr>';
 }catch(e){E('pm2-list').innerHTML='<tr><td colspan="7">Error</td></tr>'}
}
...
async function pm2Logs(name){const r=await B('/api/pm2/logs?name='+name);E('pm2-log-output').textContent=r.logs||r.error||'No logs';E('pm2-log-output').style.display='block'}

// EMAIL
async function loadEmail(){
 try{const d=await B('/api/email/accounts');
 E('email-list').innerHTML=d.accounts?.map(a=>`<tr><td>${a.user}@${a.domain}</td><td>${a.quota||'unlimited'}</td><td>${a.usage||'--'}</td><td><button class="btn btn-xs btn-danger" onclick="deleteEmail('${a.user}','${a.domain}')">Delete</button></td></tr>`).join('')||'<tr><td colspan="4">No accounts</td></tr>';
 }catch(e){E('email-list').innerHTML='<tr><td colspan="4">Error</td></tr>'}
}
...
async function loadCron(){
 ...
 E('cron-list').innerHTML=d.jobs?.map(j=>`<tr><td><code>${j.schedule||j.pattern}</code></td><td>${j.command}</td><td><button class="btn btn-xs btn-danger" onclick="deleteCron('${(j.schedule||j.pattern).replace(/'/g,"\\'")}')">Del</button></td></tr>`).join('')||'<tr><td colspan="3">No cron jobs</td></tr>';
 }catch(e){E('cron-list').innerHTML='<tr><td colspan="3">Error</td></tr>'}
}
...
async function deleteCron(p){if(!confirm('Delete cron job?'))return;const r=await B('/api/cron?pattern='+encodeURIComponent(p),{method:'DELETE'});T(r.message||r.error);loadCron()}

// BACKUPS
async function loadBackups(){
 try{const d=await B('/api/backups');
 E('backup-list').innerHTML=d.backups?.map(b=>`<tr><td>${b.name||b.filename}</td><td>${b.size||'--'}</td><td>${b.date||b.created||'--'}</td><td><button class="btn btn-xs btn-primary" onclick="T('Downloading...')">Download</button></td></tr>`).join('')||'<tr><td colspan="4">No backups</td></tr>';
 }catch(e){E('backup-list').innerHTML='<tr><td colspan="4">Error</td></tr>'}
}
async function createBackup(){T('Creating backup...');const r=await B('/api/backups/create',{method:'POST'});T(r.message||r.error);loadBackups()}

// IDE
async function loadIDE(){
 try{const d=await B('/api/ide/instances');
 E('ide-list').innerHTML=d.instances?.map(i=>`<tr><td>${i.name}</td><td>${i.port}</td><td>${statusBadge(i.status)}</td><td>${i.url||'--'}</td><td>
  <button class="btn btn-xs" onclick="ideCtl('start','${i.name}')">Start</button>
  <button class="btn btn-xs" onclick="ideCtl('stop','${i.name}')">Stop</button>
  <button class="btn btn-xs" onclick="ideCtl('restart','${i.name}')">Restart</button>
  <button class="btn btn-xs btn-danger" onclick="ideDelete('${i.name}')">Del</button>
 </td></tr>`).join('')||'<tr><td colspan="5">No instances</td></tr>';
 }catch(e){E('ide-list').innerHTML='<tr><td colspan="5">Error</td></tr>'}
}
async function ideCtl(a,n){const r=await B('/api/ide/control',{method:'POST',body:JSON.stringify({action:a,name:n})});T(r.message||r.error);loadIDE()}
