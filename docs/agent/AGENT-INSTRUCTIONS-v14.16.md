# Alfred Linux — AI Agent Build Instructions (v14.15)

> **Last updated:** 2026-07-04 ~20:06 EDT
...
**Fix:** Already patched in `lb-docker-inner-build.sh` line 410:
```bash
sed -i 's/cp -a /cp -al /g' /usr/lib/live/build/lb_binary_chroot /usr/lib/live/build/lb_chroot_includes /usr/lib/live/build/lb_binary_includes /usr/lib/live/build/bootstrap_cache 2>/dev/null || true
```
This makes it use hardlinks (zero extra space).
...
The `inner-lb-binary-only-fast.sh` mounts an overlayfs at `/work/build/chroot` merging three layers:
...
### 16. Build Runner Must Be SCP'd as a Script File
Complex docker commands inside tmux fail silently due to nested quoting (bash -> SSH -> tmux -> docker -> bash). The tmux session dies before docker even starts.

**Fix:** Write the build logic to a `.sh` file, SCP it to the server, then run it inside tmux:
```bash
tmux new-session -d -s alfred-build 'bash /home/gositeme/run-build.sh > /home/gositeme/alfred-runner.log 2>&1'
```
Never inline complex docker commands into tmux session strings.
...
**Cause:** V2 runner copies static bootloader templates from `config/bootloaders/` — these don't
contain the params from `auto/config --bootappend-live`
**Fix:** Patch `binary/boot/grub/grub.cfg` and `binary/isolinux/live.cfg` AFTER copying templates:
```bash
# Add to every linux/append line:
username=alfred user-default-groups=audio,video,render,dialout,sudo,adm,netdev,plugdev
quiet nvidia-drm.modeset=1
lsm=lockdown,integrity,tomoyo,apparmor,yama,bpf,landlock
```
...
**Fix**: Created `config/hooks/live/0900-copy-kernel.hook.binary` which runs INSIDE the `lb binary_hooks` phase (after the wipe, before `0964`). This hook does `mkdir -p binary/live && cp -a chroot/boot/vmlinuz-7* binary/live/ && cp -a chroot/boot/initrd.img-7* binary/live/`. This is the idiomatic live-build solution â€” binary hooks execute at the correct pipeline stage.
**NEVER stage kernel files before `lb binary` â€” they will be wiped.**
...
**Fix**: The files were never truly lost â€” they remained in git history. Recovery: `git checkout <last-good-commit> -- .` to restore all files from the parent commit. Then `git commit --amend` and `git push --force` to all remotes.
...
**Cause**: The `alpine/git` image's entrypoint IS `git`, so any argument is interpreted as a git subcommand. Alpine also does not ship bash â€” only `sh`.
...
Trap count as of v14.17: **118 traps**.
