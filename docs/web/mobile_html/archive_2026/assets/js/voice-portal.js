...
const CSRF = (document.querySelector('meta[name="csrf-token"]')?.content || window.CSRF_TOKEN || '').toString();
...
async function api(action, body) {
  ...
  if (body) {
    let payload = body;
    if (CSRF && payload && typeof payload === 'object' && !Array.isArray(payload) && !('csrf_token' in payload)) {
