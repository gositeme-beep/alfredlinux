...
const FORMULAS = {
    ...
    // v5.1 — Quadratic Gravity / Spacetime formulas
    einsteinHilbert: { name: 'Einstein-Hilbert', formula: 'S_EH = (1/16*pi*G) * integral R * sqrt(-g) d4x', calc: (R) => R / (16 * Math.PI * PHYS.G) },
    ricciSquared: { name: 'R² (Starobinsky)', formula: 'S_R2 = alpha * R^2', calc: (alpha, R) => alpha * R * R },
    ricciTensorSquared: { name: 'Rμν Rμν (Stelle)', formula: 'S_Ric = beta * Rμν * R^μν', calc: (beta, Rij) => beta * Rij },
    modifiedGravPotential: { name: 'Modified Φ(r)', formula: 'Phi = -GM/r * [1 + (1/3)*e^(-m0*r) - (4/3)*e^(-m2*r)]', calc: (M, r, alpha, beta) => {
        if (r < 1e-10) return -Infinity;
        const d0 = Math.abs(6*alpha + 2*beta); const d2 = Math.abs(2*beta);
        const m0 = d0>1e-10 ? 1/Math.sqrt(d0) : 0; const m2 = d2>1e-10 ? 1/Math.sqrt(d2) : 0;
        let c = 1; if(m0>0) c += (1/3)*Math.exp(-m0*r); if(m2>0) c -= (4/3)*Math.exp(-m2*r);
        return -PHYS.G*M/r * c;
    }},
    massiveSpin0: { name: 'Massive Scalar m₀', formula: 'm0^2 = 1/(6*alpha + 2*beta)', calc: (alpha, beta) => { const d=Math.abs(6*alpha+2*beta); return d>1e-10 ? 1/Math.sqrt(d) : Infinity; }},
    massiveSpin2: { name: 'Massive Graviton m₂', formula: 'm2^2 = -1/(2*beta)', calc: (beta) => { const d=Math.abs(2*beta); return d>1e-10 ? 1/Math.sqrt(d) : Infinity; }},
    schwarzschildRadius: { name: 'Schwarzschild r_s', formula: 'r_s = 2GM/c^2', calc: (M) => 2*PHYS.G*M/(PHYS.c*PHYS.c) },
    geodesicDeviation: { name: 'Geodesic Deviation', formula: 'd2xi/dtau2 = -R^a_bcd * u^b * xi^c * u^d', calc: null },
    // v5.2 — All Scientists: Searl, Pons/Fleischmann, Stan Meyer, Floyd Sweet, Keely, Schauberger, Grebennikov, Podkletnov, Biefeld-Brown, Bearden MEG
    searlSEG: { name: 'Searl Effect', formula: 'V_seg = B * dA/dt * N_rollers', calc: (B, dA_dt, N) => B * dA_dt * N },
    searlMagneticRamp: { name: 'Searl Ramp', formula: 'B_ramp = mu0*M*r*omega/(4*pi*R^3)', calc: (M,r,omega,R) => PHYS.mu_0*M*r*omega/(4*Math.PI*R*R*R) },
    searlIonization: { name: 'Searl Ion Layer', formula: 'n_e = (E_field/(e*lambda_D))^2', calc: (E,lam) => Math.pow(E/(PHYS.e*lam),2) },
    ponsFleischmannExcess: { name: 'Pons-Fleischmann', formula: 'Q_excess = Q_out - Q_in - Q_chem', calc: (Qout,Qin,Qchem) => Qout - Qin - Qchem },
    lenrCoulombBarrier: { name: 'LENR Coulomb', formula: 'E_barrier = e^2/(4*pi*eps0*r)', calc: (r) => PHYS.e*PHYS.e/(4*Math.PI*PHYS.eps_0*r) },
    lenrScreenedPotential: { name: 'LENR Screened', formula: 'V(r) = (e^2/4*pi*eps0*r)*e^(-r/lambda)', calc: (r,lam) => (PHYS.e*PHYS.e/(4*Math.PI*PHYS.eps_0*r))*Math.exp(-r/lam) },
    lenrFusionRate: { name: 'LENR Fusion Rate', formula: 'R = n1*n2*sigma*v', calc: (n1,n2,sigma,v) => n1*n2*sigma*v },
    stanMeyerWFC: { name: 'Meyer WFC', formula: 'V_resonant = 1/(2*pi*sqrt(L*C))', calc: (L,C) => 1/(2*Math.PI*Math.sqrt(L*C)) },
    stanMeyerGasProduction: { name: 'Meyer Gas Rate', formula: 'n_H2 = I*t/(2*F)', calc: (I,t) => I*t/(2*96485) },
    stanMeyerPulseFreq: { name: 'Meyer Pulse', formula: 'f_res = c/(2*d*sqrt(eps_r))', calc: (d,eps_r) => PHYS.c/(2*d*Math.sqrt(eps_r)) },
    stanMeyerVoltageIntensifier: { name: 'Meyer VIC', formula: 'V_out = L2/L1 * V_in * Q', calc: (L1,L2,Vin,Q) => (L2/L1)*Vin*Q },
    floydSweetVTA: { name: 'Sweet VTA', formula: 'P_out = V_rms * I_rms * cos(phi)', calc: (V,I,phi) => V*I*Math.cos(phi) },
    floydSweetConditionedMagnet: { name: 'Sweet Conditioning', formula: 'B_conditioned = B_r * sin(2*pi*f_c*t) * e^(-t/tau)', calc: (Br,fc,t,tau) => Br*Math.sin(2*Math.PI*fc*t)*Math.exp(-t/tau) },
    beardenMEG: { name: 'Bearden MEG', formula: 'COP_meg = (P_out + P_core)/(P_in)', calc: (Pout,Pcore,Pin) => (Pout+Pcore)/Pin },
    beardenMEGFlux: { name: 'MEG Flux Switch', formula: 'dPhi/dt = N*A*dB/dt', calc: (N,A,dB_dt) => N*A*dB_dt },
    keelySympatheticVib: { name: 'Keely Sympathetic', formula: 'f_symp = f0 * (n+1)/n', calc: (f0,n) => f0*(n+1)/n },
    keelyTriadicResonance: { name: 'Keely Triadic', formula: 'f1:f2:f3 = 1:2:3 (harmonic triad)', calc: (f) => ({ f1: f, f2: 2*f, f3: 3*f }) },
    keelyEtherDensity: { name: 'Keely Ether', formula: 'rho_ether = 3*H0^2/(8*pi*G)', calc: (H0) => 3*H0*H0/(8*Math.PI*PHYS.G) },
    schaubergerImplosion: { name: 'Schauberger Implosion', formula: 'v_imp = sqrt(2*g*h*(T_cold/T_hot))', calc: (h,Tc,Th) => Math.sqrt(2*9.81*h*(Tc/Th)) },
    schaubergerVortex: { name: 'Schauberger Vortex', formula: 'omega = v_tangential / r', calc: (vt,r) => vt/r },
    schaubergerRepulsine: { name: 'Repulsine Pressure', formula: 'dP = rho*omega^2*r^2/2', calc: (rho,omega,r) => rho*omega*omega*r*r/2 },
    grebennikov_CSE: { name: 'Grebennikov CSE', formula: 'F_cse = -dE_casimir/dz * N_cavities', calc: (dE_dz,N) => -dE_dz*N },
    grebennikov_cavityResonance: { name: 'Cavity Resonance', formula: 'f_cavity = c/(2*d)', calc: (d) => PHYS.c/(2*d) },
    podkletnovGravShield: { name: 'Podkletnov Shield', formula: 'delta_g/g = f(omega, B, T_c)', calc: (omega,B,Tc) => omega*B*1e-11/(Tc > 0 ? Tc : 1) },
    podkletnovImpulse: { name: 'Podkletnov Impulse', formula: 'F_grav = m*c*delta_v/delta_t', calc: (m,dv,dt) => m*PHYS.c*dv/dt },
    biefeldBrownForce: { name: 'Biefeld-Brown', formula: 'F = k*eps0*A*V^2/d^2', calc: (k,A,V,d) => k*PHYS.eps_0*A*V*V/(d*d) },
    biefeldBrownThrust: { name: 'Electrogravitic Thrust', formula: 'T = C*V*dV/dt', calc: (C,V,dV_dt) => C*V*dV_dt },
    lifterForce: { name: 'Lifter Force', formula: 'F_ion = I*d/mu_ion', calc: (I,d,mu) => I*d/mu },
    zpeUnifiedTuning: { name: 'ZPE Tuning', formula: 'f_zp = c/(4*L)', calc: (L) => PHYS.c/(4*L) },
    coulombLaw: { name: 'Coulomb Force', formula: 'F = k_e*q1*q2/r^2', calc: (q1,q2,r) => (1/(4*Math.PI*PHYS.eps_0))*q1*q2/(r*r) },
    faradayElectrolysis: { name: 'Faraday Electrolysis', formula: 'm = M*I*t/(z*F)', calc: (M,I,t,z) => M*I*t/(z*96485) },
};
...
var ZPE_TEMPLATES = [
    ...
    // v5.2 — Additional scientist experiments (12)
    { name: 'Searl Effect Generator (SEG)', category: 'zpe', researcher: 'John Searl (1946)', description: 'Rotating magnetic ring with neodymium roller sets generating anomalous EMF through magnetic ramp effect. Rollers orbit a central ring, inducing current in stator coils.', formula: 'V_seg = B * dA/dt * N_rollers', components: [
        { type: 'toroidal_inductor', x: 350, y: 280, value: 0.5 }, { type: 'inductor', x: 200, y: 200, value: 0.01 }, { type: 'inductor', x: 500, y: 200, value: 0.01 }, { type: 'inductor', x: 200, y: 380, value: 0.01 }, { type: 'inductor', x: 500, y: 380, value: 0.01 }, { type: 'capacitor', x: 350, y: 140, value: 1e-6 }, { type: 'em_field_probe', x: 620, y: 280, value: 0 }, { type: 'voltmeter', x: 620, y: 380, value: 0 }, { type: 'ground', x: 350, y: 460, value: 0 }
    ] },
    { name: 'Pons-Fleischmann Cold Fusion Cell', category: 'zpe', researcher: 'Pons & Fleischmann (1989)', description: 'Palladium cathode electrolytic cell in heavy water (D₂O) producing anomalous excess heat beyond chemical explanation. The original LENR experiment.', formula: 'Q_excess = Q_out - Q_in - Q_chem', components: [
        { type: 'battery', x: 100, y: 300, value: 5 }, { type: 'resistor', x: 200, y: 200, value: 10 }, { type: 'capacitor', x: 350, y: 280, value: 0.1 }, { type: 'resistor', x: 350, y: 180, value: 1 }, { type: 'ammeter', x: 500, y: 200, value: 0 }, { type: 'voltmeter', x: 500, y: 380, value: 0 }, { type: 'oscilloscope', x: 620, y: 280, value: 0 }, { type: 'ground', x: 350, y: 440, value: 0 }
    ] },
    { name: 'Stan Meyer Water Fuel Cell', category: 'zpe', researcher: 'Stanley Meyer (1990)', description: 'Resonant water-splitting cell using pulsed DC at the resonant frequency of the water capacitor. Claims gas production exceeding Faraday maximum through voltage intensification.', formula: 'f_res = c/(2*d*sqrt(eps_r)), V_out = (L2/L1)*V_in*Q', components: [
        { type: 'signal_generator', x: 100, y: 300, value: 12, frequency: 21400 }, { type: 'inductor', x: 220, y: 200, value: 0.5 }, { type: 'inductor', x: 220, y: 400, value: 0.005 }, { type: 'diode', x: 340, y: 200, value: 0.7 }, { type: 'capacitor', x: 460, y: 300, value: 1e-7 }, { type: 'resistor', x: 600, y: 300, value: 100000 }, { type: 'ammeter', x: 600, y: 180, value: 0 }, { type: 'oscilloscope', x: 600, y: 420, value: 0 }, { type: 'ground', x: 340, y: 440, value: 0 }
    ] },
    { name: 'Floyd Sweet VTA', category: 'zpe', researcher: 'Floyd Sweet (1988)', description: 'Vacuum Triode Amplifier using conditioned barium ferrite magnets with drive coils at specific frequencies, claimed to produce 500W output from milliwatt input.', formula: 'P_out = V_rms * I_rms * cos(phi)', components: [
        { type: 'signal_generator', x: 100, y: 300, value: 1, frequency: 60 }, { type: 'bifilar_coil', x: 250, y: 250, value: 0.1 }, { type: 'bifilar_coil', x: 250, y: 380, value: 0.1 }, { type: 'toroidal_inductor', x: 400, y: 300, value: 1.0 }, { type: 'capacitor', x: 550, y: 200, value: 1e-5 }, { type: 'resistor', x: 550, y: 400, value: 50 }, { type: 'voltmeter', x: 650, y: 280, value: 0 }, { type: 'ammeter', x: 650, y: 380, value: 0 }, { type: 'ground', x: 400, y: 460, value: 0 }
    ] },
    { name: 'Bearden MEG (Motionless EM Generator)', category: 'zpe', researcher: 'Tom Bearden (2002)', description: 'Motionless Electromagnetic Generator using permanent magnet flux path switching via control coils, claimed COP > 1.0 through asymmetric A-potential.', formula: 'COP = (P_out + P_core)/P_in', components: [
        { type: 'signal_generator', x: 100, y: 300, value: 5, frequency: 500 }, { type: 'bifilar_coil', x: 250, y: 250, value: 0.05 }, { type: 'toroidal_inductor', x: 400, y: 300, value: 0.5 }, { type: 'bifilar_coil', x: 550, y: 250, value: 0.05 }, { type: 'diode', x: 550, y: 380, value: 0.7 }, { type: 'capacitor', x: 650, y: 300, value: 1e-4 }, { type: 'resistor', x: 650, y: 420, value: 100 }, { type: 'ammeter', x: 250, y: 420, value: 0 }, { type: 'ground', x: 400, y: 460, value: 0 }
    ] },
    { name: 'Keely Sympathetic Vibratory Engine', category: 'zpe', researcher: 'John Keely (1872)', description: 'Resonant acoustic engine using tuning forks and chords at sympathetic frequencies. Triadic resonance (1:2:3 harmonic ratio) to create constructive interference in fluid/metal.', formula: 'f_symp = f0*(n+1)/n, triad = 1:2:3', components: [
        { type: 'signal_generator', x: 100, y: 280, value: 2, frequency: 440 }, { type: 'signal_generator', x: 100, y: 380, value: 2, frequency: 880 }, { type: 'inductor', x: 300, y: 280, value: 0.01 }, { type: 'inductor', x: 300, y: 380, value: 0.01 }, { type: 'capacitor', x: 450, y: 330, value: 1e-5 }, { type: 'em_field_probe', x: 600, y: 280, value: 0 }, { type: 'oscilloscope', x: 600, y: 400, value: 0 }, { type: 'ground', x: 300, y: 470, value: 0 }
    ] },
    { name: 'Schauberger Repulsine Vortex', category: 'zpe', researcher: 'Viktor Schauberger (1940s)', description: 'Implosion turbine using centripetal vortex flow. Cool inward spiral creates negative pressure zone. "Comprehend and copy nature" — water vortex energy concentration.', formula: 'v_imp = sqrt(2*g*h*(T_cold/T_hot)), dP = rho*omega^2*r^2/2', components: [
        { type: 'ac_source', x: 100, y: 300, value: 24, frequency: 50 }, { type: 'inductor', x: 250, y: 250, value: 0.1 }, { type: 'capacitor', x: 250, y: 400, value: 1e-4 }, { type: 'toroidal_inductor', x: 400, y: 300, value: 0.5 }, { type: 'em_field_probe', x: 550, y: 250, value: 0 }, { type: 'oscilloscope', x: 550, y: 400, value: 0 }, { type: 'ground', x: 250, y: 470, value: 0 }
    ] },
    { name: 'Grebennikov CSE Platform', category: 'zpe', researcher: 'Viktor Grebennikov (1988)', description: 'Cavity Structure Effect (CSE) using insect chitin microstructures. Nested cavity arrays creating Casimir-like force gradient. Claimed anti-gravity from multi-cavity geometry.', formula: 'F_cse = -dE_casimir/dz * N_cavities', components: [
        { type: 'casimir_plates', x: 250, y: 250, value: 1e-6, area: 0.01 }, { type: 'casimir_plates', x: 350, y: 250, value: 1e-6, area: 0.01 }, { type: 'casimir_plates', x: 450, y: 250, value: 1e-6, area: 0.01 }, { type: 'em_field_probe', x: 350, y: 380, value: 0 }, { type: 'voltmeter', x: 500, y: 380, value: 0 }, { type: 'ground', x: 350, y: 460, value: 0 }
    ] },
    { name: 'Podkletnov Gravity Shield', category: 'zpe', researcher: 'Eugene Podkletnov (1992)', description: 'Spinning superconducting YBCO disk (2-6 million RPM) above AC magnetic coils at ~70K. Claimed 0.3-2% weight reduction above the disk.', formula: 'delta_g/g = f(omega, B, T_c)', components: [
        { type: 'ac_source', x: 100, y: 300, value: 50, frequency: 1e6 }, { type: 'toroidal_inductor', x: 280, y: 250, value: 0.1 }, { type: 'toroidal_inductor', x: 280, y: 380, value: 0.1 }, { type: 'capacitor', x: 430, y: 300, value: 1e-8 }, { type: 'em_field_probe', x: 580, y: 250, value: 0 }, { type: 'voltmeter', x: 580, y: 380, value: 0 }, { type: 'ground', x: 280, y: 460, value: 0 }
    ] },
    { name: 'Biefeld-Brown Lifter', category: 'zpe', researcher: 'T. Townsend Brown (1928)', description: 'Asymmetric capacitor with high voltage (~30kV) producing thrust toward the smaller electrode. Electrogravitic effect or ion wind — the debate continues.', formula: 'F = k*eps0*A*V^2/d^2', components: [
        { type: 'high_voltage_cap', x: 200, y: 200, value: 1e-10 }, { type: 'resistor', x: 200, y: 380, value: 1e7 }, { type: 'ac_source', x: 100, y: 300, value: 30000, frequency: 0 }, { type: 'capacitor', x: 400, y: 280, value: 1e-11 }, { type: 'ammeter', x: 550, y: 200, value: 0 }, { type: 'em_field_probe', x: 550, y: 380, value: 0 }, { type: 'ground', x: 400, y: 440, value: 0 }
    ] },
    { name: 'LENR Ni-H Reactor (Rossi)', category: 'zpe', researcher: 'Andrea Rossi (2011)', description: 'Nickel-hydrogen lattice reactor with lithium aluminum hydride fuel. Heated to ~1200°C, claimed excess heat from nuclear transmutation of Ni-58 to Ni-62.', formula: 'COP = Q_out/Q_in, Q_nuclear = dm*c^2', components: [
        { type: 'ac_source', x: 100, y: 300, value: 220, frequency: 50 }, { type: 'resistor', x: 250, y: 250, value: 50 }, { type: 'resistor', x: 250, y: 380, value: 50 }, { type: 'capacitor', x: 400, y: 300, value: 1e-4 }, { type: 'voltmeter', x: 550, y: 250, value: 0 }, { type: 'ammeter', x: 550, y: 380, value: 0 }, { type: 'oscilloscope', x: 650, y: 300, value: 0 }, { type: 'ground', x: 250, y: 460, value: 0 }
    ] },
    { name: 'Magnetic Wankel Motor (Yildiz)', category: 'zpe', researcher: 'Muammer Yildiz (2013)', description: 'Permanent magnet motor with carefully arranged rotor/stator magnet geometry. Demonstrated at Delft University producing continuous rotation with no electrical input.', formula: 'T = r x F_mag, P = T*omega', components: [
        { type: 'toroidal_inductor', x: 300, y: 280, value: 0.5 }, { type: 'inductor', x: 200, y: 200, value: 0.01 }, { type: 'inductor', x: 400, y: 200, value: 0.01 }, { type: 'inductor', x: 200, y: 400, value: 0.01 }, { type: 'inductor', x: 400, y: 400, value: 0.01 }, { type: 'em_field_probe', x: 550, y: 280, value: 0 }, { type: 'oscilloscope', x: 550, y: 400, value: 0 }, { type: 'ground', x: 300, y: 470, value: 0 }
    ] }
];
