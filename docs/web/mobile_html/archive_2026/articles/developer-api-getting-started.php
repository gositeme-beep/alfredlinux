...
<p>The API base URL is <code>https://gositeme.com/api/v1</code>. All requests require the <code>Authorization</code> header with your API key.</p>

<h3>JavaScript (Node.js)</h3>
<pre><code>const response = await fetch('https://gositeme.com/api/v1/tools', {
    headers: {
...
response = requests.get('https://gositeme.com/api/v1/tools', headers=headers)
tools = response.json()
...
$ch = curl_init('https://gositeme.com/api/v1/tools');
...
<h3>JavaScript</h3>
<pre><code>const result = await fetch('https://gositeme.com/api/v1/tools/execute', {
    method: 'POST',
    headers: {
        'Authorization': 'Bearer ' + process.env.ALFRED_API_KEY,
...
<pre><code>result = requests.post(
    'https://gositeme.com/api/v1/tools/execute',
    headers=headers,
    json={
        'tool': 'content-writer',
        'input': {
...
<h3>PHP</h3>
<pre><code>$ch = curl_init('https://gositeme.com/api/v1/tools/execute');
curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => [
...
<pre><code>async function executeTool(tool, input, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const response = await fetch('https://gositeme.com/api/v1/tools/execute', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${process.env.ALFRED_API_KEY}`,
                    'Content-Type': 'application/json'
...
agent = requests.post(
    'https://gositeme.com/api/v1/agents',
    headers=headers,
    json={
        'name': 'BlogWriter',
...
task = requests.post(
    f'https://gositeme.com/api/v1/agents/{agent_id}/tasks',
    headers=headers,
    json={
...
while True:
    status = requests.get(
        f'https://gositeme.com/api/v1/agents/{agent_id}/tasks/{task_id}',
        headers=headers
    ).json()
...
const batch = await fetch('https://gositeme.com/api/v1/tools/batch', {
    method: 'POST',
    headers: {
        'Authorization': `Bearer ${process.env.ALFRED_API_KEY}`,
        'Content-Type': 'application/json'
