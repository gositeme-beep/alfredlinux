...
// Identity stats
$idStats = [
    'total_passports' => $db->query("SELECT COUNT(*) FROM fleet_passports")->fetchColumn(),
    'citizens'        => $db->query("SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'citizen'")->fetchColumn(),
    'visitors'        => $db->query("SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'visitor'")->fetchColumn(),
    'incarcerated'    => $db->query("SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'incarcerated'")->fetchColumn(),
    'restricted'      => $db->query("SELECT COUNT(*) FROM fleet_passports WHERE citizenship_status = 'restricted'")->fetchColumn(),
...
// Clearance breakdown
$clearances = $db->query("SELECT clearance_level, COUNT(*) as cnt FROM fleet_passports GROUP BY clearance_level ORDER BY cnt DESC")->fetchAll(PDO::FETCH_ASSOC);

// Platform origins
$platforms = $db->query("SELECT origin_platform, COUNT(*) as cnt FROM fleet_passports GROUP BY origin_platform ORDER BY cnt DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$deptPassports = $db->query("SELECT fp.domain as department, COUNT(*) as cnt,
    AVG(fp.reputation_score) as avg_reputation
    FROM fleet_passports fp
    GROUP BY fp.domain ORDER BY cnt DESC")->fetchAll(PDO::FETCH_ASSOC);
