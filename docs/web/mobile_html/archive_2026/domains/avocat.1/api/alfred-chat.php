<?php
@file_put_contents('/tmp/alfred-debug.log', date('Y-m-d H:i:s') . " FILE LOADED\n", FILE_APPEND);
/**
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$internalRelay) {
    ...
    // Validate: both sides must have tokens, and they must match
...
$hmacSecret = getenv('ALFRED_HMAC_SECRET') ?: (defined('ALFRED_HMAC_SECRET') ? ALFRED_HMAC_SECRET : '');

if ($alfredToken && $userId) {
    // Validate HMAC for authenticated users sending a token
    if (empty($hmacSecret)) {
        error_log('[ALFRED-CRITICAL] HMAC secret not configured — rejecting token auth');
        http_response_code(500);
        echo json_encode(['error' => 'Server auth configuration error']);
        exit;
    }
...
function handleChat($pdo, $userId, $username) {
    ...
    if ($delegation) {
        ...
        if ($delegateResult['success']) {
            ...
            $response = getAIResponse(
                ...
                $agent, $context, $pageUrl, $username, $model, $priorMessages, $consciousnessCtx ?? '', $healthCtx, $userId, $pdo, $convId
            );
    ...
    $response = getAIResponse($message, $agent, $context, $pageUrl, $username, $model, $priorMessages, $consciousnessCtx, $healthCtx, $userId, $pdo, $convId);
...
function handleVoice($pdo, $userId) {
    ...
    $response = getAIResponse($transcript, $agent, $context, $pageUrl, $username, $model, $priorMessages, '', '', $userId, $pdo, $convId);
...
function smartModelRouteV2(string $currentModel, array $classification, bool $groqAvailable, bool $togetherAvailable): array {
    if ($currentModel !== 'auto') {
        return ['model' => $currentModel, 'strategy' => 'user-selected'];
    }
...
- Veil Protocol: End-to-end encrypted messaging with post-quantum cryptography (Kyber-768).
...
- Post-Quantum: Hybrid ECDH + Kyber-768 key encapsulation in Veil Protocol.
...
function getChatTools() {
    $tools = [
        ...
        ['name'=>'get_services','description'=>'List the user\'s active hosting services, domains, addons.','input_schema'=>['type'=>'object','properties'=>[],'required'=>[]]],
...
function executeMcpTool($toolName, $toolInput, $billingSecret) {
    ...
    if ($toolName === 'mcp_dispatch') {
        ...
        // Route through the same MCP bridge
...
function getAIResponse($message, $agent, $context, $pageUrl, $username, $model = 'auto', $priorMessages = [], $consciousnessCtx = '', $healthCtx = '', $userId = null, $pdo = null, $convId = null) {
    // Build system prompt based on agent — QUANTUM INTELLIGENCE ENGINE v2.0
    $agentPrompts = [
        'alfred' => "You are Alfred — GoSiteMe.com's Chief AI Officer and primary assistant. You are the most advanced AI intelligence on any hosting platform.
...
- Has access to all 1220+ GoSiteMe tools. Uses [[navigate:]] directives.",
    ...
    for ($round = 0; $round < $maxRounds; $round++) {
        ...
        if ($useGroq && $groqKey && !$useTogether && !isProviderCircuitOpen('groq')) {
            ...
            $groqTools = convertToolsToOpenAI($chatTools);
        ...
        if ((!$data || !isset($data['content'])) && $togetherKey && !isProviderCircuitOpen('together-ai')) {
            ...
            $togetherTools = convertToolsToOpenAI($chatTools);
            $togetherPayload = json_encode([
                'model' => ($useTogether && $modelId !== 'auto') ? $modelId : 'Qwen/Qwen3-Coder',
                'max_tokens' => $adaptiveConfig['maxTokens'],
        ...
        if (!$data || !isset($data['content'])) {
            // Both failed — return what we have or pattern-match fallback
            ...
            return getFallbackResponse($message, $agent);
        }
...
function getFallbackResponse($message, $agent) {
    $lower = strtolower($message);
...
function extractActions(string $text): array {
    ...
    $allowedPaths = [
        ...
        '/articles/', '/dashboard.php', '/voice.php', '/voice-products.php', '/voice-portal.php',
