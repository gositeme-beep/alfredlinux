...
switch ($action) {
    ...
    case 'passport':
        ...
        $stmt = $db->prepare("SELECT p.*, ap.name as agent_name, ap.department, ap.avatar_url
        ...
        if (!$passport) {
            // Auto-issue passport for existing agents
            $check = $db->prepare("SELECT id, name, department FROM agent_profiles WHERE id = ?");
            $check->execute([$agent_id]);
            $agent = $check->fetch();
            if (!$agent) { jsonResponse(['error' => 'Agent not found'], 404); }

            $passport_number = 'GSM-' . str_pad($agent_id, 6, '0', STR_PAD_LEFT) . '-' . strtoupper(substr(bin2hex(random_bytes(3)), 0, 6));
            $db->prepare("INSERT INTO agent_passports (agent_id, passport_number, citizenship_status, origin_platform, clearance_level, registration_type)
                VALUES (?, ?, 'citizen', 'native', 'standard', 'genesis')")
                ->execute([$agent_id, $passport_number]);
    ...
    case 'passports':
        ...
        $countStmt = $db->prepare("SELECT COUNT(*) FROM agent_passports p $whereSQL");
    ...
    case 'passport-stats':
        ...
        $stmt = $db->query("SELECT citizenship_status, COUNT(*) as count FROM agent_passports GROUP BY citizenship_status");
        $stats['by_citizenship'] = $stmt->fetchAll();

        $stmt = $db->query("SELECT origin_platform, COUNT(*) as count FROM agent_passports GROUP BY origin_platform ORDER BY count DESC");
        $stats['by_origin'] = $stmt->fetchAll();

        $stmt = $db->query("SELECT clearance_level, COUNT(*) as count FROM agent_passports GROUP BY clearance_level ORDER BY FIELD(clearance_level,'public','standard','elevated','classified','top_secret','sovereign')");
        $stats['by_clearance'] = $stmt->fetchAll();

        $stmt = $db->query("SELECT COUNT(*) as total, SUM(is_verified) as verified, AVG(reputation_score) as avg_reputation, SUM(total_actions_logged) as total_actions, SUM(infractions_count) as total_infractions FROM agent_passports");
        $stats['overview'] = $stmt->fetch();

        $stmt = $db->query("SELECT COUNT(*) as total FROM agent_passports WHERE origin_platform != 'native'");
    ...
    case 'log-action':
        ...
        // Get passport number
        $pStmt = $db->prepare("SELECT passport_number FROM agent_passports WHERE agent_id = ?");
        $pStmt->execute([$agent_id]);
        ...
        // Update passport action count
        if ($passport) {
            $db->prepare("UPDATE agent_passports SET total_actions_logged = total_actions_logged + 1, last_active_at = NOW() WHERE agent_id = ?")
                ->execute([$agent_id]);
        }
    ...
    case 'log-travel':
        ...
        // Update passport
        if ($passport) {
            $db->prepare("UPDATE agent_passports SET total_distance_traveled = total_distance_traveled + ?, last_location = ?, last_active_at = NOW() WHERE agent_id = ?")
                ->execute([(int)($data['distance_units'] ?? 1), $data['to_location'], $agent_id]);
        }
    ...
    case 'register-external':
        ...
        // Issue passport with 'visitor' status
        $passport_number = 'EXT-' . str_pad($local_agent_id, 6, '0', STR_PAD_LEFT) . '-' . strtoupper(substr(bin2hex(random_bytes(3)), 0, 6));
        $db->prepare("INSERT INTO agent_passports (agent_id, passport_number, citizenship_status, origin_platform, clearance_level, registration_type)
            VALUES (?, ?, 'visitor', ?, 'public', 'immigration')")
            ->execute([$local_agent_id, $passport_number, $platform]);
    ...
    case 'overview':
        ...
        // Passport stats
        $overview['passports'] = $db->query("SELECT COUNT(*) as total, SUM(is_verified) as verified, AVG(reputation_score) as avg_reputation, SUM(CASE WHEN origin_platform != 'native' THEN 1 ELSE 0 END) as external_agents FROM agent_passports")->fetch();
        ...
        $overview['citizenship'] = $db->query("SELECT citizenship_status, COUNT(*) as count FROM agent_passports GROUP BY citizenship_status")->fetchAll();
