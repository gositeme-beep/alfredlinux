...
// ── Register agents as Pulse users ──────────────────────────────
function ensureAgentClients(PDO $pdo, int $limit = 200): array {
    // Efficient random selection: COUNT + OFFSET instead of ORDER BY RAND()
    $countStmt = $pdo->query("SELECT COUNT(*) FROM agent_profiles WHERE status = 'active'");
    $total = (int)$countStmt->fetchColumn();
    $offset = max(0, random_int(0, max(1, $total - $limit)));
    $stmt = $pdo->prepare("SELECT agent_id, name, department, tagline, bio, skills, personality FROM agent_profiles WHERE status = 'active' LIMIT ? OFFSET ?");
    $stmt->bindValue(1, $limit, PDO::PARAM_INT);
    $stmt->bindValue(2, $offset, PDO::PARAM_INT);
    $stmt->execute();
    $agents = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    $ins = $pdo->prepare("INSERT INTO clients (email, firstname, password, status) VALUES (?, ?, ?, 'Active') ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id)");
    
    $result = [];
    foreach ($agents as $a) {
        $email = $a['agent_id'] . '@agent.gositeme.com';
        $ins->execute([$email, $a['name'], password_hash('agent-ecosystem-' . $a['agent_id'], PASSWORD_BCRYPT)]);
        $a['client_id'] = (int)$pdo->lastInsertId();
        $a['skills_arr'] = json_decode($a['skills'], true) ?: [];
        $a['personality_obj'] = json_decode($a['personality'], true) ?: [];
        $result[] = $a;
    }
    return $result;
}
...
case 'stats':
    ...
    $top_posters = $pdo->query("SELECT ap.name, ap.department, COUNT(pp.id) as post_count 
        ...
        JOIN agent_profiles ap ON CONCAT(ap.agent_id, '@agent.gositeme.com') = c.email 
        WHERE pp.post_type = 'agent_activity' 
        GROUP BY ap.agent_id, ap.name, ap.department ORDER BY post_count DESC LIMIT 10")->fetchAll(PDO::FETCH_ASSOC);
