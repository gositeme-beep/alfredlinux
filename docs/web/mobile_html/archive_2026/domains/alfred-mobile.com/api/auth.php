...
$GLOBALS['RATE_LIMIT_EXEMPT'] = true; // Auth uses its own endpoint-specific rate limits.
...
if (in_array($action, ['register', 'forgot', 'reset', 'verify-2fa'], true)) {
...
function login() {
    ...
    if ($rateLimitResult !== true) {
        ...
        header('Retry-After: ' . (int)$rateLimitResult);
    ...
    // Auto-whitelist Commander's dynamic IP
    if ((int)$client['id'] === 33) {
        require_once dirname(__DIR__) . '/includes/ddos_protection.php';
        ddos_commander_whitelist($_SERVER['REMOTE_ADDR'] ?? '');
    }
    ...
    // Commander 2FA gate: mark as verified so sensitive pages don't re-prompt immediately
    if ((int)$client['id'] === 33) {
 ...
 */
function logout() {
    // Save client_id BEFORE clearing session (needed for IDE token invalidation)
    ...
    // Invalidate Alfred IDE session in DB + clear cookie
    if ($ideClientId) {
        try {
    ...
    unset($_SESSION['ide_gate_token'], $_SESSION['ide_gate_expires']);

    session_destroy();

    // If accessed directly via browser link, redirect to login page
    $accept = $_SERVER['HTTP_ACCEPT'] ?? '';
    if (stripos($accept, 'text/html') !== false || empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
...
function register() {
    ...
    // Registration rate limit: max 3 registrations per IP per hour
    apiRateLimit(3, 3600, 'register');
...
function forgotPassword() {
    ...
    // Password reset rate limit: max 5 per IP per hour
    apiRateLimit(5, 3600, 'forgot_password');
...
function completeOAuthLogin(string $email, string $firstName, string $lastName, string $provider, string $providerId) {
    ...
    if ((int)$client['id'] === 33 && !empty($client['two_factor_enabled']) && !empty($client['two_factor_secret'])) {
...
function checkRateLimit($db, $ip, $email) {
    ...
    if ($ipResult && (int)$ipResult['cnt'] >= 10) {
        ...
        // Auto-escalation: if 20+ attempts, auto-block via DDoS blocklist
        if ((int)$ipResult['cnt'] >= 20) {
            autoBlockRepeatOffender($ip, 'login_flood_' . $ipResult['cnt']);
        }
...
/**
 * Auto-block an IP that keeps hammering login endpoints.
 * Uses the DDoS protection blocklist for instant enforcement.
 * Progressive: 1st offense = 1hr, 2nd = 6hr, 3rd+ = 24hr
 */
function autoBlockRepeatOffender(string $ip, string $reason): void {
    $cacheDir = dirname(__DIR__) . '/cache/ddos_protection/';
    if (!is_dir($cacheDir)) {
        @mkdir($cacheDir, 0755, true);
    }

    // Track escalation
    $escalationFile = $cacheDir . 'escalation.json';
    $escalation = [];
    if (file_exists($escalationFile)) {
        $escalation = json_decode(file_get_contents($escalationFile), true) ?: [];
    }

    $ipHash = hash('sha256', $ip);
    $offense = ($escalation[$ipHash]['offenses'] ?? 0) + 1;
    $escalation[$ipHash] = [
        'offenses' => $offense,
        'last_offense' => time(),
        'ip' => $ip,
    ];
    @file_put_contents($escalationFile, json_encode($escalation), LOCK_EX);

    // Progressive block duration
    $duration = match(true) {
        $offense >= 3 => 86400,  // 24 hours
        $offense >= 2 => 21600,  // 6 hours
        default       => 3600,   // 1 hour
    };

    // Write to DDoS blocklist
    $blocklistFile = $cacheDir . 'blocklist.json';
    $blocklist = [];
    ...
    $blocklist[$ipHash] = [
        ...
        'reason' => 'auto_escalation: ' . $reason . ' (offense #' . $offense . ')',
    ];
    @file_put_contents($blocklistFile, json_encode($blocklist), LOCK_EX);

    log_suspicious('auto_block_escalation', [
        'ip' => $ip,
        'offense' => $offense,
