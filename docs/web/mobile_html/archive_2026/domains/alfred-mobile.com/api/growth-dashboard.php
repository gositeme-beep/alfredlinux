...
switch ($action) {
    ...
    case 'overview':
        ...
        try {
            $kpis['fleet_registry_total'] = (int) $db->query("SELECT TABLE_ROWS FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='alfred_agent_registry'")->fetchColumn();
            $kpis['fleet_passports_issued'] = (int) $db->query("SELECT TABLE_ROWS FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='fleet_passports'")->fetchColumn();
        } catch (\Throwable $e) {
            $kpis['fleet_registry_total'] = 0;
            $kpis['fleet_passports_issued'] = 0;
        }
