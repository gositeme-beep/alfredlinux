...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'photography-pointer-registration-dismissed',
				true
			)
		):
					...
					jQuery('#pp_panel_registration_a').first().pointer( 
						{
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'photography-pointer-registration',
...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'photography-pointer-required-plugins-dismissed',
				true
			)
		):
					...
					jQuery('#menu-appearance').first().pointer( 
						{
							content:
								...
								"<p>Please see detailed instruction <a href='https://themes.themegoods.com/photography/doc/register-your-purchase/' target='_blank'>here</a>.</p>",
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'photography-pointer-required-plugins',
	...
	add_action(
		...
		function() {
			...
			if (
				! get_user_meta(
					get_current_user_id(),
					'photography-pointer-loftloader-dismissed',
					true
				)
			):
						...
						jQuery('#menu-settings').first().pointer( 
							{
								...
								close: function() {
									jQuery.post(
										ajaxurl,
										{
											pointer: 'photography-pointer-loftloader',
