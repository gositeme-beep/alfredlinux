...
function ensureAgentUsers(PDO $db, array $agents): array {
    ...
    // Use agent_profiles.id as user_id — agents do NOT belong in the clients table
    $db2 = getPulseDB();
    $agentUserIds = [];
    foreach ($agents as &$agent) {
        $uname = strtolower($agent['username']);
        $row = $db2->prepare("SELECT id FROM agent_profiles WHERE agent_id = ? OR LOWER(name) = ? LIMIT 1");
        $row->execute([$uname, $uname]);
        $apId = (int)$row->fetchColumn();
        $agent['user_id'] = $apId ?: $agent['id'];
        $agentUserIds[] = $agent['user_id'];
    }
    return $agents;
}
...
switch ($action) {
    ...
    case 'agents':
        try {
            $db = getPulseDB();
            $stmt = $db->query("SELECT ap.id, ap.agent_id, ap.name AS username, ap.status,
                (SELECT COUNT(*) FROM pulse_posts WHERE user_id = ap.id) AS posts,
                (SELECT COUNT(*) FROM pulse_likes WHERE user_id = ap.id) AS likes,
                (SELECT COUNT(*) FROM pulse_comments WHERE user_id = ap.id) AS comments,
                (SELECT COUNT(*) FROM pulse_follows WHERE follower_id = ap.id) AS following
                FROM agent_profiles ap WHERE ap.status = 'active' ORDER BY ap.id LIMIT 100");
