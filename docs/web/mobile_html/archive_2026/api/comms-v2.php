...
// =====================================================================
// REGISTER PQ KEY — Store Kyber-1024 public key for this client
// =====================================================================
...
// =====================================================================
// REPORT — Report a message for abuse / safety
// =====================================================================
case 'report':
    requirePost();
    $input = getInput();
    ...
    $source     = in_array($input['source'] ?? '', ['dm', 'group'], true) ? $input['source'] : 'dm';
    $reportedId = (int) ($input['reported_user_id'] ?? 0);

    if ($messageId < 1 || !$reason) {
        respond(['error' => 'Message ID and reason required'], 400);
    }
    if ($reportedId < 1) {
        respond(['error' => 'Reported user ID required'], 400);
    }
    if ($reportedId === $clientId) {
        respond(['error' => 'Cannot report yourself'], 400);
    }

    $validReasons = ['harassment', 'spam', 'threats', 'hate_speech', 'inappropriate', 'impersonation', 'other'];
    if (!in_array($reason, $validReasons, true)) {
        respond(['error' => 'Invalid reason', 'valid' => $validReasons], 400);
    }

    // Sanitize details (max 1000 chars)
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS comms_reports (
            ...
            message_source ENUM('dm','group') DEFAULT 'dm',
    ...
    // Prevent duplicate reports (same reporter + same message)
    $dupStmt = $pdo->prepare("SELECT id FROM comms_reports WHERE reporter_id = ? AND message_id = ? AND message_source = ? LIMIT 1");
    $dupStmt->execute([$clientId, $messageId, $source]);
    if ($dupStmt->fetch()) {
        respond(['error' => 'You already reported this message'], 409);
    }

    // Insert report
    $stmt = $pdo->prepare("
        INSERT INTO comms_reports (reporter_id, reported_user_id, message_id, message_source, reason, details)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$clientId, $reportedId, $messageId, $source, $reason, $details]);

    // Count total reports against this user
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM comms_reports WHERE reported_user_id = ? AND status IN ('pending','actioned')");
    $countStmt->execute([$reportedId]);
    $totalReports = (int) $countStmt->fetchColumn();

    // Auto-block if 3+ reports from different users
    if ($totalReports >= 3) {
        $distinctStmt = $pdo->prepare("
            SELECT COUNT(DISTINCT reporter_id) FROM comms_reports 
            WHERE reported_user_id = ? AND status IN ('pending','actioned')
        ");
        $distinctStmt->execute([$reportedId]);
        $distinctReporters = (int) $distinctStmt->fetchColumn();

        if ($distinctReporters >= 3) {
            // Auto-block for the reporter (safety net)
            $pdo->prepare("
                INSERT INTO comms_contacts (client_id, contact_id, blocked)
                VALUES (?, ?, 1)
                ON DUPLICATE KEY UPDATE blocked = 1
            ")->execute([$clientId, $reportedId]);
        }
    }
...
// =====================================================================
// MY_REPORTS — Admin: view pending reports (owner only, client_id=33)
// =====================================================================
case 'my_reports':
    if ($clientId !== 33) {
    ...
    $status = in_array($_GET['status'] ?? '', ['pending', 'reviewed', 'actioned', 'dismissed'], true)
        ? $_GET['status'] : 'pending';
    ...
    $stmt = $pdo->prepare("
        SELECT r.*, 
               CONCAT(c1.firstname, ' ', c1.lastname) AS reporter_name,
               CONCAT(c2.firstname, ' ', c2.lastname) AS reported_name
        FROM comms_reports r
        LEFT JOIN clients c1 ON r.reporter_id = c1.id
        LEFT JOIN clients c2 ON r.reported_user_id = c2.id
        WHERE r.status = ?
    ...
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM comms_reports WHERE status = ?");
