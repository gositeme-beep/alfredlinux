...
// Ensure dedupe_hash column exists (for tables created before this column was added)
try {
    $db->exec("ALTER TABLE black_vault_news ADD COLUMN IF NOT EXISTS dedupe_hash VARCHAR(64) DEFAULT NULL");
    $db->exec("ALTER TABLE black_vault_news ADD UNIQUE INDEX IF NOT EXISTS uq_dedupe (dedupe_hash)");
...
switch ($action) {
    ...
    case 'news':
        ...
        foreach ($rows as &$row) {
            if (!empty($row['content']) && function_exists('gqes_is_encrypted') && gqes_is_encrypted($row['content'])) {
                $decrypted = vault_decrypt($row['content']);
                $row['content'] = ($decrypted !== false) ? $decrypted : $row['content'];
            }
        }
        unset($row);
        jsonResponse(['news' => $rows]);
        break;
