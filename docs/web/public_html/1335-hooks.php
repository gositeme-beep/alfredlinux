...
?>
<!DOCTYPE html>
<html lang="<?php echo htmlspecialchars($al_lang); ?>" dir="<?php echo htmlspecialchars($al_dir); ?>">
<head>
<meta charset="UTF-8">
    ...
    <div class="grid">
        ...
        if (is_dir($hooks_dir)) {
            ...
            foreach ($files as $file) {
                if ((strpos($file, '.hook.chroot') !== false || strpos($file, '.hook.binary') !== false) && strpos($file, '.bak') === false) {
                    $prefix = substr($file, 0, 4);
                    if (is_numeric($prefix) && !isset($seen_prefixes[$prefix])) {
